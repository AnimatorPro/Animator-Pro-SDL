...
int main(void)
{
	...
	if (ok) {
		ok &= check(poco_vm_compile_file(vm, FIXTURE_PATH("run.poc"), &program) == POCO_STATUS_OK,
	...
	poco_vm_destroy(vm);
	ok &= check(host_allocator_calls == 0, "Poco must not call Animator's allocator symbols");
	return ok ? 0 : 1;
}
