...
static PocoStatus module_loaded(void* user_data, const PocoModuleInfo* module)
{
	...
	if (strcmp(module->requested_name, state->expected_name) != 0) {
		fprintf(stderr, "poco_module_hooks: expected hook for %s, received %s\n",
				state->expected_name, module->requested_name);
		return POCO_STATUS_INTERNAL_ERROR;
	}
	++state->load_count;
...
static int compile_with_hooks(const char* script_name, HookState* state, PocoStatus expected_status,
							  int expect_run, PocoModuleLoadHook load_hook)
{
	...
	if (snprintf(script_path, sizeof(script_path), "%s/%s", POCO_MODULE_HOOK_FIXTURE_DIR,
	...
	if (ok && expect_run) {
		ok &= check(poco_vm_run(vm, program, NULL, &result) == POCO_STATUS_OK, "run valid module");
