...
static const PocoBindingContract bad_depth_contract = {
	...
	.pointer_contract_count =
		sizeof(bad_depth_pointer_contracts) / sizeof(bad_depth_pointer_contracts[0]),
};
...
static const PocoBinding bindings[] = {
	...
	{"int read_variadic(char *bytes, int count, ...);", (PocoNativeFunction)read_variadic,
	 &read_contract},
...
static const PocoLibrary bad_depth_library = {
	"ffi-bad-depth",
	bad_depth_bindings,
	sizeof(bad_depth_bindings) / sizeof(bad_depth_bindings[0]),
	NULL,
	NULL,
	NULL,
};
...
static PocoStatus run_fixture(PocoVm* vm, const char* name, int32_t* out_result)
{
	...
	snprintf(source_name, sizeof(source_name), "%s/%s", POCO_FFI_BOUNDS_FIXTURE_DIR, name);
...
static PocoVm* make_vm(int register_borrowed)
{
	...
	CHECK(poco_vm_register_library(vm, &library) == POCO_STATUS_OK, "register contracted bindings");
...
static void test_standard_string_contracts(void)
{
	...
	CHECK(poco_vm_create(NULL, &vm) == POCO_STATUS_OK, "create VM for standard string contracts");
	if (vm == NULL) {
		return;
	}
...
static void test_pointer_depth_validation(void)
{
	...
	CHECK(poco_vm_create(NULL, &vm) == POCO_STATUS_OK, "create VM for pointer-depth validation");
	if (vm == NULL) {
		return;
	}
