...
{
	return generic_module_initialized == 1 ? 42 : -1;
}

static PocoStatus generic_module_initialize(const PocoModuleHost *host,
	void **out_module_data)
{
	PocoDiagnostic diagnostic;

	if (host == NULL || out_module_data == NULL ||
		host->abi_version != POCO_MODULE_HOST_ABI_VERSION ||
		host->report_diagnostic == NULL || host->get_service == NULL)
...
static const PocoModuleDescriptor generic_module = {
	...
	"generic-module",
	&generic_library,
	generic_module_initialize,
	generic_module_cleanup,
};
