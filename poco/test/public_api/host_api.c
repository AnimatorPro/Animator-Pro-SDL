...
int main(void)
{
	...
	PocoLibrary first_library = {
		"test.first", first_bindings, 1, first_library_init, first_library_cleanup, NULL,
	};
	...
	if (!check(poco_vm_create(&vm_options, &vm) == POCO_STATUS_OK, "create VM") ||
		...
		!check(poco_vm_run(vm, program, &run_options, &result) == POCO_STATUS_OK,
			"run custom binding") ||
		!check(result == 42, "fixed-argument result") ||
		!check(lifecycle_event_count == 4 && lifecycle_events[0] == 1 &&
				lifecycle_events[1] == 2 && lifecycle_events[2] == 3 &&
				lifecycle_events[3] == 4,
			"deterministic lifecycle order")) {
		fprintf(stderr, "last diagnostic: status=%d message=%s\n",
			last_diagnostic_status, diagnostic_message);
	...
	if (!check(poco_vm_compile_file(vm, FIXTURE_PATH("cancel.poc"), &cancel_program) == POCO_STATUS_OK,
	...
	if (!check(poco_vm_compile_file(vm, FIXTURE_PATH("invalid.poc"), &program) == POCO_STATUS_REPORTED,
