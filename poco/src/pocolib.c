	...
	if (err < Success) {
		switch (err) {
			...
			case Err_zero_divide:
				...
				break;
			case Err_float:
				strcpy(buf, "Floating point math error");
				break;
			case Err_invalid_FILE:
				strcpy(buf, "Invalid FILE pointer");
				break;
			case Err_index_small:
				strcpy(buf, "Pointer or array index too small");
				break;
			case Err_index_big:
				strcpy(buf, "Pointer or array index too large");
				break;
			case Err_poco_free:
				strcpy(buf, "Trying to free an invalid block");
				break;
			case Err_free_null:
				strcpy(buf, "Trying to free NULL");
				break;
			case Err_free_resources:
				strcpy(buf, "Poco resource management damaged");
				break;
			case Err_zero_malloc:
				strcpy(buf, "Negative or zero allocation size");
				break;
			case Err_string:
				strcpy(buf, "String operation failed");
				break;
			case Err_fread_buf:
				strcpy(buf, "Read past Poco buffer");
				break;
			case Err_fwrite_buf:
				strcpy(buf, "Write past Poco buffer");
				break;
			...
			case Err_poco_lib_no_entry:
				strcpy(buf, "Poco library entry point missing");
				break;
			case Err_poco_lib_invalid:
				strcpy(buf, "Poco library is invalid");
				break;
			...
			case Err_poco_lib_empty:
				...
				break;
			case Err_poco_ffi_bounds:
				strcpy(buf, "Poco native binding exceeded a pointer span");
				break;
			default:
				snprintf(buf, ERRTEXT_SIZE, "Error code %d", err);
				break;
		}
	}
	return strlen(buf);
