...
PocoStatus poco_register_standard_library_catalog(PocoVm* vm)
{
	const PocoLibrary* const catalog[] = {
		poco_standard_console_library(), poco_standard_string_library(),
		poco_standard_memory_library(),  poco_standard_file_library(),
		poco_standard_math_library(),    poco_standard_path_library(),
	};
	const PocoLibraryRuntimeCleanup cleanup[] = {
		NULL, NULL, poco_standard_memory_cleanup, poco_standard_file_cleanup, NULL, NULL,
	};
	size_t index;
	PocoStatus status;

	for (index = 0; index < Array_els(catalog); ++index) {
		status = poco_vm_register_library_with_runtime_cleanup(vm, catalog[index], cleanup[index]);
