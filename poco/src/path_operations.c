/*
 * Host-neutral path operations for Poco: fnsplit and fnmerge.
 *
 * Provides the capacity-aware split/merge primitives and the standard path
 * library registered by the embeddable runtime.  Both operations accept
 * Unix/macOS and Windows path spellings.  The legacy DOS-contract binding
 * table is shared with the standalone CLI's legacy library list.
 */
					...
					size_t suffix_capacity)
{
	...
	if (path == NULL || *path == '\0' || device == NULL || dir == NULL || file == NULL ||
		suffix == NULL) {
		return Err_null_ref;
	}
...
static int po_fnsplit(char* path, char* device, char* dir, char* file, char* suffix)
{
	...
	status = poco_path_split(path, split_device, sizeof(split_device), split_dir, sizeof(split_dir),
							 split_file, sizeof(split_file), split_suffix, sizeof(split_suffix));
	if (status < Success) {
		return status;
	}
...
{
	static PocoBinding bindings[Array_els(poco_path_legacy_bindings)];
	static const PocoLibrary library = {
		POCO_STANDARD_PATH_LIBRARY_ID, bindings, Array_els(bindings), NULL, NULL, NULL,
	};
	static int initialized;
	size_t index;

	if (!initialized) {
		for (index = 0; index < Array_els(poco_path_legacy_bindings); ++index) {
			bindings[index].prototype = poco_path_legacy_bindings[index].proto;
			bindings[index].function = (PocoNativeFunction)poco_path_legacy_bindings[index].func;
			bindings[index].contract = poco_path_legacy_bindings[index].contract;
		}
		initialized = 1;
	}
	return &library;
