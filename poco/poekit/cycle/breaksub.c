...
{
	Rect* this = pthis.pt;
	SET_RECT(GetPicScreen(), color, this->x, this->y, this->width, this->height);
}
...
Boolean rect_hit(Popot pthis, Popot pd)
{
	...
	return -rad <= x + this->width && rad >= x && -rad <= y + this->height && rad >= y;
}
...
int brick_collide(Popot pball, Popot pbrick, int count)
{
	...
	for (i = 0; i < count; ++i) {
		if (brick->exists) {
			x = brick->draw.x - bx;
			y = brick->draw.y - by;
			if (-rad <= x + brick->draw.width && rad >= x && -rad <= y + brick->draw.height &&
				rad >= y) {
				return (i);
			}
		}
...
static Lib_proto calls[] = {
	...
	{oval_draw, "oval_draw(struct disk *this, int color, int xrat, int yrat);"},
	{rect_hit, "Boolean rect_hit(struct rect *this, struct disk *d);"},
	{brick_collide, "int brick_collide(struct disk *ball, struct brick *brick, int count);"},
};
