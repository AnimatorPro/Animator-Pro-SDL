...
Creature_class class_mouser = {sizeof(mouser), NULL, evolve_mouser, see_mouser};
Creature* mouse;
...
Creature_class class_greendot = {sizeof(greendot), init_greendot, evolve_greendot, see_greendot};
...
Creature_class class_fountain = {sizeof(fountain), init_fountain, evolve_fountain, see_fountain};
...
Creature_class class_purpler = {sizeof(purpler), init_purpler, evolve_purpler, see_purpler};
...
Creature_class class_harry = {sizeof(harry), init_harry, evolve_harry, see_harry};


void rad_draw_spoked_wheel(int color, int x, int y, int rad, double theta, int spokes)
...
void split_purpler(int x, int y, int dx, int dy, int afraid, int mycolor, int repro_energy,
				   int pspeed, int child_energy, int spin, int stop, int size, int wait_after_food)
 ...
 * of their state variables to their descendants. */
{
	...
	if (nearc != NULL && (!my->stop || nearc != food || my->fed + my->wait_after_food < my->age)) {
		if (gx > my->x) {
			my->ax = acc;
		} else {
			my->ax = -acc;
	...
	{
		split_purpler(my->x, my->y, my->dx, my->dy, my->afraid, my->mycolor, my->repro_energy,
...
void split_harry(int x, int y, int dx, int dy, int hunt_size, int mycolor, int repro_energy,
				 int pspeed, int child_energy, int spin, int size, int wait_after_food)
...
static void evolve_harry(harry* my)
{
	...
	if (my->energy > my->repro_energy)  // Enough energy to reproduce??
	{
		split_harry(my->x, my->y, my->dx, my->dy, my->hunt_size, my->mycolor, my->repro_energy,
					my->pspeed, my->child_energy, my->spin, my->size, my->wait_after_food);
		my->dx = 0;
		my->dy = 0;
		my->energy = my->energy - my->child_energy;
	}
...
void eco_main()
{
	...
	poeHideCursor();
	for (;;) {
		poePollInput(&mouse_x, &mouse_y, &mouse_left, &mouse_right, &key);
		key &= 0xff;
		...
		poeprintf(emessage, time, gcount, pcount, hcount, rcount, bcount,
				  hcount + pcount + rcount + bcount);
		swap();
	}
OUT:
	kill_all();
