...
Errcode abprocblit(Raster* src, /* doesn't really need to be a ramrast. */
				   Coor src_x, Coor src_y, Raster* dest, Coor dest_x, Coor dest_y, Ucoor width,
				   Ucoor height, Raster* alt, Coor alt_x, Coor alt_y, Procline pline, void* data)
{
	...
	diff = src_x;
	if ((width = pj_lclip2rects(&src_x, &dest_x, width, src->width, dest->width)) <= 0) {
		return (Err_clipped);
	}
	alt_x += src_x - diff;
	diff = src_y;
	if ((height = pj_lclip2rects(&src_y, &dest_y, height, src->height, dest->height)) <= 0) {
		return (Err_clipped);
	}
	alt_y += src_y - diff;
