...
Errcode doval(SHORT xcen, SHORT ycen, SHORT xdiam, SHORT xaspect, SHORT yaspect, dotout_func dotout,
			  void* dotdat, hline_func hlineout, void* hlinedat, bool filled)
 ...
 * twice so that the marqi and gel respectively will work */
{
	...
	for (;;) {
		...
		if (filled) {
			if (lasty != y) {
				if ((ret = (*hlineout)(ycen - y, xcen - x, xcen + x - even1, hlinedat)) != 0 ||
					(ret = (*hlineout)(ycen + y - even1, xcen - x, xcen + x - even1, hlinedat)) !=
						0) {
					return (ret);
				}
				lasty = y;
