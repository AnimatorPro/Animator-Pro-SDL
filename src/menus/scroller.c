...
/* make clipbos to represent scroller list_sel button drawing area */
{
	...
	pj_clipbox_make(cbox, (Raster*)(b->root), b->x + scroll->xoset, b->y + scroll->yoset,
...
void init_scroller(Name_scroller* scr, Wscreen* screen)
{
	...
	scr->cel_height = (scr->list_sel->height - (scr->border + scr->yoset)) / scr->cels_per_col;

	if (!scr->cels_per_row) {
		scr->cels_per_row = 1;
	}
	scr->cel_width = (scr->list_sel->width - (scr->border + scr->xoset)) / scr->cels_per_row;
	...
	if (scr->name_count == 0) {
		...
		scr->knob_height = (scr->scroll_sel->height) * scr->dcount / scr->name_count;
 ...
 * may be null and will be ignored */
{
	...
	ix = ((y / scroll->cel_height) * scroll->cels_per_row) + (x / scroll->cel_width);
	...
	why = (scroll == last_scroll && name == last_entry && (time - last_time) < DHIT_MICROS)
			  ? SCR_MDHIT
			  : SCR_MHIT;
...
static void blit_scroll_cel(Name_scroller* s, void* rast, int src_ix, int dest_ix)

/* based on index in browse sel will blit the contents of the source sel to
 * the dest sel does not clip to size */
{
	pj_blitrect(rast, GET_CEL_X(s, src_ix), GET_CEL_Y(s, src_ix), rast, GET_CEL_X(s, dest_ix),
				GET_CEL_Y(s, dest_ix), s->cel_width, s->cel_height);
}
 ...
 * not redrawn and calling redraw function for ones to be refreshed */
{
	...
	while (--obot >= 0) {
		(*scroll->draw_1_cel)(b, (Raster*)&cbox, GET_CEL_X(scroll, obot), GET_CEL_Y(scroll, obot),
...
static void bscroll_down(Button* b, int count)
{
	...
	while (otop < scroll->dcount) {
		(*scroll->draw_1_cel)(b, (Raster*)&cbox, GET_CEL_X(scroll, otop), GET_CEL_Y(scroll, otop),
							  name);
		++otop;
		if (name != NULL) {
			name = name->next;
...
static void bpagedown(void* name_scroller)
{
	...
	scroll->top_name = clip_scroll(scroll, scroll->top_name + scroll->dcount);
	...
	if (icb.my > b->y + scroller->yoff + scroller->knob_height) {
		...
		for (;;) {
			...
			if (JSTHIT(MMOVE)) {
				new_top =
					first_top +
					((icb.my - firsty) * scroller->name_count + (spare_height >> 1)) / spare_height;
...
static void xor_1_name(Button* b, Raster* rast, int x, int y, Names* name, bool hilite)
{
	Name_scroller* scroll = (Name_scroller*)(b->group);
	(void)name;
	(void)hilite;

	pj_xor_rect(rast, mc_white(b) ^ mc_black(b), x, y, scroll->cel_width, scroll->cel_height);
}
