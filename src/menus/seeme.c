...
static void box_cut_corner(void* rast, SHORT x, SHORT y, USHORT w, USHORT h, Pixel color)
{
...
{
	box_diag_corner(b->root, b->x, b->y, b->width, b->height, color, b->root->w.W_screen->bbevel);
}

void mb_dinside(Button* b, int color)
{
	diag_inside(b->root, b->x, b->y, b->width, b->height, color, b->root->w.W_screen->bbevel);
}
...
void mb_centimage(Button* b, int color, Image* image)
{
...
#ifdef _UNSCALED_
	colors[1] = color;
	tblit_image(image, colors, b->root, b->x + (b->width - image->width) / 2,
	...
	image_scale_tblit(image, colors, b->root, b->x + (b->width - w) / 2, b->y + (b->height - h) / 2,
					  w, h);
}
...
/* centers text in menu button clips top and bottom of text */
{
	...
	gftext(&cb, f, string, b->x + (1 + b->width) / 2 - txtwid / 2, font_ycent_oset(f, b->height),
		   color, TM_MASK1);
	return (txtwid);
}
 ...
 *  Left justified text.
 */
static void mb_leftext(Button* b, Pixel color, char* string)
{
	Vfont* f = b->root->font;
	...
	gftext(b->root, f, string, b->x + (fchar_spacing(f, " ") >> 1) + 1,
		   b->y + font_ycent_oset(f, b->height), color, TM_MASK1);
}
