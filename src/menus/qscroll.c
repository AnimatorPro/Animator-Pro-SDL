...
Errcode build_qscroller(char* result,  /* string we fill in, and start out with */
						...
						SHORT* ipos)   /* Initial scroller position */
{
	...
	qsw->slist.width = qsw->cursel.width = widest_name(font, items) + 2 * scroll_name_xoff(font);
	qsw->slist.height = scroll_names_ysize(font, lines);

	/* calculate arrow dimensions (mostly) */
	qsw->uparr.width = qsw->downarr.width = qsw->sbar.width = rscale_x(&s->menu_scale, LO_ARROW_W);
	...
	okcanmax = intmax((int)fstring_width(font, ok), (int)fstring_width(font, cancel)) + 4;
	...
	qsw->hailb.width = 3 * IBD + IWD + qsw->uparr.width + qsw->slist.width + rightwid + 2;
