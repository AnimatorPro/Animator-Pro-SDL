...
{
	wwtext(b->root, b->root->font, b->datme, b->x, b->y, b->width, b->height, 0, JUST_LEFT,
...
Errcode build_qnumreq(Wscreen* s, Menuhdr** pmh, char* hailing, /* text at top of box */
					  char** ok_cancel,                         /* text for buttons */
					  struct image** arrows,                    /* for slider */
					  SHORT initial, SHORT min, SHORT max,
					  Errcode (*update)(void* uddat, SHORT val), void* uddat) /* num vals */
 ...
 * initialization */
{
	...
	qw->qslb.seeme = see_qslider, qw->qslb.feelme = feel_qslider, qw->qslb.datme = &(qw->qs);
