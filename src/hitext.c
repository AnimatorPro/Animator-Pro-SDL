...
static void ttext_undraw_rect(Raster* r, void* data, int x, int y, int width, int height)
{
	pj_blitrect(data, x, y, r, x, y, width, height);
}

void etext_undraw_rect(Raster* r, void* data, int x, int y, int width, int height)
{
	(void)data;
...
/* Plot text window permanently on rendering screen considering inks and all. */
{
	if (gf->text_buf != NULL) {
		...
		if (make_render_cashes() >= 0) {
			wwtext(vb.pencel, gf->font, gf->text_buf, gf->twin.x, gf->twin.y, gf->twin.width,
				   gf->twin.height, gf->text_yoff, gf->justify_mode, gf->ccolor, TM_RENDER, sblack);
			if (vs.cycle_draw) {
				cycle_redraw_ccolor();
			}
