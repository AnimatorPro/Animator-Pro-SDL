...
test()
{
	...
	if (!have_dpmi) {
		...
		boxf(
			...
			"dpmi_flags                       = %02X\n"
			"dpmi version                     = %d.%02d\n",
			info.largest_available_bytes, info.largest_available_unlocked_pages,
			bpp * info.largest_available_unlocked_pages, info.largest_available_locked_pages,
			bpp * info.largest_available_locked_pages, info.linear_address_space_pages,
			bpp * info.linear_address_space_pages, info.total_unlocked_pages,
			bpp * info.total_unlocked_pages, info.total_free_pages, bpp * info.total_free_pages,
			...
			info.swap_file_size_pages, bpp * info.swap_file_size_pages, info.bytes_per_page,
			info.dpmi_machine, info.dpmi_flags, info.dpmi_major_version, info.dpmi_minor_version);
	}
}
#endif /* TEST_DPMI */
