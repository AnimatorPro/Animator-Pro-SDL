...
static Button cmu_bluelast_sel =
	MB_INIT1(NONEXT, NOCHILD,                                /* children */
			 80, 9, 0, 0,                                    /* w,h,x,y */
			 NODATA,                                         /* "Blue Last", */
			 ccorner_text,                                   /* seeme */
			 toggle_bgroup,                                  /* feelme */
			 NOOPT, &vs.cm_blue_last, 1, NOKEY, MB_B_GHILITE /* flags */
	);
...
static Button cmu_bluelast_hanger = MB_INIT1(NONEXT, &cmu_bluelast_sel,          /* children */
											 80, 9, 90, 3,                       /* w,h,x,y */
											 NODATA, hang_bluelast,              /* feelme */
											 NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0 /* flags */
);
...
static Button cmu_stream_sel =
	MB_INIT1(&cmu_bluelast_hanger, NOCHILD,                                  /* children */
			 80, 9, 3, 3,                                                    /* w,h,x,y */
			 NODATA,                                                         /* "Stream", */
			 ccorner_text,                                                   /* seeme */
			 toggle_stream, NOOPT, &vs.cm_streamdraw, 1, NOKEY, MB_B_GHILITE /* flags */
	);

Button cmu_pa_group_sel = MB_INIT1(NONEXT, &cmu_stream_sel, 80, 9, 0, 0, /* w,h,x,y */
								   NODATA,                               /* datme */
								   NOSEE,                                /* seeme */
								   NOFEEL,                               /* feelme */
								   NOOPT, NOGROUP, 0, NOKEY, 0           /* flags */
);
/****** move group sel ******/

static Button cmu_moveto_sel =
	MB_INIT1(NONEXT, NOCHILD,                                     /* children */
			 80, 9, 3, 3,                                         /* w,h,x,y */
			 NODATA,                                              /* "To cursor", */
			 ccorner_text,                                        /* seeme */
			 toggle_bgroup,                                       /* feelme */
			 NOOPT, &vs.cm_move_to_cursor, 1, NOKEY, MB_B_GHILITE /* flags */
	);
static Button cmu_move_group_sel = MB_INIT1(NONEXT, &cmu_moveto_sel,    /* children */
											80, 9, 0, 0,                /* w,h,x,y */
											NODATA,                     /* datme */
											NOSEE,                      /* seeme */
											NOFEEL,                     /* feelme */
...
static Pentool cel_scale_tool =
	PTOOLINIT1(NONEXT, NOTEXT, /* "Stretch", */
			   PTOOL_OPT, CELPT_SCALE, "", NO_SUBOPTS, NULL, cel_scale_ptfunc,
			   &plain_ptool_cursor.hdr, init_marqi_ctool, /* on install */
			   exit_marqi_ctool                           /* on remove */
	);
static Pentool cel_rotate_tool =
	PTOOLINIT1(&cel_scale_tool, NOTEXT, /* "Turn", */
			   PTOOL_OPT, CELPT_ROTATE, "", NO_SUBOPTS, NULL, cel_rotate_ptfunc,
			   &plain_ptool_cursor.hdr, init_marqi_ctool, /* on install */
			   exit_marqi_ctool                           /* on remove */
	);
static Pentool cel_move_tool =
	PTOOLINIT1(&cel_rotate_tool, NOTEXT, /* "Move", */
			   PTOOL_OPT, CELPT_MOVE, "", &cmu_move_group_sel, NULL, cel_move_ptfunc,
			   &plain_ptool_cursor.hdr, init_marqi_ctool, /* on install */
			   exit_marqi_ctool                           /* on remove */
	);
static Pentool cel_paste_tool =
	PTOOLINIT1(&cel_move_tool, NOTEXT, /* "Paste", */
			   PTOOL_OPT, CELPT_PASTE, "", &cmu_pa_group_sel, NULL, cel_paste_ptfunc,
			   &plain_ptool_cursor.hdr, init_paste_ctool, /* on install */
			   exit_paste_ctool                           /* on remove */
	);
static Pentool cel_paint_tool =
	PTOOLINIT1(&cel_paste_tool, NOTEXT, /* "Sprite", */
			   PTOOL_OPT, CELPT_PAINT, "", &cmu_pa_group_sel, NULL, cel_paint_ptfunc,
			   &plain_ptool_cursor.hdr, init_paint_ctool, /* on install */
			   exit_paint_ctool                           /* on remove */
	);
static Pentool cel_tcolor_tool =
	PTOOLINIT1(&cel_paint_tool, NOTEXT, /* "Set Key", */
			   PTOOL_OPT, CELPT_TCOLOR, "", NO_SUBOPTS, NULL, cel_tcolor_ptfunc,
			   &plain_ptool_cursor.hdr, init_tcolor_ptool, /* on install */
...
static Optgroup_data opg = {&vs.cur_cel_tool, (Option_tool*)&FIRST_CEL_TOOL, NULL, 0};

static Button cmu_toolopts_sel = MB_INIT1(NONEXT,                          /* next */
										  NOCHILD,                         /* children */
										  197, 29, 119, 26,                /* w,h,x,y */
										  NOTEXT, hang_toolopts,           /* seeme */
										  NOFEEL, NOOPT, &opg, 0, NOKEY, 0 /* flags */
);
...
static Button cmu_tco_sel = MB_INIT1(&cmu_toolopts_sel, /* next */
									 NOCHILD,           /* children */
									 53, 9, 6, 46,      /* w,h,x,y */
									 &cel_tcolor_tool,  /* datme */
									 see_celtool_sel, mb_set_celtool, NOOPT, &vs.cur_cel_tool,
									 CELPT_TCOLOR, NOKEY, MB_GHILITE /* flags */
);

static void see_draw_frames_option(Button* b)
{
	set_button_disable(b, thecel == NULL || cmcb->no_draw_tools || (flix.hdr.frame_count <= 1));
	see_option_name(b);
}

static Button cmu_pai_sel = MB_INIT1(&cmu_tco_sel,    /* next */
									 NOCHILD,         /* children */
									 53, 9, 6, 26,    /* w,h,x,y */
									 &cel_paint_tool, /* datme */
									 see_draw_frames_option, mb_set_celtool, NOOPT,
									 &vs.cur_cel_tool, CELPT_PAINT, NOKEY, MB_GHILITE /* flags */
);
...
static Button cmu_pas_sel = MB_INIT1(&cmu_pai_sel,    /* next */
									 NOCHILD,         /* children */
									 53, 9, 6, 36,    /* w,h,x,y */
									 &cel_paste_tool, /* datme */
									 see_draw_option, mb_set_celtool, NOOPT, &vs.cur_cel_tool,
									 CELPT_PASTE, NOKEY, MB_GHILITE /* flags */
);
static Button cmu_sca_sel = MB_INIT1(&cmu_pas_sel,    /* next */
									 NOCHILD,         /* children */
									 53, 9, 61, 46,   /* w,h,x,y */
									 &cel_scale_tool, /* datme */
									 see_option_name, mb_set_celtool, NOOPT, &vs.cur_cel_tool,
									 CELPT_SCALE, NOKEY, MB_GHILITE /* flags */
);
static Button cmu_rot_sel = MB_INIT1(&cmu_sca_sel,     /* next */
									 NOCHILD,          /* children */
									 53, 9, 61, 36,    /* w,h,x,y */
									 &cel_rotate_tool, /* datme */
									 see_option_name, mb_set_celtool, NOOPT, &vs.cur_cel_tool,
									 CELPT_ROTATE, NOKEY, MB_GHILITE /* flags */
);
static Button cmu_mov_sel = MB_INIT1(&cmu_rot_sel,   /* next */
									 NOCHILD,        /* children */
									 53, 9, 61, 26,  /* w,h,x,y */
									 &cel_move_tool, /* datme */
									 see_option_name, mb_set_celtool, NOOPT, &vs.cur_cel_tool,
									 CELPT_MOVE, NOKEY, MB_GHILITE /* flags */
);
...
static Button cmu_grid_sel = MB_INIT1(NONEXT, NOCHILD, /* children */
									  44, 9, 267, 14,  /* w,h,x,y */
									  NODATA,          /* "Grid", */
									  ncorner_text, toggle_bgroup, qgrid_keep_undo, &vs.use_grid, 1,
									  NOKEY, MB_B_GHILITE /* flags */
);
static Button cmu_mask_sel = MB_INIT1(&cmu_grid_sel,           /* next */
									  NOCHILD, 44, 9, 219, 14, /* w,h,x,y */
									  NODATA,                  /* "Mask", */
									  see_mask_button, mb_toggle_mask, qmask_keep_undo,
									  &vs.use_mask, 1, NOKEY, MB_B_GHILITE);
static Button cmu_zpan_sel = MB_INIT1(&cmu_mask_sel, &zpan_cycle_group, 39, 9, 13, 14, /* w,h,x,y */
...
static Button cmu_frames_sel =
	MB_INIT1(&cmu_zpan_sel, NOCHILD,                                        /* children */
			 34, 11, 277, 57,                                               /* w,h,x,y */
			 NODATA,                                                        /* datme */
			 see_cel_frames, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, MB_B_GHILITE /* flags */
	);

static Button cmu_minitime_sel =
	MB_INIT1(&cmu_frames_sel, NULL, /* children allocated in go_cel_menu() */
			 76, 11, 1, 57, NODATA, /* "Cel Frames:", */
			 see_cel_minitime, NOFEEL, NOOPT, &celtime_data, 0, NOKEY, 0);
static Button cmu_minipal_sel = MB_INIT1(&cmu_minitime_sel,            /* next */
										 &minipal_sel, 93, 9, 119, 14, /* w,h,x,y */
...
static Button cmu_goinks_sel =
	MB_INIT1(&cmu_minipal_sel, NOCHILD, 53, 9, 238, 3, NOTEXT, see_cur_ink, qcel_inks, qcel_inks,
			 &vs.ink_id, opq_INKID, NOKEY, MB_GHILITE);
static Button cmu_std1_sel =
	MB_INIT1(&cmu_goinks_sel, &std_head1_sel, 0, 0, 129, 3,                 /* w,h,x,y */
			 NOTEXT, hang_children, NOFEEL, NOOPT, &cel_sh1dat, 0, NOKEY, 0 /* flags */
	);
Button cmu_common_sels = MB_INIT1(&cmu_std1_sel, NOCHILD, 0, 0, 5, 3,                /* w,h,x,y */
								  NOTEXT, NOSEE, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0 /* flags */
);

static Button cmu_undo_sel =
	MB_INIT1(&cmu_mov_sel, NOCHILD,                                /* children */
			 33, 9, 93, 3,                                         /* w,h,x,y */
			 NODATA,                                               /* "Undo", */
			 dcorner_text, menu_doundo, NOOPT, NOGROUP, 0, '\b', 0 /* flags */
	);
static Button cmu_title_sel =
	MB_INIT1(&cmu_undo_sel, &cmu_common_sels, 84, 9, 5, 3, /* w,h,x,y */
			 NODATA,                                       /* "Anim Cel", */
			 hang_see_title, feel_titlebar, mb_menu_to_bottom, &tbg_moveclose, 0, 'q', 0 /* flags */
	);
...
static Redraw_node celmenu_rn = {{NULL, NULL}, /* node */
								 celmenu_credraw,
								 NULL,
								 NEW_CCOLOR};


Menuhdr cel_menu = MENU_INIT0(320, 70, 0, 0,              /* width, height, x, y */
							  CEL_MUID,                   /* id */
							  PANELMENU,                  /* type */
...
{
	return (set_curptool((Pentool*)id_find_option((Option_tool*)&FIRST_CEL_TOOL, id)));
}
...
static void mtcel_playit(void* pcel)
{
	...
	for (;;) {
		...
		if (cel->flif.hdr.frame_count > 1) {
			if ((err = gb_seek_fcel_frame(cel, cel->cd.cur_frame + 1, cbuf, false)) < Success) {
				goto error;
			}
 ...
 * from screen and current pen tool is not a cel tool */
{
	...
	if (cmcb->no_draw_tools || !thecel) {
		if (toolid == CELPT_PASTE || toolid == CELPT_PAINT || toolid == NULL_PTOOL) {
			toolid = CELPT_MOVE;
		}
	}
...
static void toolcel_color_redraw(void* dat, USHORT why)
{
	...
	if (flx_olays_hidden() || flxtime_data.draw_overlays != cm_draw_olays) {
		return;
	}

	if (((why & NEW_CCOLOR) && vs.render_one_color) || ((why & NEW_INK0) && vs.render_under) ||
		((why & NEW_INK1) && vs.cur_cel_tool == CELPT_TCOLOR) ||
		((why & NEW_CMAP) && vs.fit_colors) || ((why & NEW_CEL_TCOLOR) && vs.zero_clear)) {
...
static Redraw_node toolcel_rn = {{NULL, NULL}, /* node */
								 toolcel_color_redraw,
								 NULL,
								 NEW_CCOLOR | NEW_INK0 | NEW_INK1 | NEW_CMAP | NEW_CEL_TCOLOR};

static Redraw_node tcel_rmode_rn = {{NULL, NULL}, /* node */
...
{
	if (check_toggle_abort() || common_header_keys() || check_undo_key()) {
		return (true);
	}
	return false;
}
...
static void do_cel_menu(bool no_draw_tools)
{
	...
	if ((err = soft_buttons(panel_key, cel_smblist, Array_els(cel_smblist), &softbs)) < Success) {
		goto error;
	}

	if ((err = load_soft_pull(&cmcb->tpull, 12, "cel", CELPULL_MUID, cm_selit, do_celpull)) <
		Success) {
		goto error;
	}
	...
	if ((err = clone_buttonlist(&timeslider_sel, &cmu_minitime_sel.children)) < Success) {
		goto error;
	}
	...
	err = do_menuloop(vb.screen, &cel_menu, NULL, &cmcb->tpull, do_celmenu_keys);
