 ...
 */
static FilePath* filepath_create_header(const char* str)
{
	FilePath* filepath;
	enum FilePathType type;
	size_t len;
	...
	if (str[0] == DIR_DELIM || str[0] == DIR_DELIM2) {
		type = FILEPATH_ROOT;
		len = 1;
	} else if (('A' <= drive && drive <= 'Z') && (str[1] == DEV_DELIM) &&
 ...
 */
static Errcode filepath_append_node(FilePath* filepath, const char* str)
{
	FilePathNode* node;
	Errcode err;
	size_t len;

	if (!pj_assert(filepath != NULL)) {
 ...
 *  manipulation.
 */
FilePath* filepath_create_from_string(const char* str)
{
	FilePath* filepath;
	Errcode err;
 ...
 */
Errcode filepath_destroy(FilePath* filepath)
{
	Errcode err;
 ...
 */
Errcode filepath_append(FilePath* filepath, const char* str)
{
 ...
 *  device.
 */
Errcode filepath_drop_tail(FilePath* filepath)
{
	Dlnode* node;
 ...
 */
Errcode filepath_to_cstr(const FilePath* filepath, char delim, char* str, size_t n)
{
	Errcode err;
	const Dlnode* node;
	char* start = str;
	...
	if (!pj_assert(filepath->dl.head != NULL)) {
