   ...
   (gasp) to render it. */
{
	...
	do {
		for (i = 0; i < frames; i++) {
			...
			msome_vector((void*)points, point_count, mh.pdot, &mh, !closed, sizeof(Short_xyz));
			err = poll_abort();
			clock += speed;
			do {
				wait_sync();
			...
			msome_vector((void*)points, point_count, undo_marqidot, &mh, !closed,
						 sizeof(Short_xyz));
...
Errcode save_tween(char* name, Tween_state* ts)
{
	...
	for (link = (Tween_link*)(ts->links.head); NULL != (next = (Tween_link*)(link->node.next));
		 link = next) {
...
Errcode tween1(void* tween1_data, int ix, int intween, int scale, Autoarg* aa)
{
	...
	if ((dpoly.clipped_list = d = pj_malloc(dpoly.pt_count * sizeof(LLpoint))) == NULL) {
		err = Err_no_memory;
		goto OUT;
	}
	linkup_poly(&dpoly);
