...
static Button fcancel_sel = MB_INIT1(NONEXT, NOCHILD, 49, 15, 265, 78, NODATA, /* "Cancel", */
									 dcorner_text, mb_gclose_cancel, NOOPT, NOGROUP, 0, ' ', 0);

static Button fok_sel = MB_INIT1(&fcancel_sel, NOCHILD, 49, 15, 214, 78, NULL, dcorner_text,
								 mb_gclose_ok, NOOPT, NOGROUP, 0, '\r', 0);

static Button fplus_sel = MB_INIT1(&fok_sel, NOCHILD, 18 + 1, 15, 188, 78, NODATA, /* "+", */
								   ccorner_text, fq_ok_plus, NOOPT, NOGROUP, 0, '+', 0);

/************** device buttons hanger ***********/
static Button fmu_dev_hanger = MB_INIT1(&fplus_sel, NOCHILD, 17, 13, 106, 28, NODATA, NOSEE, NOFEEL,
										NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_topdev_hanger = MB_INIT1(&fplus_sel, NOCHILD, 17, 13, 106, 12, NODATA, NOSEE,
...
static Button fmu_wild3_sel = MB_INIT1(NONEXT, NOCHILD, 31, 13, 283, 60, NODATA, see_opt_wild,
									   fq_set_wild, NOOPT, &wild_stringq.string, 0, NOKEY, 0);
static Button fmu_wild2_sel =
	MB_INIT1(&fmu_wild3_sel, NOCHILD, 31, 13, 249, 60, NODATA, see_opt_wild, fq_set_wild, NOOPT,
			 &wild_stringq.string, 0, NOKEY, 0);
static Button fmu_wild1_sel =
	MB_INIT1(&fmu_wild2_sel, NOCHILD, 31, 13, 215, 60, NODATA, see_opt_wild, fq_set_wild, NOOPT,
			 &wild_stringq.string, 0, NOKEY, 0);
static Button fmu_sdots_sel = MB_INIT1(NULL, /* Filled in at run time. */
									   &fmu_wild1_sel, 24, 13, 188, 60, "*.*", see_opt_wild,
									   fq_set_wild, NOOPT, &wild_stringq.string, 0, NOKEY, 0);

static Button fwild_sel = MB_INIT1(&fmu_sdots_sel, NOCHILD, 71, 13, 243, 44, &wild_stringq,
								   see_string_req, fq_wild_stringq, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fwildp_sel =
	MB_INIT1(&fwild_sel, NOCHILD, 9 * 6 + 1, 13, 184, 44, NODATA, /* "Wildcard:", */
			 black_leftlabel, NOFEEL, NOOPT, NOGROUP, 0, '*', 0);
static char und_drawer[PATH_SIZE] = "c:";
static Stringq drawer_stringq = {
	2, 2, 16, PATH_SIZE - 1, 0, NULL, und_drawer, 0, 0,
};

static Button fdrawer_sel =
	MB_INIT1(&fwildp_sel, NOCHILD, 101, 13, 213, 28, &drawer_stringq, see_string_req,
			 fq_drawer_stringq, NOOPT, &fwild_sel, 0, NOKEY, 0);

static Button fdrawerp_sel =
	MB_INIT1(&fdrawer_sel, NOCHILD, 4 * 6 + 1, 13, 184, 28, NODATA, /* "Dir:", */
			 black_leftlabel, undo_drawer, NOOPT, NOGROUP, 0, NOKEY, 0);
static char und_file[PATH_SIZE] = "zippy";
static Stringq file_stringq = {
	STQ_LJUST, STQ_CENTER, 15, PATH_SIZE - 1, 0, NULL, und_file,
};
static Button ffile_sel =
	MB_INIT1(&fdrawerp_sel, NOCHILD, 95, 13, 219, 12, &file_stringq, see_string_req,
			 accept_file_name, NOOPT, &fdrawer_sel, 0, '\t', 0);
static Button ffilep_sel =
	MB_INIT1(&ffile_sel, NOCHILD, 6 * 5 + 1, 13, 184, 12, NODATA, /* "File:", */
			 black_leftlabel, undo_file, NOOPT, NOGROUP, 0, NOKEY, 0);
static Name_scroller fscroller;
static Button flist_sel = MB_INIT1(&ffilep_sel, NOCHILD, 81, 83, 20, 12, NODATA, see_scroll_names,
								   feel_scroll_cels, NOOPT, &fscroller, 0, NOKEY, 0);

static Button fdown_sel = MB_INIT1(&flist_sel, NOCHILD, 15, 13, 3, 82, &ctridown, ccorner_image,
								   scroll_incdown, NOOPT, &fscroller, 0, DARROW, 0);

static Button fscroll_sel = MB_INIT1(&fdown_sel, NOCHILD, 15, 55, 3, 26, &fscroller, see_scrollbar,
									 feel_scrollbar, NOOPT, &fscroller, 0, NOKEY, 0);

static Button fup_sel = MB_INIT1(&fscroll_sel, NOCHILD, 15, 13, 3, 12, &ctriup, ccorner_image,
								 scroll_incup, NOOPT, &fscroller, 0, UARROW, 0);

static Button ftitle_sel = MB_INIT1(
	&fup_sel, NOCHILD, 320, 10, 0, 0, NULL, /* gets set to prompt by pj_get_filename(prompt) */
...
static Smu_button_list fmu_smblist[] = {
	{"ok", {&fok_sel}},      {"cancel", {&fcancel_sel}}, {"plus", {&fplus_sel}},
	{"wild", {&fwildp_sel}}, {"dir", {&fdrawerp_sel}},   {"file", {&ffilep_sel}},
};
...
static void feel_1_fname(Button* m, Raster* rast, int x, int y, Names* entry, int why)
{
	char* name;
	(void)rast;
	(void)x;
...
static Errcode qfile(char* path_out,   /* Initial contents ignored.  Returns full path. */
					 ...
					 char* wildbuf)    /* buffer for wildcard must be at least "WILD_SIZE" long */
{
	...
	if ((err = alloc_dev_sels(dhanger, &fmu_devsel_320size, 4, 5, dir_in, new_drawer_with_data,
							  NULL)) < Success) {
		goto error;
	}
	if ((err = soft_buttons("fscroll_panel", fmu_smblist, Array_els(fmu_smblist), &ss)) < Success) {
		goto error;
	}
	...
	err = do_reqloop(icb.input_screen, &fileq_menu, &ffile_sel, NULL, NULL);
