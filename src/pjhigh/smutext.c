...
#ifdef SLUFFED
Errcode soft_scatters(char* symname, Smu_strings* s, char** scatters[], int count)
...
Errcode soft_name_scatters(char* symname, Smu_name_scats* nscats, unsigned int num_scats,
						   void** allocd, USHORT flags)
/* If fill all is true it will abort error if all the symbols are not found */
{
	return (smu_name_scatters(&smu_sm, symname, nscats, num_scats, allocd, flags));
}
