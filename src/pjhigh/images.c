...
static UBYTE ikill[] = {
	0x07, 0xC0, 0x0C, 0x60, 0x1E, 0x30, 0x13, 0x10, 0x11, 0x90, 0x18, 0xF0, 0x0C, 0x60, 0x07, 0xC0,
};
...
static UBYTE ileft[] = {
	BITS8(_, _, _, X, _, _, _, _), BITS8(_, _, X, _, _, _, _, _), BITS8(_, X, X, _, _, _, _, _),
	BITS8(X, X, X, X, X, X, X, X), BITS8(_, X, X, _, _, _, _, _), BITS8(_, _, X, _, _, _, _, _),
	BITS8(_, _, _, X, _, _, _, _),
};
...
static UBYTE iup[] = {
	BITS8(_, _, _, X, _, _, _, _), BITS8(_, _, X, X, X, _, _, _), BITS8(_, X, X, X, X, X, _, _),
	BITS8(X, _, _, X, _, _, X, _), BITS8(_, _, _, X, _, _, _, _), BITS8(_, _, _, X, _, _, _, _),
	BITS8(_, _, _, X, _, _, _, _),
};
...
static UBYTE ictridown[] = {
	BITS8(_, X, X, X, X, X, X, _), BITS8(X, X, X, X, X, X, X, X), BITS8(_, X, X, X, X, X, X, _),
	BITS8(_, _, X, X, X, X, _, _), BITS8(_, _, _, X, X, _, _, _),
};
Image ctridown = IMAGE_INIT1(ITYPE_BITPLANES, 1, ictridown, 8, 5);

static UBYTE ictriup[] = {
	BITS8(_, _, _, X, X, _, _, _), BITS8(_, _, X, X, X, X, _, _), BITS8(_, X, X, X, X, X, X, _),
	BITS8(X, X, X, X, X, X, X, X), BITS8(_, X, X, X, X, X, X, _),
};
