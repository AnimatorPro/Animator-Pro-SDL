...
static Errcode read_pic_pixels(XFILE* xf, Pic_header* pic, Raster* cel, SHORT chunk_type,
							   LONG offset, LONG data_size)
{
	Errcode err;
	LONG bufsize;
	Bytemap rr;
	...
	switch (chunk_type) {
		...
		case PIC_BYTEPIXELS:
			...
			break;
check_readplane:
			if (pic->depth != cel->pdepth || (ULONG)data_size != cel->hw.bm.psize ||
			...
			return xffreadoset(xf, ((Bytemap*)cel)->bm.bp[0], offset, ((Bytemap*)cel)->bm.psize);
 ...
 *  center it otherwise.
 */
Errcode pj_read_picbody(XFILE* xf, Pic_header* pic, Raster* cel, Cmap* cmap)
{
	...
	if (pic->id.type == OPIC_MAGIC) {
		...
		return read_pic_pixels(xf, pic, cel, PIC_BYTEPIXELS, sizeof(Opic_header) + (COLORS * 3),
							   pic->width * pic->height);
	}

	init_chunkparse(&pd, xf, DONT_READ_ROOT, 0, sizeof(Pic_header), pic->id.size);
	...
	while (get_next_chunk(&pd)) {
		switch (pd.type) {
			...
			case PIC_BITPIXELS:
			case PIC_BYTEPIXELS:
				err = read_pic_pixels(xf, pic, cel, pd.type, pd.chunk_offset + sizeof(Chunk_id),
...
/* if do_colors is FALSE it may load a raster without a cmap */
{
	...
	err = pj_read_picbody(xf, &pic, (Raster*)rcel, do_colors ? rcel->cmap : NULL);
	if (err < Success) {
		goto error;
	}
