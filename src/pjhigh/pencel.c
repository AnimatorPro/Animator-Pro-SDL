 ...
 * This should be called after a hide_mp(). */
{
	...
	if (height == vb.screen->wndo.height && width == vb.screen->wndo.width) {
		...
		if (height <= vb.screen->wndo.height && width <= vb.screen->wndo.width) {
			...
			bclip0xy_rect((Rectangle*)&(wi.RECTSTART), (Rectangle*)&(vb.screen->wndo.RECTSTART));
		} else {
 ...
 */
{
	...
	draw_quad((Raster*)&vb.screen->wndo, c1, w->x - 1, w->y - 1, w->width + 2, w->height + 2);
}
 ...
 */
{
	if (PENWNDO != &(vb.screen->wndo) &&
		(CODEOBSCURES != clipcode_crects((Cliprect*)&(PENWNDO->CRECTSTART),
