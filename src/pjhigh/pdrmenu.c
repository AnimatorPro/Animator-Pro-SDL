...
static Errcode build_pdr_list(Names* wildlist, Pdr_entry** pdrlist, Pdr_entry** current,
							  char* curname, int rwmode, /* 0 = any,
														  * 1 = must read,
														  * 2 = must write
														  * 3 = must both */
	...
	for (;;) {
		for (;;) {
			...
			if ((err = load_pdr(pdr_name, &pd)) >= Success) {
				if (multiframe_only && pd->max_read_frames <= 1 && pd->max_write_frames <= 1) {
					continue;
				}
...
#define PDE_TITLE_OSET 11
		...
		tit_start = entry->name_buf + sprintf(entry->name_buf, "%c %-9.*s", rw_char,
...
#define box_text scts[4].toload.s
	...
	if ((err = soft_name_scatters("pdr_info", scts, Array_els(scts), &sctbuf, SCT_DIRECT)) <
		Success) {
		goto error;
	}
	...
	snftextf(read_text, sizeof(read_text), "!%d", read_frames ? read_yes : read_no, read_frames);

	snftextf(write_text, sizeof(write_text), "!%d", write_frames ? write_yes : write_no,
			 write_frames);

	/* note this will handle a NULL pointer for info_text */

	if (((soft_text = rex_key_or_text(info_text, &info_text)) != NULL) &&
		(smu_load_name_text(&smu_sm, "picdrive_texts", soft_text, &soft_text) >= Success)) {
		info_text = soft_text;
	}

	continu_box("!%s%s%s%s%s", box_text, pj_get_path_name(pdr_path), title_text, read_text,
				write_text, info_text);
											  ...
											  * frame */ )
{
	...
	if ((err = build_pdr_list(namelist, &pdrlist, &current, name_buf, rwmode, multiframe_only)) <
		Success) {
		goto error;
	}


	err = go_driver_scroller(header, (Names*)pdrlist, (Names*)current, pick_pdr, pdr_info_box,
							 &pdat, NULL);
