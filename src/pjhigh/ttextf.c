...
static Errcode open_topwin(Vfont* f)
{
	...
	sclip_rect((Rectangle*)&(wi.RECTSTART), (Rectangle*)&(vb.screen->wndo.RECTSTART));
 ...
 * sizeof buff[] or else !! */
{
	...
	return (gftext(topwin, f, buff, spwid, font_ycent_oset(f, topwin->height), vb.screen->SBLACK,
				   TM_MASK2, vb.screen->SWHITE));
}
