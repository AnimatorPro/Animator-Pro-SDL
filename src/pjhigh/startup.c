...
static Errcode set_vdriver(Argparse_list* ap, int argc, char** argv, int position)
{
	char* cpt;
...
static Errcode set_config_name(Argparse_list* ap, int argc, char** argv, int position)
{
...
static Errcode rethelp(Argparse_list* ap, int argc, char** argv, int position)
{
...
static Errcode set_max_memory(Argparse_list* ap, int argc, char** argv, int position)
{
	char* smem = argv[1];
...
Errcode open_pj_startup_screen(Errcode (*init_with_screen)(void* iwdat), void* iwdat)
{
	Screen_mode smode;
	...
	if (vb.vdriver_name != NULL) {
		clear_struct(&smode);
		copy_insure_suffix(vb.vdriver_name, ".drv", smode.drv_name, sizeof(smode.drv_name));
