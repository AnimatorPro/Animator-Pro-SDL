...
AA_config vconfg = {
	/* Ram image of v.cfg containing default values
...
static Errcode open_config(XFILE** pxf, bool create)
{
	const char* config_name = vb.config_name;

	if (!pj_assert(vb.config_name != NULL)) {
		return Err_abort;
	}

	return xffopen(config_name, pxf, create ? XREADWRITE_CLOBBER : XREADWRITE_OPEN);
}
...
char* get_default_config_name()
{
	...
	if (!initialized) {
		...
		sprintf(config_name, "%s%s%s", config_name, SEP, default_config_name);
		initialized = 1;
	}
