...
Errcode open_screen(Screen_mode* sm, char* path)
{
	...
	if (!sm->width || !sm->height) {
		if ((err = get_drv_default_size(sm->mode, &sm->width, &sm->height)) < Success) {
			goto error;
		}
	}
	/* open the screen cel */

	if ((err = alloc_display_rcel(vb.vd, &vb.cel_a, sm->width, sm->height, sm->mode)) < Success) {
		goto error;
	}
	return (Success);
error:
	...
	restore_ivmode();
	return (softerr(err, "!%s%d%d%d", "driver_open", pathbuf, sm->width, sm->height, sm->mode));
}
 ...
 * cleaned up and the error returned */
{
	...
	if (init_with_screen != NULL && (err = init_with_screen(iwdat)) < Success) {
		goto error;
	}
