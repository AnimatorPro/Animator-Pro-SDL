...
static Errcode build_mode_list(Mode_entry** pmlist, Names** pwildlist, Names** pcurrent)
{
	...
	while (drv_entry != NULL) {
		...
		for (mode = 0; mode < mode_count; ++mode) {
			...
			if (mode_info.width.min == mode_info.width.max) {
				...
				sprintf(wstr, "%d:%d", mode_info.width.min, mode_info.width.max);
			}

			if (mode_info.height.min == mode_info.height.max) {
				sprintf(hstr, "%d", mode_info.height.max);
			} else {
				sprintf(hstr, "%d:%d", mode_info.height.min, mode_info.height.max);
			}
			sprintf(mentry->name_buf, "%-12.12s %-2d %-18.18s %s X %s", mentry->drv_file, mode,
					mode_info.mode_name, wstr, hstr);
...
static bool show_driver_info(Names* entry, void* dat)
{
	...
	if (((soft_info = rex_key_or_text(more_info, &more_info)) != NULL) &&
		(smu_load_name_text(&smu_sm, "vdriver_texts", soft_info, &soft_info) >= Success)) {
		more_info = soft_info;
	}

	soft_continu_box("!%s%s%d%d%d%d%d%d%d", "vdriver_info", more_info, mentry->drv_file,
					 mentry->mode, mi.bits, mi.width.min, mi.width.max, mi.height.min,
					 mi.height.max, mi.store_bytes);
...
error:
	...
	softerr(err, "!%s%d", "driver_info", mentry->drv_file, mentry->mode);
	show_mp();
...
static Errcode go_screen_menu(Screen_mode* sm)
{
	...
	err = go_driver_scroller(stack_string("screen_menu", sbuf), (Names*)mlist, current,
