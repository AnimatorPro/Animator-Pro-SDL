...
/*** Button Data ***/
static Button fmu_can_sel =
	MB_INIT1(NONEXT, NOCHILD, 49, 15, 268, 79, NODATA, /* Read in from menu file */
			 dcorner_text, mb_close_cancel, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_ok_sel =
	MB_INIT1(&fmu_can_sel, NOCHILD, 49, 15, 216, 79, NODATA, /* Read in from menu file */
			 dcorner_text, mb_close_ok, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_sam_sel = MB_INIT1(&fmu_ok_sel, NOCHILD, 129, 74, 99, 3, NOTEXT, font_sample_text,
...
static Button fmu_nam_sel = MB_INIT1(&fmu_sam_sel, NOCHILD, 79, 15, 235, 7 - 2, NULL, see_font_name,
									 NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_tdi_sel =
	MB_INIT1(&fmu_nam_sel, NOCHILD, 114, 15, 99, 79, NODATA, /* Read in from menu file */
			 dcorner_text, get_any_font, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_unz_sel =
	MB_INIT1(&fmu_tdi_sel, NOCHILD, 50, 11, 250, 61, NODATA, /* Read in from menu file */
			 see_font_unzag, set_font_unzag, set_font_unzag, NOGROUP, 1, NOKEY, MB_GHILITE);
static Button fmu_she_sel =
	MB_INIT1(&fmu_unz_sel, NOCHILD, 23, 11, 288, 54 - 6 - 2, NOTEXT, see_font_height,
			 set_font_height, set_font_height, NOGROUP, 0, NOKEY, 0);
static Button fmu_hei_sel =
	MB_INIT1(&fmu_she_sel, NOCHILD, 47, 11, 234, 54 - 6 - 2, NODATA, /* Read in from menu file */
			 grey_ctext, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_ssp_sel =
	MB_INIT1(&fmu_hei_sel, NOCHILD, 23, 11, 288, 42 - 6 - 2, NOTEXT, see_font_spacing,
			 set_font_spacing, set_font_spacing, NOGROUP, 0, NOKEY, 0);
static Button fmu_spa_sel =
	MB_INIT1(&fmu_ssp_sel, NOCHILD, 47, 11, 234, 42 - 6 - 2, NODATA, /* Read in from menu file */
			 grey_ctext, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_sle_sel =
	MB_INIT1(&fmu_spa_sel, NOCHILD, 23, 11, 288, 30 - 6 - 2, NOTEXT, see_font_leading,
			 set_font_leading, set_font_leading, NOGROUP, 0, NOKEY, 0);
static Button fmu_lea_sel =
	MB_INIT1(&fmu_sle_sel, NOCHILD, 47, 11, 234, 30 - 6 - 2, NODATA, /* Read in from menu file */
			 grey_ctext, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0);
static Button fmu_lis_sel =
	MB_INIT1(&fmu_lea_sel, NOCHILD, 78, 78, 17, 17, &font_scroller, see_scroll_names,
			 feel_scroll_cels, NOOPT, &font_scroller, 0, NOKEY, 0);
static Button fmu_dow_sel = MB_INIT1(&fmu_lis_sel, NOCHILD, 13, 11, 3, 84, &ctridown, ccorner_image,
									 scroll_incdown, NOOPT, &font_scroller, -1, DARROW, 0);
static Button fmu_scr_sel =
	MB_INIT1(&fmu_dow_sel, NOCHILD, 11, 58, 4, 27, &font_scroller, see_scrollbar, rt_feel_scrollbar,
			 NOOPT, &font_scroller, 0, NOKEY, 0);
static Button fmu_ups_sel = MB_INIT1(&fmu_scr_sel, NOCHILD, 13, 11, 3, 17, &ctriup, ccorner_image,
									 scroll_incup, NOOPT, &font_scroller, -1, UARROW, 0);
static Button fmu_tit_sel =
	MB_INIT1(&fmu_ups_sel, NOCHILD, 92, 12, 3, 3, NODATA, /* Read in from menu file */
			 see_titlebar, feel_titlebar, mb_menu_to_bottom, &tbg_moveclose, 0, NOKEY, 0);
Menuhdr fmu_menu = {
	{
		320,
		97,
...
static Smu_button_list font_blist[] = {
	{"cancel", {&fmu_can_sel}}, {"ok", {&fmu_ok_sel}},     {"dir", {&fmu_tdi_sel}},
	{"space", {&fmu_spa_sel}},  {"lead", {&fmu_lea_sel}},  {"height", {&fmu_hei_sel}},
	{"title", {&fmu_tit_sel}},  {"unzag", {&fmu_unz_sel}},
};
   ...
   (space permitting).  Always print lower case even if it won't fit. */
{
	...
	if (qfcb->font_err == Success) {
		pj_clipbox_make(&cb, (Raster*)m->root, m->x + 2, m->y + 2, m->width - 4, m->height - 4);
...
static void set_font_spacing(Button* m)
{
	...
	if (!clip_soft_qreq_number(&spacing, 0, f->widest_image * 5, update_font_spacing, f,
							   "font_spacing")) {
		spacing = ospacing;
	}
...
static void set_font_leading(Button* m)
{
	...
	if (!clip_soft_qreq_number(&leading, leadmin, leadmax, update_font_leading, f,
							   "font_leading")) {
		leading = oleading;
	}
...
{
	if ((qfcb->font_err = load_font(fpath, &qfcb->vfont, qfcb->point_size, qfcb->unzag_flag)) >=
		Success) {
...
static void maybe_load_qcb_font(char* fpath)
{
	...
	if (load_qcb_font(fpath) < Success) {
		...
		qfcb->font_err = load_qcb_font(qfcb->fpath);
	} else {
   ...
   for that type of font */
{
	...
	if ((fpath = pj_get_filename(stack_string("sel_any_font", sbuf), ".FNT;.PFB",
								 stack_string("sel_str", selb), qfcb->fpath, lpath, false,
								 qfcb->ptop_name, qfcb->wildcard)) != NULL) {
...
static void feel_1_font(Button* b, Raster* rast, int x, int y, Names* entry, int why)
{
	char fpath[PATH_SIZE];
	(void)rast;
	(void)x;
...
Errcode font_req(char* font_path,                 /* On Success filled in with new path. Not
				 ...
				 SHORT* ptop_name,                /* may be NULL top name for directory scroller
												   * Updated if changed even if canceled out */
				 SHORT* point_size, Vfont* pfont, /* On Success this font is replaced... (Make
				 ...
				 SHORT* punzag_flag)              /* Do we unzag this font? */
{
	...
	clear_struct(&cb);
	if ((mu_err = soft_buttons("font_panel", font_blist, Array_els(font_blist), &ss)) < Success) {
		goto OUT;
	}
