...
static Button brw_cpi_sel = MB_INIT1(NONEXT, NOCHILD, 66, 42, 102, 154, NOTEXT, draw_cpi, NOFEEL,
									 mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button brw_list_sel =
	MB_INIT1(&brw_cpi_sel, NOCHILD, 299, 150, 20, 0, NODATA, draw_scroll_cels, feel_scroll_cels,
			 mb_close_cancel, &bscroller, 0, NOKEY, MB_NORESCALE);
static Button brw_downarr =
	MB_INIT1(&brw_list_sel, NOCHILD, 13, 10 + 1, 0, 136, &ctridown, ccorner_image, scroll_incdown,
			 mb_close_cancel, &bscroller, -1, DARROW, 0);
static Button brw_scroller_sel =
	MB_INIT1(&brw_downarr, NOCHILD, 11, 123, 1, 12, &bscroller, see_scrollbar, slow_feel_scrollbar,
			 mb_close_cancel, &bscroller, 0, NOKEY, MB_NORESCALE);
static Button brw_uparr = MB_INIT1(&brw_scroller_sel, NOCHILD, 13, 11, 0, 0, &ctriup, ccorner_image,
								   scroll_incup, mb_close_cancel, &bscroller, -1, UARROW, 0);

/*** Button Data ***/

#define BRO_DRAWER (*((char**)&(bro_pat_sel.datme)))

static Button bro_pat_sel =
	MB_INIT1(&brw_uparr, NOCHILD, 124, 11, 195, 186, NULL, /* set to drawer buffer */
			 black_pathlabel, NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button bro_tpa_sel = MB_INIT1(&bro_pat_sel, NOCHILD, 30, 7, 169, 188, NODATA, /* "Dir", */
									 grey_ctext, NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);

static Button bro_dev_hanger =
	MB_INIT1(&bro_tpa_sel, NOCHILD, 13, 9, 209, 156, NODATA, NULL, /* set by alloc dev sels */
			 NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);

static Rectangle bro_devsel_320size = {14, 9, 15, 11}; /* width,height,dx,dy */

static Button bro_tdr_sel =
	MB_INIT1(&bro_dev_hanger, NOCHILD, 30, 7, 175, 157, NODATA, /* "Drive", */
			 grey_ctext, NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button bro_ok_sel =
	MB_INIT1(&bro_tdr_sel, NOCHILD, 45, 9, 4, 189, NODATA, /* "Ok", */
			 ccorner_text, browse_action, mb_close_cancel, NOGROUP, 0, '\r', 0);

/* lots of casting need here aaack */
#define CPI_NAME (*((char**)&(bro_tna_sel.datme)))

static Button bro_tna_sel =
	MB_INIT1(&bro_ok_sel, NOCHILD, 95, 7, 4, 180, NULL, /* set in browse_files() */
			 black_ctext, NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button bro_can_sel =
	MB_INIT1(&bro_tna_sel, NOCHILD, 45, 9, 54, 189, NODATA, /* "Cancel", */
			 ccorner_text, mb_close_cancel, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
...
static Button bro_play_sel =
	MB_INIT1(&bro_can_sel, NOCHILD, 45, 9, 54, 189, NOTEXT, NOSEE, change_mode, mb_close_cancel,
			 &vs.browse_action, BA_VIEW, 'v', MB_B_GHILITE);
static Button bro_del_sel = MB_INIT1(&bro_play_sel, NOCHILD, 45, 9, 54, 170, NODATA, /* "Delete", */
									 ccorner_text, change_mode, mb_close_cancel, &vs.browse_action,
									 BA_KILL, 'd', MB_B_GHILITE);
static Button bro_inf_sel = MB_INIT1(&bro_del_sel, NOCHILD, 45, 9, 54, 162, NODATA, /* "Info", */
									 ccorner_text, change_mode, mb_close_cancel, &vs.browse_action,
									 BA_INFO, 'i', MB_B_GHILITE);
static Button bro_vie_sel = MB_INIT1(&bro_inf_sel, NOCHILD, 45, 9, 4, 170, NODATA, /* "Play", */
									 ccorner_text, change_mode, mb_close_cancel, &vs.browse_action,
									 BA_PLAY, 'p', MB_B_GHILITE);
static Button bro_loa_sel = MB_INIT1(&bro_vie_sel, NOCHILD, 45, 9, 4, 162, NODATA, /* "Load", */
									 ccorner_text, change_mode, mb_close_cancel, &vs.browse_action,
									 BA_LOAD, 'l', MB_B_GHILITE);
static Button bro_tit_sel = MB_INIT1(&bro_loa_sel, NOCHILD, 95, 7, 4, 153, NOTEXT, /* "Browse", */
									 see_titlebar, NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button bro_tst_sel = MB_INIT1(&bro_tit_sel, NOCHILD, 320, 4, 0, 147, NOTEXT, grey_block,
									 NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
static Button brw_panel = MB_INIT1(NONEXT, &bro_tst_sel, 320, 53, 0, 147, NOTEXT, wbg_ncorner_back,
								   NOFEEL, mb_close_cancel, NOGROUP, 0, NOKEY, 0);
 ...
 * a visible cel */
{
	...
	if ((*pel = name_in_list(CPI_NAME, bro_wild_list)) != NULL) {
		elix = slist_ix((Slnode*)bro_wild_list, (Slnode*)*pel) - bscroller.top_name;
...
static void text_1_browse(Raster* rast, Vfont* font, Names* el, Pixel color, int x, int y)
{
	int textx;
	...
	gftext(rast, font, el->name, textx, y + brw_cpi_sel.height + 1 + vb.screen->is_hires, color,
		   TM_MASK1);
}

static void hilite_1_browse(Button* b, Raster* rast, int x, int y, Names* entry, bool hilit)
{
	Rectangle brect;
		...
		if (el->name[0] != DIR_DELIM && el->name[0] != 0) {
			if ((err = postage_stamp(rast, el->name, x + 1, y + 1, brw_cpi_sel.width - 2,
				...
				gftext(rast, font, cant_read_str,
					   psize.x + font_xcent_oset(font, cant_read_str, psize.width),
					   psize.y + font_ycent_oset(font, psize.height), sred, TM_MASK1);
			} else {
				psize.x -= 1;
				psize.y -= 1;
				psize.width += 2;
...
static void feel_1_browse(Button* list_sel, Raster* rast, int x, int y, Names* entry, int why)
{
	char* title;
	(void)rast;
	(void)x;
...
static Errcode view_anim(char* name, Rcel* screen, bool loop)
...
/* loop is 0 or 1.  1 if want to repeat animation until key hit */
{
	...
	cel = center_virtual_rcel(screen, &stack_cel, flif.hdr.width, flif.hdr.height);
...
static void draw_cpi(Button* m)
{
	...
	if ((elix = find_elix(&spos, &el)) >= 0) {
		...
		pj_blitrect(root, brect.x, brect.y, root, m->x + brect.x - spos.x, m->y + brect.y - spos.y,
	...
	} else /* see if there is a file */
	{
		...
		if ((postage_stamp(root, CPI_NAME, m->x + 1, m->y + 1, brw_cpi_sel.width - 2,
						   brw_cpi_sel.height - 2, &brect)) < Success) {
			return;
		}
...
static Errcode show_fli_info(char* title)
{
	...
	if (err >= Success) {
		soft_continu_box("!%s%d%ld%ld%d%d%d", "browse_info", title, fh.frame_count, fh.size,
						 fh.size / (fh.frame_count + 1), millisec_to_jiffies(fh.speed), fh.width,
						 fh.height);
	} else {
		...
		soft_continu_box("!%s%ld%d%d", "browse_cel_info", title, size, pic.width, pic.height);
	}
	err = Success;
	...
	switch (vs.browse_action) {
		...
		case BA_KILL: /* kill */
		{
			if (really_delete(fpath)) {
				if ((softerr(pj_delete(fpath), "!%s", "cant_delete", fpath)) < Success) {
					break;
				}
				if ((elix = find_elix(&spos, &el)) >= -1) {
					el->name[0] = 0; /* mark it dead */
					if (elix >= 0) {
						draw_1_browse(&brw_list_sel, (Raster*)(m->root), spos.x, spos.y, el);
					}
				}
				CPI_NAME[0] = 0;
				bredraw_cpic();
...
static Errcode browse_anims(char* inpath, char* outpath, char* title_key, char* loadb_key,
							SHORT* scroll_top)
   ...
   button for default/accept file radio button */
{
	...
	if ((err = soft_buttons(panel_key, bro_smblist, Array_els(bro_smblist), &ss)) < Success) {
		goto error;
	}
	if ((err = alloc_dev_sels(&bro_dev_hanger, &bro_devsel_320size, 7, 3, drawer, new_bdrawer,
							  drawer)) < Success) {
		goto error;
	}
...
/* do browse menu with default action to load */
{
	...
	if (browse_anims(cpath.path, name, "fli_title", "load", &cpath.scroller_top) >= Success) {
...
/* do browse menu with default action to load */
{
	...
	if ((err = browse_anims(cpath.path, name, "cel_title", "load", &cpath.scroller_top)) >=
		Success) {
