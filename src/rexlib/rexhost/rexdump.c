...
static void conv_old(EXP_HDR* hdrsp, OEXP_HDR* ohdrsp)
{
	...
	hdrsp->exp_ldisize =
		((ohdrsp->exe_size) - 1) * REX_BLK_SIZE + (ohdrsp->exe_szrem) - (hdrsp->exp_ldimg);
...
static Errcode dump_code_and_fixup(Jfile file, EXP_HDR* hdrsp, OEXP_HDR* ohdrsp)
{
	...
	*/
	...
	if ((retcode = pj_readoset(file, load_addr, hdrsp->exp_ldimg, hdrsp->exp_ldisize)) < Success) {
		goto ERROUT;
	}
...
int main(int argc, char** argv)
{
	...
	switch (argc) {
		...
		default:
			printf(
				"Usage: rexdump rex_file_path [outfile [password]]\n"
