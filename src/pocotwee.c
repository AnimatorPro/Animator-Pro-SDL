...
extern Errcode tween1(void* tween1_data, int ix, int intween, int scale,
					  Autoarg* aa);  // from tweenhi.c
...
static ErrCode po_tween_get_poly(Poly* poly, Popot* pptcount, Popot* ppx, Popot* ppy)
{
	...
	if ((ptcount = pptcount->pt) == NULL || (px = ppx->pt) == NULL || (py = ppy->pt) == NULL) {
		return builtin_err = Err_null_ref;
	}
...
{
	return po_tween_set_poly(ptcount, &pop_x, &pop_y, &poco_tween_state.p0, &poco_tween_state.p1);
}
...
{
	return po_tween_get_poly(&poco_tween_state.p0, &pop_ptcount, &pop_x, &pop_y);
}
...
{
	return po_tween_set_poly(ptcount, &pop_x, &pop_y, &poco_tween_state.p1, &poco_tween_state.p0);
}
...
{
	return po_tween_get_poly(&poco_tween_state.p1, &pop_ptcount, &pop_x, &pop_y);
}
...
static ErrCode po_tween_one_link(int start_point, int end_point)
{
	...
	return tween_add_a_link(&poco_tween_state, start_point, end_point, vs.closed_curve, &nlink);
}
...
static ErrCode po_tween_set_links(int link_count, Popot pop_starts, Popot pop_ends)
{
	...
	if ((err = tween_add_a_link(&poco_tween_state, start, end, vs.closed_curve, &nlink)) <
		Success) {
		return err;
	}
	...
	for (i = 1; i < link_count; ++i) {
		if ((err = tween_add_a_link(&poco_tween_state, starts[i], ends[i], vs.closed_curve,
									&nlink)) < Success) {
			return err;
		}
	}
...
static ErrCode po_tween_get_links(Popot pop_link_count, Popot pop_starts, Popot pop_ends)
{
	...
	if ((plink_count = pop_link_count.pt) == NULL || (pstarts = pop_starts.pt) == NULL ||
	...
	if (link_count != 0) {
		if ((err = po_lmalloc_2(&starts, &ends, link_count * sizeof(int))) < Success) {
		...
		for (link = (Tween_link*)(poco_tween_state.links.head);
			 NULL != (next = (Tween_link*)(link->node.next)); link = next) {
			*s++ = link->start;
			*e++ = link->end;
		}
	}
 ...
 * 2 arrays format.
 *--------------------------------------------------------------------------*/
static ErrCode po_short_xyz_to_arrays(int point_count, Short_xyz* point_list, Popot* ret_x,
									  Popot* ret_y)
{
	Popot x, y;
	Errcode err;
...
static ErrCode po_tween_make_poly(double time, Popot pop_ptcount, Popot pop_x, Popot pop_y)
{
	...
	if ((ppoint_count = pop_ptcount.pt) == NULL || (px = pop_x.pt) == NULL ||
		(py = pop_y.pt) == NULL) {
	...
	if ((err = ts_to_tw_list(&poco_tween_state, vs.closed_curve, &tlist)) < Success) {
		goto CLEANUP;
	}
	scale = SCALE_ONE * time;
	calc_tween_points(&tlist, vs.closed_curve, scale, &points, &point_count);
	if ((err = po_short_xyz_to_arrays(point_count, points, px, py)) < Success) {
		goto CLEANUP;
	}
...
ErrCode po_tween_load(Popot pop_file_name) ErrCode
	po_tween_save(Popot pop_file_name) bool po_tween_exists(void) void po_tween_clear(void) ErrCode
	po_tween_set_start(int ptcount, Popot pop_x, Popot pop_y) ErrCode
	po_tween_get_start(Popot pop_ptcount, Popot pop_x, Popot pop_y) ErrCode
	po_tween_set_end(int ptcount, Popot pop_x, Popot pop_y) ErrCode
	po_tween_get_end(Popot pop_ptcount, Popot pop_x,
					 Popot pop_y) void po_tween_swap_ends(void) ErrCode
	po_tween_end_to_start(void) ErrCode po_tween_one_link(int start_point, int end_point) ErrCode
	po_tween_set_links(int link_count, Popot pop_starts, Popot pop_ends) ErrCode
	po_tween_get_links(Popot pop_link_count, Popot pop_starts, Popot pop_ends) void po_tween_clear_links(
		void) void po_tween_set_splined(bool is_splined) bool po_tween_get_splined(void) ErrCode
	po_tween_trails(void) ErrCode
	po_tween_make_poly(double time, Popot pop_ptcount, Popot pop_x, Popot pop_y) ErrCode
	po_tween_render(void)
#endif /* NEVER */
