...
main()
{
	...
	if ((err = alloc_display_rcel(vdevice, &screen, mode_info.width.actual, mode_info.height.actual,
								  mode)) < Success) {
		goto ERROR;
	}
...
int random(void)
{
	return ((((ULONG)(randseed = ((randseed * 0x41c64e6d) + 0x3039))) >> 10) & 0x7fff);
}
...
static void calc_bounce_rect(Moving_wndo* this, Rectangle* pos)
{
	...
	pos->x += this->dx;
	pos->y += this->dy;
	if (pos->x < 0 || pos->x > win->W_screen->wndo.width - pos->width) {
		this->dx = -this->dx;
		pos->x += this->dx;
	}
	if (pos->y < 0 || pos->y > win->W_screen->wndo.height - pos->height) {
		this->dy = -this->dy;
		pos->y += this->dy;
	}
}
