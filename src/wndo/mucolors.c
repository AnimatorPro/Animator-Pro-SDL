...
static void f_colors(Wscreen* s, Cmap* cmap, Pixel* mucolors, bool set_lasts, bool force_unique)
{
	int i;
	...
	for (i = 0; i < NUM_MUCOLORS; ++i) {
		if (force_unique) {
			*mucolors = closestc_excl(&(s->mc_ideals[i]), cmap->ctab, cmap->num_colors, ignores, i);
			ignores[i] = *mucolors;
		} else {
			*mucolors = closestc(&(s->mc_ideals[i]), cmap->ctab, cmap->num_colors);
		}
		if (set_lasts) {
...
{
	static Pixel mcolors[] = {FIRST_MUCOLOR, FIRST_MUCOLOR + 1, FIRST_MUCOLOR + 2,
							  FIRST_MUCOLOR + 3, FIRST_MUCOLOR + 4};

	if (!visible_mucolors(s->viscel->cmap, s->mc_colors)) {
		s->mc_alt = (UBYTE)(~(((UBYTE)~0) << NUM_MUCOLORS));
		pj_set_colors(s->viscel, FIRST_MUCOLOR, NUM_MUCOLORS, (UBYTE*)(s->mc_ideals));
...
bool lastmuc_changed(Wscreen* s)
{
	...
	for (i = 0; i < NUM_MUCOLORS; ++i) {
		...
		if (last->r != this->r || last->g != this->g || last->b != this->b) {
			return (1);
		}
		++last;
	}
...
/* returns true and installs window refresh task if color set was changed */
{
	...
	f_mucolors(ws, false);
	check_mucmap(ws);
	if (memcmp(&ocolors, &ws->mc_colors, sizeof(ocolors)) || oalt != ws->mc_alt) {
		++ws->mc_csetid;
...
static int rtask_func(Waitask* wt)
{
	...
	for (w = (Wndo*)(ws->wilist.head); (next = ((Dlnode*)w)->next) != NULL; w = (Wndo*)next) {
