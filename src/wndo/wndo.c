...
typedef void (*Bhblit)(Raster* behind, Coor x, Coor y, Raster* rast, Coor rx, Coor ry, Ucoor width,
					   Ucoor height);

static void blit_saveonly(Raster* behind, Coor x, Coor y, Raster* rast, Coor rx, Coor ry,
		...
		for (;;) {
			...
			if (nextx >= xmax) /* equal will always be */
			{
				if (rastid != w->W_rastid) {
					rast = w->rasts[rastid];
					(*blit)(&w->behind, x, y, rast, (x + w->behind.x - rast->x), (osy - rast->y),
							(xmax - x), (nexty - y));
				}
				if (nexty >= ymax) {
					return;
 ...
 * redraws a window, must have either a newpos a newoset or both */
{
	...
	if ((pos.x == w->x) && (pos.y == w->y) && (pos.MaxX == w->W_xmax) && (pos.MaxY == w->W_ymax) &&
		(oset.x == woset.x) && (oset.y == woset.y)) {
		return (0); /* no change */
	}

	if (!(w->flags & WNDO_HIDDEN) &&
		(wasin = crects_overlap((Cliprect*)&(w->CRECTSTART), (Cliprect*)&(ws->wndo.CRECTSTART))) !=
			0) {
		if (w->flags & WNDO_BACKDROP) /* move it's image to new place */
		{
			blitmove_rect(&(ws->wndo), w->x, w->y, &(ws->wndo), pos.x, pos.y, w->width, w->height);
			set_leftbehind(&(ws->wndo), 0, w->x, w->y, pos.x, pos.y, w->width, w->height);
		} else {
	...
	if (crects_overlap((Cliprect*)&(w->CRECTSTART), (Cliprect*)&(ws->wndo.CRECTSTART))) {
 ...
 * in it so the window may be scrolled smoothly across the screen */
{
	...
	for (;;) {
		...
		pj_blitrect(&(w->behind), w->x - w->behind.x, w->y - w->behind.y, (Raster*)&(ws->wndo),
		...
		if (CODEOBSCURES !=
			clipcode_crects((Cliprect*)&(w->CRECTSTART), (Cliprect*)&(ws->wndo.CRECTSTART))) {
			/* clear any area left uncovered by last blit */
			set_leftbehind(&(ws->wndo), 0, lastx, lasty, w->x, w->y, w->W_xmax - w->x,
						   w->W_ymax - w->y);
		}
	}
...
/* makes window invisible but maintains position in list */
{
	...
	if ((NULL == w->W_node.next) || w->flags & WNDO_HIDDEN) {
		return;
	}
	ws = w->W_screen;
...
#ifdef FORTESTING
	free =
		pj_malloc((TOSTRUCT(Wndo, free_ydots, freeclips)->behind.height) + (1 * sizeof(Cliphead)));
 ...
 */
{
	...
	sclip_rect((Rectangle*)&(w->RECTSTART), (Rectangle*)&(ws->viscel->RECTSTART));
...
#endif
		if ((wi->over != NULL) && (find_header(&(wi->over->W_node)) == &(ws->wilist))) {
...
{
	if ((x < w->x) || (x >= w->W_xmax) || (y < w->y) || (y >= w->W_ymax)) {
		return (0);
	}
	return (1);
}
...
{
	if (((Ucoor)x) >= w->width || ((Ucoor)y) >= w->height) {
		return (0);
	}
