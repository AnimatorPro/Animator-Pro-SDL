...
static void _win_put_hseg(Raster* wndo, Pixel* pixbuf, Coor x, Coor y, Ucoor width)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		...
		if (nextx >= xmax) {
			rast = w->rasts[rastid];
			PUT_HSEG(rast, pixbuf, x + w->behind.x - rast->x, osy - rast->y, xmax - x);
			return;
		}
...
static void _win_get_hseg(Raster* wndo, Pixel* pixbuf, Coor x, Coor y, Ucoor width)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		...
		if (nextx >= xmax) {
			rast = w->rasts[rastid];
			GET_HSEG(rast, pixbuf, x + w->behind.x - rast->x, osy - rast->y, xmax - x);
			return;
		}
		ydots = w->ydots[nextx];  /* get next ydots for nextx */
		if (rastid == ydots[y]) { /* if same raster continue and check next one */
...
static void _win_put_vseg(Raster* wndo, Pixel* pixbuf, Coor x, Coor y, Ucoor height)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		if (nexty >= ymax) {
			rast = w->rasts[rastid];
			PUT_VSEG(rast, pixbuf, osx - rast->x, y + w->behind.y - rast->y, ymax - y);
			return;
		}
		if (rastid != ydots[nexty]) /* if next not same raster do segment */
		{
			...
			rastid = ydots[nexty];
			hdif = nexty - y;
			PUT_VSEG(rast, pixbuf, osx - rast->x, y + w->behind.y - rast->y, hdif);
			pixbuf += hdif;
			y = nexty;
		}
...
static void _win_get_vseg(Raster* wndo, Pixel* pixbuf, Coor x, Coor y, Ucoor height)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		if (nexty >= ymax) {
			rast = w->rasts[rastid];
			GET_VSEG(rast, pixbuf, osx - rast->x, y + w->behind.y - rast->y, ymax - y);
			return;
		}
		if (rastid != ydots[nexty]) /* if next not same raster do segment */
		{
			...
			rastid = ydots[nexty];
			hdif = nexty - y;
			GET_VSEG(rast, pixbuf, osx - rast->x, y + w->behind.y - rast->y, hdif);
			pixbuf += hdif;
			y = nexty;
		}
		nexty = w->vchanges[nexty];
...
static void _win_set_hline(Raster* wndo, Pixel color, Coor x, Coor y, Ucoor width)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		...
		if (nextx >= xmax) {
			rast = w->rasts[rastid];
			SET_HLINE(rast, color, x + w->behind.x - rast->x, osy - rast->y, xmax - x);
			return;
		}
		ydots = w->ydots[nextx];  /* get next ydots for nextx */
		if (rastid == ydots[y]) { /* if same raster continue and check next one */
...
static void _win_set_vline(Raster* wndo, Pixel color, Coor x, Coor y, Ucoor height)
{
	Wndo* w = (Wndo*)wndo;
	SHORT rastid;
	UBYTE* ydots;
	...
	for (;;) {
		if (nexty >= ymax) {
			rast = w->rasts[rastid];
			SET_VLINE(rast, color, osx - rast->x, y + w->behind.y - rast->y, ymax - y);
			return;
		}
		if (rastid != ydots[nexty]) /* if same raster check next one */
		{
			rast = w->rasts[rastid];
			rastid = ydots[nexty];
			SET_VLINE(rast, color, osx - rast->x, y + w->behind.y - rast->y, nexty - y);
			y = nexty;
		}
		nexty = w->vchanges[nexty];
	}
}
static void _win_set_rect(Raster* wndo, Pixel color, Coor x, Coor y, Ucoor width, Ucoor height)
/* set a rectangle to a color */
{
	...
	for (;;) {
		...
		for (;;) {
			...
			if (nextx >= xmax) {
				rast = w->rasts[rastid];
				SET_RECT(rast, color, x + w->behind.x - rast->x, osy - rast->y, xmax - x, dheight);
				if (nexty >= ymax) {
					return;
				}
			...
			rast = w->rasts[rastid];
			rastid = ydots[y];
			SET_RECT(rast, color, x + w->behind.x - rast->x, osy - rast->y, nextx - x, dheight);
			x = nextx;
		}
		x = firstx;
		y = nexty;
...
static void _win_xor_rect(Raster* wndo, Pixel color, Coor x, Coor y, Ucoor width, Ucoor height)
/* xors a rectangle with a color */
{
	...
	for (;;) {
		...
		for (;;) {
			...
			if (nextx >= xmax) {
				rast = w->rasts[rastid];
				XOR_RECT(rast, color, x + w->behind.x - rast->x, osy - rast->y, xmax - x, dheight);
				if (nexty >= ymax) {
					return;
				}
				break; /* break for nextx loop */
			...
			rast = w->rasts[rastid];
			rastid = ydots[y];
			XOR_RECT(rast, color, x + w->behind.x - rast->x, osy - rast->y, nextx - x, dheight);
			x = nextx;
		}
		x = firstx;
		y = nexty;
...
static void _win_mask1blit(UBYTE* mbytes, unsigned int mbpr, Coor mx, Coor my, Raster* wndo, Coor x,
						   Coor y, Ucoor width, Ucoor height, Pixel oncolor)
{
	Wndo* w = (Wndo*)wndo;
	SHORT xmax, ymax;
	SHORT nextx, nexty;
	...
	for (;;) {
		...
		for (;;) {
			...
			if (nextx >= xmax) {
				rast = w->rasts[rastid];
				MASK1BLIT(mbytes, mbpr, mx, my, rast, x + w->behind.x - rast->x, osy - rast->y,
...
static void _win_mask2blit(UBYTE* mbytes, unsigned int mbpr, Coor mx, Coor my, Raster* wndo, Coor x,
						   Coor y, Ucoor width, Ucoor height, Pixel oncolor, Pixel offcolor)
{
	Wndo* w = (Wndo*)wndo;
	SHORT xmax, ymax;
	SHORT nextx, nexty;
	...
	for (;;) {
		...
		for (;;) {
			...
			if (nextx >= xmax) {
				rast = w->rasts[rastid];
				MASK2BLIT(mbytes, mbpr, mx, my, rast, x + w->behind.x - rast->x, osy - rast->y,
...
static check_set_mcalt(Wscreen* s, LONG start, LONG count, Rgb3* ctab)
{
	...
	while (start < count) {
		if (mask & s->mc_alt) {
			if (ideals->r != ctab->r || ideals->g != ctab->g || ideals->b != ctab->b) {
				s->mc_alt &= ~mask;
			}
		}
		++ideals;
...
static void cset_check_mcalt(Wscreen* s)
{
	...
	while (mask) {
		if (mask & s->mc_alt) {
			if (altnew->r != ideals->r || altnew->g != ideals->g || altnew->b != ideals->b) {
				s->mc_alt &= ~mask;
			}
			mask >>= 1;
			--altnew;
