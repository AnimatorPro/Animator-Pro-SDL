...
static void init_tbox(Tbox* tbox, Wscreen* s, char* formats, char* text, va_list args,
					  char* extratext)
{
	clear_struct(tbox);
	tbox->text = text;
...
static void diag_line(void* rast, int color, int dots, int x, int y, int dx, int dy)
{
	while (--dots >= 0) {
...
static void drawcpyr(void* rast, int color, int count, int x, int y, int w, int dy)
...
static int build_tbclicks(Tbox* tbox, register Tbclicker* clicks, char** tblock)
...
/* adds xoset to clicks x value and draws clicks at positino of tbox */
{
	...
	while (clicks) {
		...
		diag_inside(s->viscel, x, y, clicks->width, clicks->height, s->SBLACK, s->bbevel);
		box_diag_corner(s->viscel, x, y, clicks->width, clicks->height, s->SGREY, s->bbevel);

		gftext(s->viscel, s->mufont, clicks->text, x + clicks->tx, y + clicks->ty, s->SWHITE,
			   TM_MASK1);
...
{
	if (tbox->rastopen) {
		pj_blitrect(&(tbox->r), 0, 0, tbox->tbs->viscel, tbox->r.x, tbox->r.y, tbox->r.width,
					tbox->r.height);
...
/* returns Success if all ok ecode on fail */
{
	...
	{
		if ((err = get_formatted_ftext(&tbuf, sizeof(sbuf), tbox->formats, text, tbox->args,
									   false)) < Success) {
			goto error;
		}
		if (tbuf != NULL) {
		...
		tbox->r.height += (cheight * wwcount_lines(tbs->mufont, text, (tbs->wndo.width / 4) * 3,
												   (SHORT*)&tbox->r.width));
	...
	if (tbox->extratext) {
		...
		tbox->r.height += cheight * (1 + wwcount_lines(tbs->mufont, tbox->extratext,
													   (tbs->wndo.width / 4) * 3, &txtwidth));
	...
	get_requestor_position(tbs, tbox->r.width, tbox->r.height, (Rectangle*)&(tbox->r.RECTSTART));
	...
	pj_blitrect(tbs->viscel, tbox->r.x, tbox->r.y, &(tbox->r), 0, 0, tbox->r.width, tbox->r.height);

	/* fill rectangle with solid color */

	pj_set_rect(tbs->viscel, tbs->SWHITE, tbox->r.x, tbox->r.y, tbox->r.width, tbox->r.height);

	/* draw border around whole box */

	draw_quad((Raster*)tbs->viscel, tbs->SGREY, tbox->r.x, tbox->r.y, tbox->r.width,
			  tbox->r.height);

	/* draw textblock with border offsets */

	if (text != NULL) {
		wwtext(tbs->viscel, tbs->mufont, text, tbox->r.x + txbord, tbox->r.y + vbord,
			   tbox->r.width - (txbord << 1), tbox->r.height, 0, JUST_LEFT, tbs->SBLACK, TM_MASK1,
			   0);
	}

	if (tbox->extratext != NULL) {
		wwtext(tbs->viscel, tbs->mufont, tbox->extratext, tbox->r.x + txbord, tbox->r.y + extray,
			   tbox->r.width - (txbord << 1), tbox->r.height, 0, JUST_CENTER, tbs->SBLACK, TM_MASK1,
			   0);
	}
...
Errcode tboxf_choice(Wscreen* s, char* formats, char* text, va_list args, char** choices,
					 char* extratext)
{
	Tbox tbox;
