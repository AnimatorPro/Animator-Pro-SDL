...
Errcode va_wait_wndo(Wscreen* screen, char* wait_str, char* formats, char* text, va_list args)
{
	Errcode err;
	char sbuf[256];
	char* tbuf;
	...
	draw_quad((Raster*)screen->wait_box, screen->SGREY, 0, 0, box.width, box.height);

	box.x = cwid;
	box.width -= (cwid << 1);
	justify_line(screen->wait_box, f, wait_str, box.x, box.y, box.width, screen->SBLACK, TM_MASK1,
				 0, JUST_CENTER, NULL, 0);
	box.y += (lineht << 1);

	wwtext(screen->wait_box, f, text, box.x, box.y, box.width, box.height, 0, JUST_LEFT,
		   screen->SBLACK, TM_MASK1, 0);
