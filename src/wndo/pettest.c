...
static WndoInit newin3 = {40, 110, 20, 40, 0, 0, NULL, NULL, 0};
...
static void do_text(Raster* rast, register char* s, int x, int y, int color, VFUNC tblit,
					int bcolor)
{
...
static test_rects(Wndo* w, Raster* r)
{
	...
	for (y = 0; y < w->height; y += 7) {
		do_text((Raster*)w, "lots and lots.  Megabytes.  Gigabytes.  Terabytes.  RAM!!!!", 0, y, 0,
				pj_mask1blit, 20);
	}
}
...
static void main(int argc, char** argv)
{
	...
	for (i = 1; i < SWS + 1; i++) {
		...
		do_text((Raster*)smallw[i], buf, 0, 0, 0, pj_mask1blit, 20);
	}
	smallw[SWS + 1] = wndo2;
	smallw[SWS + 2] = wndo3;
