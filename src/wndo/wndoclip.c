...
/* The window-clip code passes a trailing debug label to pj_stuff_pointers;
 * drop it and call the real three-argument routine. */
 ...
 * clipped already before this is called */
{
	...
	if (STARTX(dbuf) < crect->x) {
		...
		pj_stuff_pointers(nextdbuf, &dbufs[osrect.x], NEXTX(nextdbuf) - STARTX(nextdbuf), 0);
...
next_is_within:
		...
		for (;;) {
			...
			{
				pj_stuff_pointers(dbuf, &dbufs[osrect.x], crect->MaxX - STARTX(nextdbuf), 2);
 ...
 * returns 1 if root window is obscured by a window 0 if not */
{
	...
	for (;;) {
		...
		if (!(w->flags & WNDO_HIDDEN) && and_cliprects((Cliprect*)&(cw.rect.CRECTSTART),
													   (Cliprect*)&(w->CRECTSTART), &clipped)) {
			...
			if ((NEXTX(w->ydots[0]) == w->behind.width) && (w->vchanges[0] == w->behind.height)) {
				w->onerast = *(w->ydots[0]);
