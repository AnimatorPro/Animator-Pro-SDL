...
static Errcode init_celt_cache(Ink* inky)
{
	...
	cache_size =
		sizeof(Celt_cache) + ((vb.pencel->width + vb.pencel->height) * (sizeof(SHORT) * 2));
	...
	if (thecel->cd.rotang.z) {
		if (dot.x != thecel->cd.cent.x || dot.y != thecel->cd.cent.y) {
			...
			/* integer fixed point */
			{
				...
				dot.x =
					((LONG)(fx + ((fx < 0 ? -1 : 1) << (SCSH - 1))) >> SCSH) + thecel->cd.cent.x;

				dot.y =
					((LONG)(fy + ((fy < 0 ? -1 : 1) << (SCSH - 1))) >> SCSH) + thecel->cd.cent.y;
			}
		}
	}
...
Pixel celt_dot(const Ink* inky, SHORT x, SHORT y)
{
	...
	if (NULL != (cfit = thecel->cfit) && !(cfit->flags & CCFIT_NULL)) {
...
void celt_hline(const Ink* inky, SHORT x0, SHORT y, SHORT width)
{
	...
	if (ct_cache->get_hseg == NULL) {
		...
		if (thecel->cd.rotang.z != 0 || width <= ct_cache->width) {
			dpt = sbuf + width;
			while (spt < dpt) {
		...
		} else /* have to get hline in pieces */
		{
			...
			for (;;) {
				if (w <= ct_cache->width) {
					...
					ct_cache->get_hseg((Raster*)cel, spt, 0, cxy.y, ct_cache->width);
					spt += ct_cache->width;
					w -= ct_cache->width;
				}
			}
