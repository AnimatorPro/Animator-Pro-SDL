...
static void cmask_line(UBYTE* line1, UBYTE* line2, Bitmap* mask, SHORT y)
{
	int width;
	UBYTE* mbyte;
	UBYTE mbit;
...
static void cupdate_line(UBYTE* newline, UBYTE* oldline, UBYTE* destline, Bitmap* mask, SHORT y)
{
	int width;
	UBYTE* mbyte;
	UBYTE mbit;
...
static int update_changes(Rcel* new, Rcel* old, Rcel* dest, Bitmap* mask)
...
static void restore_blue(Rcel* src, Rcel* dest, Pixel ink)
	...
	if ((alloc_mask(&mask, 0, 0)) >= 0) {
		if (report_temp_save_rcel(&opic, vb.pencel) >= 0) {
			...
			if (err >= 0) {
				...
				{
					pj_rcel_copy(undof, vb.pencel);
					pj_mask1blit(mask->bm.bp[0], mask->bm.bpr, 0, 0, vb.pencel, 0, 0, mask->width,
								 mask->height, vs.inks[1]);
				} else {
