...
{
	SHORT cel_pulltab[] = {
		POS_REST_PUL, POS_MIR_PUL, POS_TUR_PUL, POS_STR_PUL, POS_TO__PUL, CEL_SVC_PUL, CEL_SAV_PUL,
	};
...
static void cm_save_cursor(void)
{
	...
	hide_mp();
	if (thecel->cd.stretch.x || thecel->cd.stretch.y || thecel->cd.rotang.x ||
		thecel->cd.rotang.y || thecel->cd.rotang.z) {
	...
	if ((title = pj_get_filename(stack_string("save_cursor", buf), ".CUR", save_str, "", NULL, 1,
								 NULL, NULL)) == NULL) {
		goto done;
	}
