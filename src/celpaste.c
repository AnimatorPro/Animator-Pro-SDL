...
static Button apa_paintopt_sel = MB_INIT1(NONEXT,                             /* next */
										  &cmu_pa_group_sel, 0, 0, 121, 27,   /* w,h,x,y */
										  NOTEXT, hang_children,              /* seeme */
										  NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0 /* flags */
);
static Button apa_rendseq_sel =
	MB_INIT1(&apa_paintopt_sel, NOCHILD,                               /* children */
			 108, 9, 6, 29,                                            /* w,h,x,y */
			 NODATA,                                                   /* "Render Sprite", */
			 ccorner_text, render_overlay, NOOPT, NOGROUP, 0, NOKEY, 0 /* flags */
	);
static Button apa_kill_sel =
	MB_INIT1(&apa_rendseq_sel,                                       /* next */
			 NOCHILD,                                                /* children */
			 108, 9, 6, 41,                                          /* w,h,x,y */
			 NODATA,                                                 /* "Cancel Sprite", */
			 ccorner_text, kill_overlay, NOOPT, NOGROUP, 0, NOKEY, 0 /* flags */
	);

static Button apa_title_sel =
	MB_INIT1(&apa_kill_sel,    /* next */
			 &cmu_common_sels, /* children */
			 110, 9, 5, 3,     /* w,h,x,y */
			 NODATA,           /* "Sprite", */
			 hang_see_title, mb_clipmove_menu, mb_menu_to_bottom, NOGROUP, 0, NOKEY, 0 /* flags */
	);

static Menuhdr anipaste_menu = MENU_INIT0(320, 70, 0, 0,              /* width, height, x, y */
										  ANIPASTE_MUID,              /* id */
										  PANELMENU,                  /* type */
...
{
	if (check_toggle_menu() || common_header_keys() || check_undo_key()) {
		return (true);
	}
	return (false);
}
 ...
 * when this is called */
{
	...
	if ((err = soft_buttons("sprite_panel", apa_smblist, Array_els(apa_smblist), &ss)) < Success) {
		return (err);
	}
	oundo = vl.undoit;
	vl.undoit = NULL;
...
static Errcode render1_ram_olay(void* poverlays, int ix, int intween, int scale, Autoarg* aa)
...
static Errcode render1_file_olay(void* data, int ix, int intween, int scale, Autoarg* aa)
{
	Errcode err;
	Fod* fod = data;
...
get_next_chunk:
		...
		err = xffreadoset(fod->xf, &fod->next_one, fod->next_pos, sizeof(Chunk_id));
		if (err < Success) {
			break;
		}
...
static void do_delta_celcurs(SHORT dx, SHORT dy)
{
	...
	do_leftbehind(thecel->xf.mmax.x, thecel->xf.mmax.y, thecel->xf.ommax.x, thecel->xf.ommax.y,
...
static Errcode advance_draw_fli(Fli_frame* frame, VFUNC pre_decomp, void* pddat)
...
static Errcode add_cel_overlay(Fli_frame* cbuf, Short_xy* cpos, SHORT frame, Rectangle* cr,
							   int frame_ix)
{
	Errcode err;
	LONG size;
	...
	if ((size = pj_fli_comp_rect(cbuf, undof, vb.pencel, &ovrect, 0, COMP_DELTA_FRAME,
								 pj_fli_comp_ani)) < 0) {
		err = size;
		goto error;
	}
 ...
 * to undof. this clips */
{
	...
	if (chgr->height > (celr->height << 1)) {
		...
		save_undo_rect(chgr->x, chgr->y + celr->height, chgr->width,
	...
	} else /* vertical overlap */
	{
		if (chgr->width > (celr->width << 1)) /* horiz separation */
		{
			...
			save_undo_rect(chgr->x + celr->width, chgr->y + celr->height - height, width, height);
		}
...
{
	pj_blitrect(cb->crast2, 0, 0, vb.pencel, thecel->xf.mmax.x, thecel->xf.mmax.y,
...
static Errcode finish_cel_overlay(void)
{
	...
	if (vs.cm_blue_last && !vs.cm_streamdraw) {
		...
		pj_blitrect(rast2, 0, 0, undof, rast2->x, rast2->y, changerect.width, changerect.height);
	...
	pj_blitrect(rast1, 0, 0, undof, rast1->x, rast1->y, thecel->xf.mmax.width,
				thecel->xf.mmax.height);
 ...
 * cel's current position */
{
	...
	if (delta) {
		...
		save_not_within(&changerect, (Rectangle*)&(thecel->xf.mmax.RECTSTART));
	...
	pj_blitrect(undof, thecel->xf.mmax.x, thecel->xf.mmax.y, rast2, 0, 0, width, height);
	...
	if ((err = add_cel_overlay(cbuf, &thecel->cd.cent, thecel->cd.cur_frame, &changerect,
							   frame_ix)) < 0) {
		goto restore_error;
	}
...
restore_error:
	...
	pj_blitrect(rast2, 0, 0, undof, thecel->xf.mmax.x, thecel->xf.mmax.y, width, height);
...
/* alloc buffers needed exclusively for paint tool */
{
	...
	cmcb->finish_buf_size =
		pj_fli_cbuf_size(vb.pencel->width, vb.pencel->height, vb.pencel->cmap->num_colors);
...
/* Make sure we've got them or else! */
{
	...
	if (cmcb->crast1 != NULL && cmcb->crast1->width == thecel->xf.mmax.width &&
		cmcb->crast1->height == thecel->xf.mmax.height && cmcb->crast2 != NULL &&
			...
			if (JSTHIT(MBPEN)) {
				if (JSTHIT(MMOVE) ||
					((thecel->flif.hdr.frame_count > 1) &&
					 (vs.cm_streamdraw || (vs.cycle_draw && vs.render_one_color)))) {
					do_delta_celcurs(dx, dy);
				}
				save_fcel_undo(thecel);
