...
/* ellipse.c */
extern Errcode oval_loop(Poly* poly, Pixel color, int* pxrad, int* pyrad, int* ptheta);

/* poly.c */
extern void reverse_poly(Poly* p);

extern Errcode poly_to_vertices(Poly* poly, bool closed, Short_xyz** pvertices);

extern void poly_ave_3d(Poly* p, Short_xyz* v);

extern int calc_zpoly(Short_xyz* s, Short_xy* d, int count, int x, int y, int z);

extern void poly_to_3d(Poly* sp, Short_xyz* d);
extern void poly_bounds(Poly* p, struct cliprect* cr);

/* polytool.c */
extern void poly_cline_with_render_dot(SHORT x1, SHORT y1, SHORT x2, SHORT y2, void* data);
...
extern void make_poly_loop(Poly* poly, bool curved, bool closed, LLpoint* this, int color);

extern Errcode make_poly(Poly* p, bool closed);

extern int make_sp_wpoly(Poly* poly, int x0, int y0, int rad, int theta, int points, int star,
						 int sratio);

extern Errcode polystar_loop(Poly* poly, int star, int dot_color, int dash_color, int x0, int y0,
							 int* ptheta, int* prad);
...
extern void pp_find_next_prev(Poly* poly, LLpoint* point, LLpoint** next, LLpoint** prev);

extern void linkup_poly(Poly* p);
extern Errcode update_poly(Poly* s, Poly* d);

extern void dotty_disp_poly(Poly* p, bool closed, Pixel dit_color, Pixel dot_color);
...
/* spline.c */
extern int some_spline(Poly* poly, dotout_func dotout, void* dotdat,
					   void (*vecout)(SHORT, SHORT, SHORT, SHORT, dotout_func, void*), int closed,
					   int ir);

extern int partial_spline(Poly* poly, dotout_func dotout, void* dotdat,
...
extern void fill_add_shape(Poly* poly, UBYTE* on_off_buf, SHORT bpr, SHORT xoff, SHORT yoff);

extern Errcode fill_concave(Poly* poly, hline_func hline, void* hldat);

extern Errcode fill_on_off(SHORT bpr, SHORT width, SHORT height, SHORT xoff, SHORT yoff,
						   UBYTE* imagept, hline_func hline, void* hldat);

extern Errcode fill_poly_inside(Poly* pl, hline_func hline, void* hldat);

extern void hollow_polygon(Poly* poly, line_func lineout, void* ldat, bool closed);

extern Errcode filled_polygon(Poly* poly, hline_func hline, void* hldat, line_func line,
							  void* ldat);
