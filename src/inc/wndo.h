...
typedef struct wndo {
	RASTHDR_FIELDS; /* the common raster header for raster library
					 ...
					 * with w->width and w->height and are for the sake of the 					  *
					 * raster libraries */
...
#define WNDO_BACKDROP                                     \
	0x0004 /* window is a backdrop window that does not   \
...
#define WS_REFRESH_SET                                  \
	0x2000 /* a request to refresh the windows has been \
			* set */
#define WS_NOMENUS                                   \
	0x4000 /* in init flags don't allocate menu part \
			* starting with WS_FIRST_MENUFIELD       \
			* in screen flags "its not a menu screen" */
#define WS_MUCOLORS_UP                            \
	0x8000 /* menu colors are currently needed by \
			* windows */
...
#define MAX_WNDOS                                    \
	20 /* the largest number of windows one can open \
...
extern void box_diag_corner(void* rast, int x, int y, int w, int h, int color, int bevel);

extern void diag_inside(void* rast, int x, int y, int w, int h, int color, int bevel);
...
Errcode tboxf_choice(Wscreen* s, char* formats, char* text, va_list args, char** choices,
					 char* extratext);
Errcode va_wait_wndo(Wscreen* screen, char* wait_str, char* formats, char* fmt, va_list args);
