...
int smu_get_errtext(struct softmenu* sm, char* symname, /* name of symbol in resource file */
...
void pullfmt(struct menuhdr* mp, int subspace, int charw, int charh, Rectangle* screen_rect);
...
Errcode smu_string(struct softmenu* sm, char* key, char* buf, int len);
...
#define SCT_OPTIONAL                              \
	0x0001 /* if this is set items in the list    \
...
#define SCT_DIRECT                                \
	0x0002 /* treats toload as direct pointers to \
...
Errcode soft_name_scatters(char* symname, Smu_name_scats* scts, unsigned int num_scatters,
						   void** allocd, USHORT flags);
Errcode soft_load_ftext_type(char* key, va_list* pargs, char** pformats, char** ptext);
...
#define stack_string(key, buf) soft_string((key), (buf), sizeof(buf) / sizeof((buf)[0]))
