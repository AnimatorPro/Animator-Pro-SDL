...
#define TTEXTF_MAXCHARS                   \
	80 /* size of maximum displayed chars \
		* in ttextf() call */
...
extern Errcode varg_continu_box(char* formats, char* text, va_list args, char* etext);
...
bool ud_qreq_number(short* inum, short min, short max, Errcode (*update)(void* data, SHORT val),
					void* vfuncdat, char* hailing, ...);
...
bool varg_qreq_string(char* strbuf, int bufsize, char* formats, char* text, va_list args);
...
extern Errcode alloc_dev_sels(Button* hanger, Rectangle* size320x200, int numcols, int numrows,
							  char* drawer, Errcode (*on_newdrawer)(void* data), void* data);
