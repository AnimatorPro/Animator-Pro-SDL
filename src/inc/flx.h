...
Errcode gb_unfli_flx_frame(Flxfile* flx, struct rcel* screen, int ix, int wait, Fli_frame* frame);
...
extern Errcode write_first_flxframe(char* name, Flxfile* flxf, void* cbuf, struct rcel* frame1);

extern Errcode write_next_flxframe(char* name, Flxfile* flxf, void* cbuf, struct rcel* last_screen,
								   struct rcel* this_screen);

extern Errcode write_ring_flxframe(char* name, Flxfile* flxf, void* cbuf, struct rcel* last_screen,
								   struct rcel* first_screen);

extern Errcode write_first_flxblack(char* name, Flxfile* flxf, struct rcel* screen);
...
Errcode add_flx_olayrec(Short_xy* cpos, SHORT cframe, Rectangle* fpos, Fli_frame* rec,
						int frame_ix);
...
extern Errcode make_flx_record(Flxfile* flx, ULONG recnum, void* newdata, LONG size,
							   bool overwrite);
...
extern Errcode make_pdr_tempflx(char* pdr_name, char* flicname, struct anim_info* ainfo);
