...
#define DMARQI_MILLIS                                   \
	100 /* milli-seconds between marqi draws for creepy \
		 * marqis */
...
/* vmarqi.c */
extern void cinit_marqihdr(Marqihdr* mh, Pixel oncolor, Pixel offcolor, bool bothwins);
...
extern Errcode rubba_vertex(Short_xy* p0, Short_xy* p1, Short_xy* p2, void (*dispfunc)(Short_xy* v),
							Pixel color);

extern Errcode get_rub_vertex(Short_xy* p0, Short_xy* p1, Short_xy* p2, Pixel color);
...
extern void msome_vector(Short_xy* pts, int count, dotout_func dotout, void* dotdat, int open,
						 int pt_size);

extern void marqi_vector(Marqihdr* mh, Short_xy* pts, int count, int pt_size);

/* wndo/marqi.c */
extern void init_marqihdr(Marqihdr* mh, struct wndo* w, Rectangle* port, Pixel oncolor,
						  Pixel offcolor);
