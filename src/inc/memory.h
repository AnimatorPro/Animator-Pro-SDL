...
#define news(t) ((((t) = pj_zalloc(sizeof(*(t)))) != NULL) ? Success : Err_no_memory)
...
extern unsigned int pj_bcontrast(const void* xs, const void* ys, unsigned int n);

extern unsigned int pj_til_next_skip(const void* xs, const void* ys, unsigned int n,
									 unsigned int mustmatch);

extern unsigned int pj_til_next_same(const void* src, unsigned int n, unsigned int mustmatch);
