...
extern Errcode transpblit(struct rcel* tcel, int clearcolor, int clear, int tinting);  // 310

extern Errcode rblit_cel(struct rcel* c, struct tcolxldat* txd);  // 343

extern void render_mask_blit(UBYTE* mplane, SHORT mbpr, SHORT mx, SHORT my, void* drast, SHORT rx,
							 SHORT ry, USHORT width, USHORT height);  // 410

extern void render_mask_alpha_blit(UBYTE* alpha, int abpr, int x, int y, int w, int h,
...
extern Errcode render_transform(struct rcel* cel, struct xformspec* xf,
								struct tcolxldat* txd);  // 851
