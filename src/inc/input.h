...
#define SHOWCURSOR()                    \
	{                                   \
		if (WANTDRAWCURS) DRAWCURSOR(); \
	}
#define HIDECURSOR()                        \
	{                                       \
		if (WANTUNDRAWCURS) UNDRAWCURSOR(); \
	}

#define MOVECURSOR()                                                          \
	{                                                                         \
		if (icb.mset.on && icb.mcurs_up > 0) (*(icb.curs->moveit))(icb.curs); \
	}

#define SET_BUPBITS(state) ((state) |= (~((state) << BUPBIT1)) & ((MBPEN | MBRIGHT) << BUPBIT1))
...
#define ICB_COPYTO_LAST() *((struct icb_coords*)&(icb.lastmx)) = *((struct icb_coords*)&(icb.mx))

#define ICB_COPYFROM_LAST() *((struct icb_coords*)&(icb.mx)) = *((struct icb_coords*)&(icb.lastmx))

#define CHECK_POP_ICB(pushed)                     \
	{                                             \
		if (icb.push != pushed) _pop_icb(pushed); \
	}
...
#define MACRO_REC                                       \
	0x80U /* this is used in macro system for           \
...
#define HISTATE                                    \
	0x8000U /* the low priority state flags in the \
...
#define ICB_TIMEOUT                                        \
	0x10000000U /* anim wait input timed out or first call \
				 * for use inside anim funcs only */

#define HI_BSTATES                                            \
	0x01FF0000U /* low priority upper 16 bit states that will \
...
int anim_wait_input(ULONG waitflags, ULONG funcflags, int maxfields, FUNC func, void* funcdata);
