...
#define MB_GHILITE                         \
	1 /* button is hilit if *(SHORT*)group \
	   * is == b->identity */
#define MB_B_GHILITE                      \
	2 /* button is hilit if *(BYTE*)group \
	   * is == (BYTE)b->identity */
...
#define MB_ROOTISWNDO                                             \
	0x0020                 /* the root is pointer to the menuwndo \
...
#define MB_DISABOPT                                                       \
	0x0100                  /* optme is also disabled when MB_DISABLED is \
							 * true */
...
#define MB_SCALE_ABSW                                 \
	0x1000 /* when scaling, scale the width to the    \
			* absolute value of it's position and NOT \
			* relative to the button's start */
#define MB_SCALE_ABSH                                                                           \
	0x2000                                           /* when scaling, scale the height to the   \
													  * absolute value of it's position and NOT \
													  * relative to the button's start */
...
#define MENU_NOMB_RESCALE                           \
	0x0010 /* button scaling disabled for this menu \
...
#define QSL_INIT1(mi, ma, v, add1, ud, aro)                                                        \
	{mi,  ma, (v), aro, 12, QSL_LARRBOX | QSL_RARRBOX, ud, (add1) ? &qsfmt_shortp1 : &qsfmt_short, \
	 NULL}

#define QTSL_INIT1(mi, ma, v, add1, ud, aro, wrap) \
	{mi,  ma,                                      \
	 (v), aro,                                     \
	 12,  wrap ? QSL_MINWRAP | QSL_MAXWRAP : 0,    \
	 ud,  (add1) ? &qsfmt_shortp1 : &qsfmt_short}
...
LONG do_menuloop(Wscreen* screen, Menuhdr* menu, Button* initb, Menuhdr* pull,
				 FUNC default_doinput);
LONG do_reqloop(Wscreen* screen, Menuhdr* menu, Button* initb, Menuhdr* pull, FUNC default_doinput);
...
Errcode build_qnumreq(Wscreen* s, Menuhdr** pmh, char* hailing, char** ok_cancel, Image** arrows,
...
extern Errcode build_qstrreq(Wscreen* s, Menuhdr** pmh, char* hailing, char** ok_cancel,
...
extern void pullfmt(Menuhdr* mp, int subspace, int charw, int charh, Rectangle* screen_rect);
