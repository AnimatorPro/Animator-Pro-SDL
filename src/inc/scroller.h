...
typedef struct name_scroller {
	...
	void (*draw_1_cel)(Button* list_sel, struct raster* rast, int x, int y, Names* entry);
	...
	void (*high_1_cel)(Button* list_sel, struct raster* rast, int x, int y, Names* entry,
					   bool highlite);
	...
	void (*feel_1_cel)(Button* list_sel, struct raster* rast, int x, int y, Names* entry, int why);
...
extern Errcode build_qscroller(char* result, Wscreen* s, Menuhdr** pmh, char* hailing, Names* items,
							   SHORT lines, char* ok, char* cancel, SHORT* ipos);
...
extern Errcode go_driver_scroller(char* title, Names* list, Names* initial_entry,
