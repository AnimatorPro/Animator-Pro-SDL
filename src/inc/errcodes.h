...
#define Err_no_render_target \
	(VDERR - 23)                    /* Unable to make a matching texture for the render target */
...
#define FONTERR -300
#ifdef FONTERR
#define Err_unknown_font_type                        \
	(FONTERR - 0) /* most likely it's a .fnt suffix, \
...
#define Err_poco_ffi_no_map_insert (POCOERR_FFI - 3) /* Could not insert entry into function map \
													  */
#define Err_poco_ffi_func_not_found \
	(POCOERR_FFI - 4) /* Tried to call a C function that does not exist */
#define Err_poco_ffi_variadic_overflow \
	(POCOERR_FFI - 5) /* Variadic parameter count is greater than FFI_MAX_ARGS */
...
#ifdef POCOLIBERR
#define Err_poco_lib_not_found                                                                 \
	(POCOLIBERR - 0) /* Library file not found in search paths - searched in script directory, \
						current working directory, and executable directory */
#define Err_poco_lib_load_failed                                                             \
	(POCOLIBERR - 1) /* Failed to load shared library - dlopen()/LoadLibrary() failed, check \
						poco_dlerror() for details */
#define Err_poco_lib_no_entry                                                                     \
	(POCOLIBERR - 2) /* Library loaded but entry point poco_rexlib_get not found - library may be \
						missing symbol export */
#define Err_poco_lib_invalid                                                                 \
	(POCOLIBERR - 3) /* Entry point returned NULL or invalid structure - library entry point \
						function returned NULL pointer */
#define Err_poco_lib_version \
	(POCOLIBERR -            \
	 4) /* Library version mismatch - exe->hdr.version does not match POCOREX_VERSION */
#define Err_poco_lib_empty \
	(POCOLIBERR - 5) /* Library has no functions - lib.lib == NULL or lib.count == 0 */
/* end POCOLIBERR */
#endif
...
#define Err_too_many_args                                                  \
	(SMUERR - 13)                        /* too many replaceable arguments \
										  * in a format string */
#define Err_n_args_invalid (SMUERR - 14) /* formats can't include %n types */
#define Err_bad_argnum                                  \
	(SMUERR - 15) /* ![??] argument number decriptor is \
				   * invalid */
#define Err_format_invalid                              \
	(SMUERR - 16) /* a formattter '%%' type argument is \
				   * invalid */
/* end SMUERR */
#endif
