...
#define DONT_SEEK_ROOT                                \
	-1 /* argument for root offest of init_chunkparse \
		* that asks to start parse at current file    \
		* position */

#define DONT_READ_ROOT                                 \
	-1 /* argument for root type of init_chunkparse    \
...
#define ROOT_CHUNK_TYPE                               \
	-1 /* chunk type set by get_next_chunk() for root \
		* chunk if not DONT_READ_ROOT in init */

/* these items will read or copy the ammount of data specified by the
 * fchunk.size field. */

extern void init_chunkparse(Chunkparse_data* pd, XFILE* xf, LONG root_type, LONG root_oset,
