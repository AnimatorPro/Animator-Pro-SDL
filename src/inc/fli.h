...
\ /* depth of a pixel */
	SHORT flags;
LONG speed;
\ /* speed in milisec 1/1000 sec per frame */
	USHORT unused;
...
extern const Flicomp pj_fli_comp_ani; /* high res animator variable resolution fli */
...
extern Errcode pj_fli_read_head(const char* title, Fli_head* flih, XFILE** pxf,
								enum XReadWriteMode mode);
...
extern Errcode pj_fli_open(char* path, Flifile* flif, enum XReadWriteMode mode);

extern Errcode squawk_open_flifile(char* path, Flifile* flif, enum XReadWriteMode mode);
...
extern Errcode pj_fli_add_frame1(char* name, Flifile* flif, void* cbuf, struct rcel* frame1);

Errcode pj_fli_add_next(char* name, Flifile* flif, void* cbuf, struct rcel* last_screen,
						struct rcel* this_screen);
Errcode pj_fli_add_ring(char* name, Flifile* flif, void* cbuf, struct rcel* last_screen,
						struct rcel* first_screen);
...
extern LONG pj_fli_comp_rect(void* comp_buf, struct rcel* last_screen, struct rcel* this_screen,
							 struct rectangle* rect, bool do_colors, SHORT frame_type,
							 Flicomp comp_type);

extern LONG pj_fli_comp_cel(void* comp_buf, struct rcel* last_screen, struct rcel* this_screen,
							SHORT frame_type, Flicomp comp_type);

extern LONG pj_fli_comp_frame1(void* cbuf, struct rcel* this_screen, Flicomp comp_type);

extern Errcode pj_write_one_frame_fli(char* name, Flifile* flif, struct rcel* screen);
...
extern void pj_fli_uncomp_rect(struct rcel* f, Fli_frame* frame, struct rectangle* rect,
							   int colors);

extern void pj_fli_uncomp_frame(struct rcel* screen, Fli_frame* frame, int colors);
...
extern Errcode pj_i_read_uncomp1(char* fname, Flifile* flif, struct rcel* fscreen, Fli_frame* ff,
								 bool colors);

extern Errcode pj_fli_read_uncomp(char* name, Flifile* flif, struct rcel* fscreen, Fli_frame* ff,
								  int colors);

extern Errcode pj_fli_read_first(char* name, Flifile* flif, struct rcel* fscreen, bool colors);

extern Errcode pj_fli_read_next(char* name, Flifile* flif, struct rcel* fscreen, bool colors);
...
extern void pj_get_stampsize(SHORT maxw, SHORT maxh, SHORT sw, SHORT sh, SHORT* pw, SHORT* ph);

extern LONG pj_build_rect_pstamp(struct rcel* screen, void* cbuf, SHORT x, SHORT y, USHORT width,
								 USHORT height);
...
/* fli.c */
extern Errcode gb_unfli(struct rcel* screen, int ix, int wait, Fli_frame* frame);
...
extern Errcode gb_fli_tseek(struct rcel* screen, int cur_ix, int new_ix, struct fli_frame* fbuf);

extern Errcode gb_fli_abs_tseek(struct rcel* screen, int new_ix, struct fli_frame* fbuf);
