...
typedef struct vfont {
	...
	/* draw text function */
	Errcode (*gftext)(RASType* r, struct vfont* f, unsigned char* s, int x, int y, Pixel col,
					  Text_mode tmode, Pixel bcol);
...
extern Errcode font_req(char* font_path, char* wildcard, SHORT* ptop_name, SHORT* point_size,
...
extern void blit_for_mode(int tmode, UBYTE* mbytes, Coor mbpr, Coor mx, Coor my, RASType* r,
						  Coor rx, Coor ry, Ucoor w, Ucoor h, Pixel oncol, Pixel offcol);

Errcode gftext(RASType* rast, Vfont* f, char* s, int x, int y, Pixel color, Text_mode mode,
			   BCOLvar);
...
extern void bitmask_to_alpha_channel(Pixel* dest, int shrinker, UBYTE* bitplane, int w, int h,
									 int bpr, int y_fraction);
