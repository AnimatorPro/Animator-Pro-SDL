...
#define MEMBER(struc, field) ((struc*)NULL)->field
...
#else
#define OFFSET(struc, field) (USHORT)((ULONG)((PTR) & MEMBER(struc, field) - (PTR)NULL))
#endif

/* offset to first byte after a field */

#define POSTOSET(struc, field) (OFFSET(struc, field) + sizeof(MEMBER(struc, field)))

/* returns pointer to structure given struct name, field name, and
 * pointer to field */

#define TOSTRUCT(struc, field, fieldptr) ((struc*)((PTR)(fieldptr) - OFFSET(struc, field)))
...
#define VGETVAL(pt, typ) \
	*((typ*)pt);         \
	(pt) = OPTR(pt, sizeof(typ))
