...
/* tweenhi.c */
extern void a_wireframe_tween(Tween_state* tween, int frames, int speed, Pixel dit_color,
							  Pixel dash_color, bool closed, int play_mode);
...
extern Errcode tween_add_a_link(Tween_state* ts, int startix, int endix, bool closed,
								Tween_link** pnewl);

extern Errcode calc_path_pos(Poly* poly, Short_xyz* delta_array, int scale, bool closed);

extern void trash_tw_list(Tw_tlist* twl);
extern Errcode ts_to_tw_list(Tween_state* vin, bool closed, Tw_tlist* tout);

extern void calc_tween_points(Tw_tlist* tl, bool closed, int scale, Short_xyz** ppts, int* pcount);
