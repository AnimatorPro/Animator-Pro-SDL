...
extern int just_charstart(Vfont* font, int x, int lwidth, char* linebuf, int charpos, int just);

int justify_line(Raster* r, Vfont* font, char* linebuf, int x, int y, int w, Pixel color,
				 Text_mode tmode, Pixel color2, int just, SHORT* cposx, int cpos_idx);

void wwtext(Raster* screen, Vfont* f, char* s, int x, int y, int w, int h, int skiplines,
			int justify, Pixel color, Text_mode tmode, Pixel color2);
