...
static Pixel smooth_dot(Ink* inky, SHORT x, SHORT y)
{
	...
	aid->true_blend(GET_DOT(aid->undo, x, y) + aid->undo->cmap->ctab,
					GET_DOT(aid->undo, x + 1, y) + aid->undo->cmap->ctab, inky->strength, &result);
...
static void smooth_hline(const Ink* inky, SHORT x0, const SHORT y, SHORT width)
{
	...
	void (*blend)(Rgb3* dest, Rgb3* source, UBYTE percent, Rgb3* result) = aid->true_blend;
...
RootInk smooth_ink_opt = {
	INKINIT(NONEXT, RL_KEYTEXT("smooth_n") "Smooth", INK_OPT, 0, RL_KEYTEXT("smooth_help"),
			NO_SUBOPTS, smooth_dot, smooth_hline, 50, FALSE, NO_MC, NO_FC, (INK_NEEDS_UNDO), ),
	init_smooth,
};
