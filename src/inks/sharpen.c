...
static Pixel sharpen_dot(Ink* inky, SHORT x, SHORT y)
{
	...
	if (x > 0 && x < undo->width - 1 && y > 0 && y < undo->height - 1) {
		...
		full.g = gtot;
		full.b = btot;
		aid->true_blend(ctab + local_pix[4], /* get RGB value for middle pixel */
						&full, inky->strength, &result);
...
RootInk sharpen_ink_opt = {
	INKINIT(NONEXT, RL_KEYTEXT("sharpen_n") "Sharpen", INK_OPT, 0,
			RL_KEYTEXT("sharpen_help") "Image processing technique that helps focus "
									   "a blurry picture.",
			NO_SUBOPTS, sharpen_dot, NULL, 50, FALSE, NO_MC, NO_FC,
