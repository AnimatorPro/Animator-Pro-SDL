...
static Pixel merge_dot(Ink* inky, SHORT x, SHORT y)
{
	...
	aid->true_blend(CGET_DOT(aid->undo, x, y) + aid->undo->cmap->ctab,
					CGET_DOT(aid->alt, x, y) + aid->alt->cmap->ctab, inky->strength, &result);
...
RootInk merge_ink_opt = {
	INKINIT(NONEXT, RL_KEYTEXT("merge_n") "Merge", INK_OPT, 0, RL_KEYTEXT("merge_help"), NO_SUBOPTS,
			merge_dot, NULL, 50, FALSE, NO_MC, NO_FC, (INK_NEEDS_UNDO | INK_NEEDS_ALT), ),
	init_merge,
};
