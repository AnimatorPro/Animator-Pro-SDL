...
static Button tsg_rs3_sel = MB_INIT1(NONEXT, NOCHILD, 12, 11, 234, 149, &cright, wbg_ncorner_image,
									 inc_tseg, NOOPT, &vs.stop_seg, -1, NOKEY, 0);
static Button tsg_rs2_sel =
	MB_INIT1(&tsg_rs3_sel, NOCHILD, 27, 11, 208, 149, &vs.stop_seg, ncorner_short_plus1,
			 stuff_with_cur_ix, NOOPT, &vs.stop_seg, -1, NOKEY, 0);
static Button tsg_rs1_sel =
	MB_INIT1(&tsg_rs2_sel, NOCHILD, 12, 11, 197, 149, &cleft, wbg_ncorner_image, dec_tseg, NOOPT,
			 &vs.stop_seg, -1, NOKEY, 0);
static Button tsg_tseg_sel = MB_INIT1(&tsg_rs1_sel, NOCHILD, 150, 11, 48, 149, NOTEXT, see_tseg,
									  feel_tseg, rfeel_tseg, &vs.start_seg, 0, NOKEY, 0);
static Button tsg_ls3_sel =
	MB_INIT1(&tsg_tseg_sel, NOCHILD, 12, 11, 37, 149, &cright, wbg_ncorner_image, inc_tseg, NOOPT,
			 &vs.start_seg, 0, NOKEY, 0);
static Button tsg_ls2_sel =
	MB_INIT1(&tsg_ls3_sel, NOCHILD, 27, 11, 11, 149, &vs.start_seg, ncorner_short_plus1,
			 stuff_with_cur_ix, NOOPT, &vs.start_seg, 0, NOKEY, 0);
Button tseg_slider_sel = MB_INIT1(&tsg_ls2_sel, NOCHILD, 12, 11, 0, 149, &cleft, wbg_ncorner_image,
								  dec_tseg, NOOPT, &vs.start_seg, 0, NOKEY, 0);


static Button ts_wid_sel = MB_INIT1(NONEXT, NOCHILD, 26 + 1, 11, 285, 149, 0, see_twidth, NOFEEL,
									NOOPT, NOGROUP, 0, NOKEY, 0);
Button tseg_a_sel = MB_INIT1(
	&ts_wid_sel, NOCHILD, 11, 9, 270, 150, NODATA, /* "A", */ /* loaded by init_menu_parts() */
	dcorner_text, change_time_mode, NOOPT, &vs.time_mode, 2, NOKEY, MB_GHILITE);
Button tseg_s_sel = MB_INIT1(
	&tseg_a_sel, NOCHILD, 11, 9, 260, 150, NODATA, /* "S", */ /* loaded by init_menu_parts() */
	dcorner_text, change_time_mode, NOOPT, &vs.time_mode, 1, NOKEY, MB_GHILITE);
Button tseg_f_sel = MB_INIT1(
	&tseg_s_sel, NOCHILD, 11, 9, 250, 150, NODATA, /* "F", */ /* loaded by init_menu_parts() */
	dcorner_text, change_time_mode, NOOPT, &vs.time_mode, 0, NOKEY, MB_GHILITE);

Button tseg_group_sel = MB_INIT1(&tseg_f_sel, &tseg_slider_sel, 320, 11, 0, 149, NOTEXT,
...
static void see_tseg(Button* b)
{
	...
	arrow_start = astart = rscale_by(nwidth, seg_lower, flix.hdr.frame_count) + nx;
