...
static UBYTE circ3_mask[] = {
	BITS8(_, _, X, _, _, _, _, _), BITS8(_, X, X, X, _, _, _, _), BITS8(X, X, X, X, X, _, _, _),
	BITS8(_, X, X, X, _, _, _, _), BITS8(_, _, X, _, _, _, _, _),
};

static UBYTE circ4_mask[] = {
	BITS8(_, _, X, X, _, _, _, _), BITS8(_, X, X, X, X, _, _, _), BITS8(X, X, X, X, X, X, _, _),
	BITS8(X, X, X, X, X, X, _, _), BITS8(_, X, X, X, X, _, _, _), BITS8(_, _, X, X, _, _, _, _),
};

static UBYTE circ5_mask[] = {
	BITS8(_, _, X, X, X, _, _, _), BITS8(_, X, X, X, X, X, _, _), BITS8(X, X, X, X, X, X, X, _),
	BITS8(X, X, X, X, X, X, X, _), BITS8(X, X, X, X, X, X, X, _), BITS8(_, X, X, X, X, X, _, _),
	BITS8(_, _, X, X, X, _, _, _),
};

static UBYTE circ6_mask[] = {
	BITS8(_, _, _, X, X, _, _, _), BITS8(_, X, X, X, X, X, X, _), BITS8(_, X, X, X, X, X, X, _),
	BITS8(X, X, X, X, X, X, X, X), BITS8(X, X, X, X, X, X, X, X), BITS8(_, X, X, X, X, X, X, _),
	BITS8(_, X, X, X, X, X, X, _), BITS8(_, _, _, X, X, _, _, _),
};
...
{
	pj_blitrect(src, ubrush->x = x - rb->cent.x, ubrush->y = y - rb->cent.y, ubrush, 0, 0,
				rb->width, rb->height);
}
...
Errcode init_brushes(void)
{
	...
	if ((err = valloc_bytemap((Raster**)&thebrush.rast, BRUSH_MAX_WIDTH, BRUSH_MAX_HEIGHT)) <
		Success) {
		return (err);
	}
	if ((err = valloc_bytemap((Raster**)&ubrush, BRUSH_MAX_WIDTH, BRUSH_MAX_HEIGHT)) < Success) {
		return (err);
	}
...
Errcode set_circle_brush(int size)
{
	...
	if (rb->type == CIRCLE_BRUSH && rb->b.circ.size == size) {
		return (Success);
	}
...
static Errcode set_square_brush(int size)
{
	...
	if (rb->type == SQUARE_BRUSH && rb->b.square.size == size) {
		return (Success);
	}
...
void draw_line_brush(void* rast, Short_xy* cent, Pixel color, int size, int angle)
{
	Short_xy pt;

	get_line_end(&pt, size, angle);

	line(rast, color, cent->x - pt.x, cent->y - pt.y, cent->x + pt.x, cent->y + pt.y);
}
...
/* size of 0 is a dot and 1 is 2 dots on up */
{
	...
	if (rb->type == LINE_BRUSH && rb->b.line.size == size && rb->b.line.angle == degrees) {
		return (Success);
	}
	...
	line(rb->rast, 1, rb->cent.x - pt.x, rb->cent.y - pt.y, rb->cent.x + pt.x, rb->cent.y + pt.y);
...
void blit_brush(Rbrush* rb, void* dest, Coor x, Coor y)
{
	...
	procblit(rb->rast, 0, 0, dest, x - rb->cent.x, y - rb->cent.y, rb->width, rb->height,
			 tbli_xlatline, &rb->tcxl);
}

void zoom_blit_brush(Rbrush* rb, Coor x, Coor y)
{
	check_rbrush_xlat(rb);
	zoom_txlatblit((Raster*)rb->rast, 0, 0, rb->width, rb->height, x - rb->cent.x, y - rb->cent.y,
				   &rb->tcxl);
}
...
void see_pen(Button* b)
{
	...
	if (vl.brush->width >= b->width || vl.brush->height >= b->height) {
		mc_frame(b, MC_BLACK);
	}
}
