...
Errcode gel_tool_loop(Errcode (*get_posp)(Pos_p* pp, void* idata, SHORT mode), void* idata)
...
/* for EFUNC non Success is end of input, its return is returned by this */
{
	...
	for (;;) {
		...
		for (i = 0; i < 4; i++) {
			gel_factor = gft[i];
			gel_thash = th[i];
			if ((err = doval(rp.x, rp.y, (pwt[i] << 1) + 1, vb.pencel->aspect_dx,
							 vb.pencel->aspect_dy, NULL, NULL, render_hline, vb.pencel, true)) <
				Success) {
				goto error;
			}
		}
...
void see_gel_brush(Button* b)
{
	...
	white_block(b);
	for (i = pw * 4; i > 0; i -= pw) {
		doval(b->width >> 1, b->height >> 1, i, cb.aspect_dx, cb.aspect_dy, seegb_dot, &cb, NULL,
			  NULL, false);
	}
}
