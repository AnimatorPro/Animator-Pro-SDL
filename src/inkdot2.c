...
static Pixel rclosest_color(Thash* th, const Rgb3* oldc, const SHORT underc, const SHORT dither)
{
	if (dither) {
...
{
	if (vl.alt_cel) {
		return (rclosest_color(inky->inkdata, vl.alt_cel->cmap->ctab, pj_get_dot(vl.alt_cel, x, y),
							   inky->dither));
	} else {
		return (vs.ccolor);
	}
...
Pixel out_dot(const Ink* inky, SHORT x, SHORT y)
{
	...
	if (endc == pj_get_dot(undof, x + 1, y) && endc == pj_get_dot(undof, x - 1, y) &&
		endc == pj_get_dot(undof, x, y - 1) && endc == pj_get_dot(undof, x, y + 1)) {
		return (vs.inks[0]);
	} else {
		return (endc);
	}
...
Pixel des_dot(const Ink* inky, SHORT x, SHORT y)
{
	...
	desaturate(vb.pencel->cmap->ctab + pj_get_dot(undof, x, y), &rgb, inky->strength);
	return (bclosest_col(&rgb, COLORS, inky->dither));
}
...
Pixel swe_dot(const Ink* inky, SHORT x, SHORT y)
{
	...
	if (endc != color) {
		if (endc == pj_get_dot(undof, x, y + 1) && endc == pj_get_dot(undof, x + 1, y) &&
...
static char chtab[32] = {
	0x0,  0x8,  0x0,  0xcc, 0x2e, 0x8, 0xea, 0xc8, 0x7f, 0x8, 0x40, 0x0, 0x3a, 0x8, 0x0, 0x0,
	0x74, 0xfc, 0x40, 0xc8, 0x7f, 0x4, 0x46, 0x80, 0x76, 0x0, 0x40, 0x0, 0x24, 0x0, 0x0, 0x0,
};
