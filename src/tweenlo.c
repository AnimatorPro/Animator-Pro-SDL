...
/* reverse start and end of each "tween_link" node in list */
{
	...
	for (link = (Tween_link*)(list->head); NULL != (next = (Tween_link*)(link->node.next));
		 link = next) {
		swap = link->start;
		link->start = link->end;
		link->end = swap;
...
Errcode tween_add_a_link(Tween_state* ts, int startix, int endix, bool closed, Tween_link** pnewl)
 ...
 */
{
	...
	if (link_count == 0) {
		if (closed) {
			ts->p0.clipped_list = slist_el((Slnode*)ts->p0.clipped_list, startix);
			ts->p1.clipped_list = slist_el((Slnode*)ts->p1.clipped_list, endix);
			if ((err = add_link(&ts->links, 0, 0, pnewl)) < Success) {
				goto OUT;
			}
			goto OUT;
		} else {
	...
	if ((err = add_link(&ts->links, startix, endix, pnewl)) < Success) {
		goto OUT;
	}
OUT:
	return err;
...
static void sample_vertex(Short_xyz* vtx, int vcount, int vsign, Short_xyz* delta_array, int scale)
...
Errcode calc_path_pos(Poly* poly, Short_xyz* delta_array, int scale, bool closed)
...
static Errcode new_tw_thread(Tw_tlist* tout, int start0, int end0, int start1, int end1)
{
	Tw_thread* newt;
	Errcode err;
	int max;
...
/* Calculate the endpoints of each thread within the output tween array */
{
	...
	for (link = (Tw_thread*)(tls->list.head); (next = (Tw_thread*)(link->node.next)) != NULL;
		 link = next) {
 ...
 * our tween calculations and some other stuff. */
{
	...
	if (tct <= 1) {
		...
		i = tct - 1;
		while (--i >= 0) {
			if ((err = new_tw_thread(tout, link->start, link->end, next->start, next->end)) <
				Success) {
				goto ERROR;
			}
			link = next;
		...
		if ((err = new_tw_thread(tout, link->start, link->end, sct, dct)) < Success) {
			goto ERROR;
		}
	}
	if (!closed) {
...
static Short_xyz* calc_tw_thread(Short_xyz* t0, int t0_count, int t0_inc, Short_xyz* t1,
								 int t1_count, int t1_inc, Short_xyz* inter, int icount, int scale)
...
static void calc_tw_list(Tw_tlist* tls, bool closed, int scale)
{
	...
	for (link = (Tw_thread*)(tls->list.head); (next = (Tw_thread*)(link->node.next)) != NULL;
		 link = next) {
		calc_tw_thread(link->source, link->scount + 1, 1, link->dest, link->dcount + 1, link->dinc,
					   link->inter, link->icount, scale);
	}
	if (!closed) {
...
void calc_tween_points(Tw_tlist* tl, bool closed, int scale, Short_xyz** ppts, int* pcount)
