...
static UBYTE render_with_mask, render_fast, render_real_fast, render_write_mask, render_read_mask,
	render_hlineable;
static int ink_type;
static Pixel (*ink_dot)(const Ink* inky, const SHORT x, const SHORT y);

Aa_ink_data ink_aid = {
	0,       0,       NULL,       NULL,     NULL,           NULL,           COLORS,       RGB_MAX,
	RGB_MAX, RGB_MAX, true_blend, closestc, make_ink_bhash, free_ink_bhash, bclosest_col,
};
...
void hsp_hline(const Ink* inky, SHORT x0, SHORT y, SHORT width)
{
	...
	if (inky->dither) {
		...
		while (--i >= 0) {
			colo = p / q;
			if ((SHORT)((p) % (q) * 256L / (q)) > ((((x) & (y)) * 83 + fac2 + fac3) & 255)) {
				colo++;
			}
			*spt++ = bun[colo];
			fac2 += 63;
...
/* This one is so slow don't bother to have a hline routine... */
{
	...
	return (
		dfrom_range(calc_distance(x, y, vl.rgc.x, vl.rgc.y) % vl.rgr, vl.rgr, x, y, inky->dither));
}
...
Pixel soft_dot(const Ink* inky, SHORT x, SHORT y)
{
	...
	average_colors(&rgb, neighbors, get_neighbors(neighbors, x, y), vb.pencel->cmap);
...
static Pixel tvdot(const Ink* inky, const SHORT x, const SHORT y, Rcel* source)
{
	...
	if (inky->dither) {
		true_blend(vb.pencel->cmap->ctab + underc, &tcolor_src, inky->strength, &rgb);
...
/* This one has basically an in-line tclosest_color for speed. */
{
	...
	if (inky->dither) {
		while (--i >= 0) {
			true_blend(ctab + *spt++, &tcolor_src, inky->strength, &rgb);
...
static Pixel tccolor(const SHORT dcolor, const SHORT tcolor, const SHORT tintper, Thash* t)
{
	Rgb3 rgb;
	Rgb3* cm;
...
static Pixel gel_dot(SHORT x, SHORT y, Pixel color, SHORT dither)
{
	...
	if (ink_type == opq_INKID) {
		return (tccolor(pj_get_dot(vb.pencel, x, y), color, gel_factor, gel_thash));
	} else {
...
void render_dot(SHORT x, SHORT y, void* data)
{
	...
	if (x < rdta.cr.x || y < rdta.cr.y || x >= rdta.cr.MaxX || y >= rdta.cr.MaxY) {
		return;
	}
