...
{
	if ((vl.zoomwndo) && !(vl.zoomwndo->flags & WNDO_HIDDEN)) {
		if (!bothwins) {
			return (DOZOOM);
		}
		...
		if (CODEOBSCURES == clipcode_crects((Cliprect*)&(vl.zoomwndo->CRECTSTART),
...
/* "dotout" routine to erase from undo screen */
{
	...
	if (((USHORT)x) < vb.pencel->width && ((USHORT)y) < vb.pencel->height) {
		(*(mh->putdot))((Raster*)vb.pencel, pj_get_dot((Raster*)undof, x, y), x, y);
	}
}
 ...
 */
Errcode rubba_vertex(Short_xy* p0, Short_xy* p1, Short_xy* p2, void (*dispfunc)(Short_xy* v),
					 Pixel color)
{
	Marqihdr mh;
	Vecdat vd;
	...
	if ((vd.save = pj_malloc((vb.pencel->width + vb.pencel->height) * vd.count)) == NULL) {
	...
	anim_wait_input(KEYHIT | MBRIGHT | MBPEN, MMOVE, mh.waitcount, anim_rubpoly, &mh);
...
Errcode get_rub_vertex(Short_xy* p0, Short_xy* p1, /* the moved vertex */
...
{
	top_textf("!%3d%3d%3d%3d%3d%3d%3ld%3d", rub_line_str, v[0].x, v[0].y,
			  intabs(v[0].x - v[1].x) + 1, intabs(v[0].y - v[1].y) + 1, v[1].x, v[1].y,
...
Errcode get_rub_axis(Short_xy* ends, Pixel oncol, Pixel offcol)
{
	...
	anim_wait_input(KEYHIT | MBRIGHT | MBPEN, MMOVE, rd.mh.waitcount, anim_axis, &rd);
...
/* increment mod delta move, clip, and Draw frame. */
{
	...
	if (JSTHIT(MMOVE) || !(rd->saved)) {
		...
		if (rd->clip.width) {
			bclip_rect((Rectangle*)&(newrect.RECTSTART), (Rectangle*)&(rd->clip.RECTSTART));
		}
 ...
 * penwndo or zoom window */
{
	...
	anim_wait_input(KEYHIT | MBRIGHT | MBPEN, MMOVE, mh.waitcount, anim_moverect, &mh);
...
/* increment mod delta size, clip, and Draw frame. */
{
	...
	if (JSTHIT(MMOVE) || !(rd->saved)) {
		frame_tocrect(rd->x0, rd->y0, icb.mx, icb.my, (Cliprect*)&(newrect.CRECTSTART));

		and_cliprects((Cliprect*)&(rd->clip.CRECTSTART), (Cliprect*)&(newrect.CRECTSTART),
					  (Cliprect*)&(newrect.CRECTSTART));

		if (!(newrect.x == rd->fr.x && newrect.y == rd->fr.y && newrect.MaxX == rd->fr.MaxX &&
			...
			!(rd->saved)) /* will force first time */
		{
			...
			top_textf("!%3d%3d%3d%3d%3d%3d", rub_rect_str, rd->x0, rd->y0,
					  newrect.width + rd->extra, newrect.height + rd->extra, clipx, clipy);
 ...
 * penwndo or zoom window */
{
	...
	anim_wait_input(KEYHIT | MBRIGHT | MBPEN, MMOVE, mh.waitcount, anim_rubrect, &mh);
...
void savedraw_circle(Marqi_circdat* cd, Short_xy* cent, SHORT d)
{
	...
	doval(cent->x, cent->y, d, vb.pencel->aspect_dx, vb.pencel->aspect_dy, savedraw_marqidot,
		  &cd->mh, NULL, NULL, false);
	cd->saved = 1;
}
...
void restore_circle(Marqi_circdat* cd, Short_xy* cent, SHORT d)
{
	...
	cd->mh.dotbuf = cd->save;
	doval(cent->x, cent->y, d, vb.pencel->aspect_dx, vb.pencel->aspect_dy, restore_marqidot,
		  &cd->mh, NULL, NULL, false);
}
...
static int anim_rubcirc(Marqi_circdat* cd)
{
	...
	if (JSTHIT(MMOVE) || !(cd->saved)) {
		...
		if (d != cd->d || cd->cent.x != cent.x || cd->cent.y != cent.y || !(cd->saved)) {
			if (!(cd->movecent)) {
	...
	doval(cd->cent.x, cd->cent.y, cd->d, vb.pencel->aspect_dx, vb.pencel->aspect_dy, cd->mh.pdot,
		  &(cd->mh), NULL, NULL, false);
 ...
 * move the center rather than the radius */
{
	...
	anim_wait_input(KEYHIT | MBRIGHT | MBPEN, MMOVE, cd.mh.waitcount, anim_rubcirc, &cd);
...
void msome_vector(Short_xy* pts, int count, dotout_func dotout, void* dotdat, int open, int pt_size)
