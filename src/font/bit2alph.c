...
void bitmask_to_alpha_channel(Pixel* dest, int shrinker, UBYTE* bitplane, int w, int h, int bpr,
							  int y_fraction)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	for (y = 0; y < h;) {
		...
		for (x = 0; x < w; x += shrinker) {
			...
			if (endx <= w) {            /* if we're not gonna go off the edge */
				mask = pmasktab[x & 7]; /* use the normal mask, else use a	 */
			} else {                    /* mask that will clip off-edge bits.*/
