...
static void type1_token_init(Type1_token* tok, XFILE* file, int (*source)(XFILE* f))
{
	clear_struct(tok);
	tok->file = file;
	tok->source = source;
...
static void type1_parse_custom_encoding(Type1_font* tcd, Type1_token* tok, int size)
 ...
 * or it will be an array of name/number pairs. */
{
	...
	type1_get_token(tok);
	if (tok->type == TTT_NAME && ((strcmp(tok->string, "StandardEncoding") == 0) ||
	...
	if (tok->type == TTT_NUMBER) {
		...
		type1_get_token(tok);
		if (tok->type == TTT_NAME && strcmp(tok->string, "array") == 0) {
   ...
   presence of a carriage return. */
{
	...
	if (cs[0] == ' ' || cs[0] == '\t' ||
		...
		byte_in = hex_byte_in;
		for (i = 0; i < 4; i++) {
			if (!((cs[i] >= '0' && cs[0] <= '0') || (cs[i] >= 'A' && cs[0] <= 'F') ||
				  (cs[i] >= 'a' && cs[0] <= 'f'))) {
				byte_in = xfgetc;
				break;
			}
...
static unsigned char* alloc_and_read_RD_string(Type1_font* tcd, Type1_token* tok, int binary_size)
{
	unsigned char* buf;
...
static void type1_get_char_strings(Type1_font* tcd, Type1_token* tok)
{
	...
	tcd->letter_names = letter_names = loader_alloc(tcd, letter_count * sizeof(*letter_names));
	tcd->letter_defs = letter_defs = loader_alloc(tcd, letter_count * sizeof(*letter_defs));
	for (;;) {
		type1_get_token(tok);
		if (tok->type == TTT_EOF) {
			...
			if (tok->type == TTT_NUMBER) { /* bocklin.pfb has a character with a missing name.
			...
			letter_defs[letter_ix] = alloc_and_read_RD_string(tcd, tok, binary_size);
...
static Errcode read_font(XFILE* fp, Type1_font** ptcd)
{
	...
	if ((tcd = pj_zalloc(sizeof(*tcd))) == NULL) {
		...
		if (Success != (err = setjmp(type1_load_errhandler))) { /* Got here via longjmp. */
			tcd_freez(&tcd);
		} else {
			rtype1(tcd, fp);
		}
...
#define AddPoint(output, x, y)             \
	{                                      \
		output->shape_point(output, x, y); \
		++pcount;                          \
	}
#define ClosePath(output)                        \
	{                                            \
		if (pcount) output->shape_close(output); \
		pcount = 0;                              \
	}
...
static Errcode draw_bezier(Type1_output* output, long x0, long y0, long x1, long y1, long x2,
						   long y2, long x3, long y3, int n)
/*----------------------------------------------------------------------*
 *	Draw a bezier curve.
 *----------------------------------------------------------------------*/
{
 ...
 *----------------------------------------------------------------------*/
{
	...
	switch (procno) {
		case 0: /* Flex */
			draw_bezier(output, flexarg[0][X], flexarg[0][Y], flexarg[2][X], flexarg[2][Y],
						flexarg[3][X], flexarg[3][Y], flexarg[4][X], flexarg[4][Y], bnum);
			draw_bezier(output, flexarg[4][X], flexarg[4][Y], flexarg[5][X], flexarg[5][Y],
						flexarg[6][X], flexarg[6][Y], flexarg[7][X], flexarg[7][Y], bnum);
...
static Errcode type1_exchars(Type1_font* tcd, int* pwidth, Type1_output* output, int bezier_points,
							 unsigned char* cp, int ix, int iy)
...
#ifdef DEBUG
			ddlog("%s %d %d %d %d %d\n", type1_command_string(c), S0, S1, S2, S3, S4);
#endif /* DEBUG */
			switch (c) {
				...
				case Seac: /* 12-6:  Standard encoding accented char */
				{
					...
					if (base_ix < BYTE_MAX) {
						if ((base = tcd->ascii_defs[base_ix]) != NULL) {
							if ((err = type1_exchars(tcd, &lwidth, output, bezier_points, base, ix,
													 iy)) < Success) {
								goto ERROR;
							}
						}
					...
					if (accent_ix < BYTE_MAX) {
						if ((accent = tcd->ascii_defs[accent_ix]) != NULL) {
							if ((err = type1_exchars(tcd, &lwidth, output, bezier_points, accent,
													 ix + xoff + fx - asb, iy + fy + yoff)) <
								Success) {
								goto ERROR;
							}
						}
				...
				case Hvcurveto: /* 31:  Horizontal-vertical curve to */
					Sl(4);
					Opath(output);
					draw_bezier(output, curx, cury, curx + S3, cury, curx + S3 + S2, cury + S1,
				...
				case Rrcurveto: /* 8:  Relative curve to */
					Sl(6);
					Opath(output);
					draw_bezier(output, curx, cury, curx + S5, cury + S4, curx + S5 + S3,
								cury + S4 + S2, curx + S5 + S3 + S1, cury + S4 + S2 + S0, bnum);
					Clear();
					break;
				case Vhcurveto: /* 30:  Vertical-horizontal curve to */
					Sl(4);
					Opath(output);
					draw_bezier(output, curx, cury, curx, cury + S3, curx + S2, cury + S3 + S1,
...
static Errcode type1_interp_char(Type1_font* tcd, int* pwidth, Type1_output* output,
								 int bezier_points, unsigned char* cp)
...
static void box_into_bounding_box(Type1_box* dest, Type1_box* new)
/*----------------------------------------------------------------------*
...
static Type1_output bounds_output = {bounds_letter_open, bounds_close,     bounds_add_point,
									 bounds_close,       bounds_add_point, NULL};

static Errcode bounds_interpret(Type1_font* tcd, unsigned char* def, Type1_box* bounds)
/************************************************************************
 *
 ************************************************************************/
{
...
static Type1_bitplane empty_bitplane =
	...
	{&empty_pixel, 1, 1, 1, 0, 0};

static Type1_bitplane* alloc_type1_bitplane(Block_allocator* ba, Type1_box* bounds)
{
	Type1_bitplane* bits;
	int width, height;
	...
	if ((bits->bits = alloc_from_block(ba, height * bits->bpr)) == NULL) {
		return NULL;
	}
	return bits;
}
...
static void type1_bits_outline(Poly* poly, int xoff, int yoff, bool closed, Type1_bitplane* bits)
/* Draw a polygon outline on a bitplane. */
{
	...
	while (--count >= 0) {
		next = this->next;
		pj_cline(this->x - xoff, this->y - yoff, next->x - xoff, next->y - yoff, type1_bits_dot,
				 bits);
		this = next;
	}
}
 ...
 *----------------------------------------------------------------------*/
{
	...
	if ((*(bits_out->pbits) = pbits = alloc_type1_bitplane(bits_out->ba, &bounds)) == NULL) {
		return Err_no_memory;
	}
	...
	err = fill_on_off(bits.bpr, bits.width, bits.height, 0, 0, bits.bits, type1_bits_hline, &bits);
...
static Type1_output fill_output = {fill_letter_open, fill_letter_close, fill_shape_open,
								   fill_shape_close, fill_point,        NULL};
 ...
 ************************************************************************/
{
	return fill_scale_interpret(tcd, def, xoff, yoff, tcd->scale.scalex, tcd->scale.scaley,
								bits_out);
}
...
static Type1_output simple_output = {
	simple_letter_open, simple_letter_close, simple_shape_open, simple_shape_close, simple_point,
};
 ...
 *----------------------------------------------------------------------*/
{
	...
	while (--i >= 0) {
		if (*ascii_defs++ != NULL) {
			if ((char_right_overlap = pbox->xmax * scalex - *pwidth) > font_right_overlap) {
...
static void scale_box(double scalex, double scaley, Type1_box* in, Type1_box* out)
{
 ...
 *----------------------------------------------------------------------*/
{
	...
	free_scale_info(&tcd->scale);
	construct_block_allocator(&tcd->scale.ba, mem_block_size, pj_zalloc, pj_free);
...
static Errcode shrink_bitplane_to_alpha(Block_allocator* ba, Type1_bitplane* source, int shrink,
										Type1_alpha** palpha)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	if ((alpha->channel = alloc_from_block(ba, alpha->height * alpha->bpr)) == NULL) {
		return Err_no_memory;
	}

	/* Now copy in the shrunken bits. */
	bitmask_to_alpha_channel(alpha->channel, shrink, source->bits, source->width, source->height,
							 source->bpr, y_remainder);
...
typedef void (*Talpha_blit)(UBYTE* alpha, int abpr, int x, int y, int w, int h, Rcel* r,
							Pixel oncolor);

#ifdef UNUSED
void mask_alpha_blit(UBYTE* alpha, int abpr, int x, int y, int w, int h, Rcel* r, Pixel oncolor)
/*****************************************************************************
		...
		for (curx = x; curx < endx; ++curx) {
			...
			if (curalpha) {
				if (curalpha == 255) {
					...
					alpha_blend(&ctab[pj_get_dot(r, curx, y)], &ctab[oncolor], &dest, curalpha);
...
static Errcode new_alpha_image(Type1_font* tcd, unsigned char letter, Type1_alpha** palpha)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	output.ba = &local_ba;
	output.pbits = &bits;
	if ((err = fill_scale_interpret(tcd, tcd->ascii_defs[letter], 0, 0, shrink * tcd->scale.scalex,
									shrink * tcd->scale.scaley, &output)) >= Success) {
		bits->y -= shrink * tcd->scale.scaley * tcd->font_bounds.ymin;
		if ((err = shrink_bitplane_to_alpha(&tcd->scale.ba, bits, shrink, &alpha)) >= Success) {
			*palpha = alpha;
		}
	}
...
static Errcode get_alpha_image(Type1_font* tcd, unsigned char letter, Type1_alpha** palpha)
/*****************************************************************************
...
static Errcode soft_gftext(Raster* rast, Vfont* v, register unsigned char* s, int x, int y,
						   Pixel color, Talpha_blit alpha_blit)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	while ((c = *s++) != EOS) {
		...
		if (c == '\t') {
			...
			(*alpha_blit)(alpha->channel, alpha->bpr, x + alpha->x, y + alpha->y, alpha->width,
						  alpha->height, (Rcel*)rast, color);
...
Errcode shrinky_blit(Type1_bitplane* bits, int x, int y, Rcel* rast, Pixel color)
/*
 ...
 */
{
	...
	if ((channel = pj_malloc(nwidth * nheight * sizeof(Pixel))) == NULL) {
		return Err_no_memory;
	}
	bitmask_to_alpha_channel(channel, shrink, bits->bits, width, height, bits->bpr, y_remainder);
	mask_alpha_blit(channel, nwidth, x + bits->x / shrink, y + bits->y / shrink, nwidth, nheight,
					rast, color);
...
static Errcode get_bit_image(Type1_font* tcd, unsigned char letter, Type1_bitplane** pbits)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	ddlog(">>> %c [%#x] <<<\n", letter, letter);
#endif /* DEBUG */
	if ((err = fill_interpret(tcd, tcd->ascii_defs[letter], 0, 0, &output)) >= Success) {
...
static Errcode vfont_gftext(Raster* rast, Vfont* v, register unsigned char* s, int x, int y,
							Pixel color, Text_mode tmode, Pixel bcolor)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	if (tcd->scale.unzag_flag) {
		if (tmode == TM_RENDER) {
			return soft_gftext(rast, v, s, x, y, color, render_mask_alpha_blit);
		}
	}
	while ((c = *s++) != EOS) {
		/* Crude clipping for speed */
		...
		if (c == '\t') {
			...
			blit_for_mode(tmode, bits->bits, bits->bpr, 0, 0, rast, x + bits->x, y + bits->y,
 ...
 *----------------------------------------------------------------------*/
{
	vfont->widest_image = tcd->scale.max_bounds.xmax - tcd->scale.max_bounds.xmin + 1;
	vfont->widest_char = tcd->scale.widest;
	vfont->tab_width = vfont->widest_char * TABEXP;
	vfont->image_height = tcd->scale.max_bounds.ymax - tcd->scale.max_bounds.ymin + 1;
...
static void test_type1(Type1_font* tcd)
{
	...
	for (i = 0; i < BYTE_MAX; ++i) {
		...
		dlog("%c %#x :: %s at %p width %d/%d\n", c, i, letter_name(tcd, i), tcd->ascii_defs[i],
			 tcd->scale.width[i], tcd->letter_width[i]);
	}
	print_unused(tcd);
	for (i = 'a'; i < 'm'; ++i) {
		if ((tcd->ascii_defs[i]) != NULL) {
			width = simple_interpret(tcd, tcd->ascii_defs[i], xoff, 200, (Raster*)vb.pencel);
			dlog("%c %d %d\n", i, xoff, width);
			xoff += width;
		}
	}
	...
	for (i = 0; i < tcd->letter_count; ++i) {
		...
		if (def != NULL && !is_used(tcd, def)) {
			if ((err = fill_interpret(tcd, def, 0, 0, &output)) > Success) {
				pj_set_rast(rast, 0);
				pj_mask2blit(bits->bits, bits->bpr, 0, 0, rast, 100 + bits->x, 100 + bits->y,
...
static Errcode load_vfont(char* title, Vfont* vfont, SHORT height, SHORT unzag_flag)
{
	Type1_font* tcd;
	Errcode err;
...
Font_dev type1_font_dev = {NULL,       "Postscript Type 1", "*.PF?", check_type1_font,
						   load_vfont, TYPE1FONT,           0};
