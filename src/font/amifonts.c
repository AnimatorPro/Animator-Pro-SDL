...
static Errcode ami_gftext(Raster* rast, Vfont* vfont, register unsigned char* s, int x, int y,
						  Pixel color, Text_mode tmode, Pixel bcolor)
{
	...
	while ((c = oem_to_ansi[*s++]) != 0) {
		...
		blit_for_mode(tmode, fcb->image, fcb->head.h2.tf_modulo, fcb->loc[c].startx, 0, rast, x1, y,
...
static Errcode load_ami_font(char* title, Vfont* vfont, SHORT height, SHORT unzag_flag)
{
	XFILE* xf = NULL;
	Errcode err;
	Afcb* fcb;
	...
	vfont->tab_width = vfont->widest_image = vfont->widest_char = fcb->head.h2.tf_xsize;
...
Font_dev ami_font_dev = {NULL, "Amiga Mono", "*.*", check_ami_font, load_ami_font, AMIFONT, 0};
