...
static char* isomap[224] = {
	...
	"asciitilde",   /* 126 */
	"degree",
	/* 127 */ /* AutoCAD-specific */
	"plusminus",
	/* 128 */ /* AutoCAD-specific */
	"emptyset",
	/* 129 */         /* AutoCAD-specific */
	NULL,             /* 130 */
	NULL,             /* 131 */
	NULL,             /* 132 */
	NULL,             /* 133 */
...
#else
	int scurx = curx + cury * skewsin + 0.499, sx = x + y * skewsin + 0.499;

	if (x != curx || y != cury) {
		int dx = sx - scurx, dy = y - cury;
...
#ifndef OLDWAY
				int mx = curx + (x - curx) / 2, my = cury + dy / 2;

				goTo(mx, my);
				goTo(x, y);
#else
				int maxd = max(abs(dx), abs(dy)), sfac = maxd / 64;
...
static void othersubr(procno, nargs, argp) int procno, nargs, argp;
{
	...
	switch (procno) {
		case 0: /* Flex */
			...
			if (noflex) {
				...
				bezier(flexarg[0][X], flexarg[0][Y], flexarg[2][X], flexarg[2][Y], flexarg[3][X],
					   flexarg[3][Y], flexarg[4][X], flexarg[4][Y], bnum);
				bezier(flexarg[4][X], flexarg[4][Y], flexarg[5][X], flexarg[5][Y], flexarg[6][X],
					   flexarg[6][Y], flexarg[7][X], flexarg[7][Y], bnum);
			}
...
static void compshape(cp, fp, subshape) unsigned char* cp;
FILE* fp;
int subshape;
{
	int sub;
		...
		if (c < 32) {
			...
			switch (c) {
				...
				case Seac: /* 12-6:  Standard encoding accented char */
					...
					{
						int asb = S4, adx = S3, ady = S2, bchar = S1, achar = S0;
						...
						if (bc != NULL) {
							...
							fprintf(stderr, "Bogus accent char of %d in seac.\n", achar);
						}
					}
					Emit(6); /* Pop location */
					curx = cury = 0;
		...
		if (strcmp(chardefs[j][0], charmap[i - 32]) == 0) {
			...
			if (codep > 0) {
				...
				if (islower(*charmap[i - 32])) {
					...
					sprintf(chname, "upper_case%c%s", tolower(*charmap[i - 32]),
							charmap[i - 32] + 1);
				}
				fprintf(fp, "*%d,%d,%s\n", c, codep + 1, chname);
				outline[0] = EOS;
	...
	for (i = 32; i < 256; i++) {
		if (charmap[i - 32] != NULL) {
			if (!compout(fp, i, i)) {
				...
				if (!substituted) {
					fprintf(fp, "; Warning: character %d (%s) not defined in font.\n", i,
							charmap[i - 32]);
					if (!compout(fp, '?', i)) {
						compout(fp, ' ', i);
					}
