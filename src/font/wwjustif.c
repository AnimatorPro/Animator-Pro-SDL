...
static int just_fill(Raster* screen, Vfont* font, char* linebuf, int chars, int x, int y,
					 Pixel color, Text_mode tmode, Pixel color2, int xspace, int cxidx)
...
int justify_line(Raster* screen, Vfont* font, char* linebuf, int x, int y, int w, Pixel color,
				 Text_mode tmode, Pixel color2, int just, SHORT* pcharx, int charx_idx)
 ...
 * position of the first-last char */
{
	...
	switch (just) {
		case JUST_FILL: {
			textwid = just_fill(screen, font, linebuf, ccount, x, y, color, tmode, color2, wleft,
								charx_idx);
			if (pcharx) {
