 ...
 */
{
	...
	fprintf(f, "0x%x, %d, \"%s\", %d, %d,\n", fcb->cfont.id, fcb->cfont.size, fcb->cfont.facename,
			fcb->cfont.ADE_lo, fcb->cfont.ADE_hi);
	fprintf(f, "%d, %d, %d, %d, %d,\n", fcb->cfont.top_dist, fcb->cfont.asc_dist,
			fcb->cfont.hlf_dist, fcb->cfont.des_dist, fcb->cfont.bot_dist);
	fprintf(f, "%d, %d, %d, %d,\n", fcb->cfont.wchr_wdt, fcb->cfont.wcel_wdt, fcb->cfont.lft_ofst,
			fcb->cfont.rgt_ofst);
	fprintf(f, "%d, %d, 0x%x, 0x%x,\n", fcb->cfont.thckning, fcb->cfont.undrline,
			fcb->cfont.lghtng_m, fcb->cfont.skewng_m);
 ...
 */
{
	...
	if (offset_size > 300 || offset_size <= 2 || head->wchr_wdt <= 0 || head->wchr_wdt > 1000 ||
		head->wcel_wdt < head->wchr_wdt || head->wcel_wdt > 1000 || head->frm_wdt <= 0 ||
		head->frm_hgt <= 0 || head->frm_hgt > 1000) {
...
static Errcode load_st_font(char* title, Vfont* vfont, SHORT height, SHORT unzag_flag)
/*
...
static Errcode st_gftext(Raster* rast, Vfont* vfont, register unsigned char* s, int x, int y,
 ...
 */
{
	...
	while ((c = *s++) != 0) {
		...
		blit_for_mode(tmode, (UBYTE*)data, wd, sx, 0, rast, x, y, imageWid, ht, color, bcolor);
		switch (font_type) {
			case STPROP:
				x += imageWid;
				break;
...
Font_dev st_font_dev =
	 ...
	 */
	{NULL, "Animator 1.0", "*.FNT", check_st_font, load_st_font, STFONT, 0};
