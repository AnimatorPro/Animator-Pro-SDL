...
static Errcode verify_hpjet_font(XFILE* xf)
{
	Errcode err;
	char buf[3];
...
static Errcode hpjet_read_chars(XFILE* xf, Hfcb* fcb)
{
	Errcode err;
	char buf[3];
 ...
 * in our terms. */
{
	...
	while (--i >= 0) {
		if ((letter = *lets++) != NULL) {
			...
			if ((temp = letter->top_offset = fcb->head.baseline_distance - 1 - letter->top_offset) <
				miny) {
				miny = temp;
			}
...
static Errcode hpjet_gftext(Raster* rast, Vfont* vfont, register unsigned char* s, int x, int y,
							Pixel color, Text_mode tmode, Pixel bcolor)
{
	...
	while ((c = *s++) != 0) {
		...
		if (c == '\t') {
			...
			blit_for_mode(tmode, (UBYTE*)(let + 1), (let->character_width + 7) >> 3, 0, 0, rast,
						  x + let->left_offset, y + let->top_offset, let->character_width,
						  let->character_height, color, bcolor);
...
static Errcode load_hpjet_font(char* title, Vfont* vfont, SHORT height, SHORT unzag_flag)
{
	Errcode err;
	Hfcb* hfcb = NULL;
	(void)height;
...
Font_dev hpjet_font_dev = {
	NULL, "HP Downloadable", "*.M*", check_hpjet_font, load_hpjet_font, HPJET, 0};
