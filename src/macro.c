...
#define IS_MRTYPE(flags, type) (((flags) & (MACRO_REC | MR_TYPES)) == (MACRO_REC | (type)))
...
#define MACSTATE                                                     \
	(~(BDOWNBITS << BUPBIT1)) /* state flags saved in macro          \
...
#define PAUSE_WAIT                                           \
	(1 << (BUPBIT1 + 1)) /* before this event is executed we \
...
void qsave_macro(void)
{
	...
	if (!pj_exists(macro_name)) {
		...
		if ((title = vset_get_filename(stack_string("save_mac", buf), ".REC", save_str, MACRO_PATH,
									   NULL, 1)) != NULL) {
...
void qload_macro(void)
{
	...
	title = vset_get_filename(stack_string("load_mac", buf), ".REC", load_str, MACRO_PATH, NULL, 1);
	if (title == NULL) {
		return;
	}
...
Errcode end_abort_atom(void)
{
	...
	if (icb.macro_mode == MAKE_MACRO) {
		...
		if (Mcb.ar.flags & AR_ABORTLEVEL && Mcb.ab_level <= Mcb.mab_level) {
