...
static Errcode putxlat_line(void* pldat, Pixel* line, Coor x, Coor y, Ucoor width)
{
	Pldat* pld = pldat;
 ...
 * make it use the old cfit if cfit is present */
{
	...
	} else if (clearc == -2 || make_simple_cfit(rc->cmap, screen->cmap, cfit, clearc)) {
	...
	if (cel->flags & FCEL_XFORMED) {
		...
		if (pld.xlat) {
			xlatblit(rc, 0, 0, screen, rc->x, rc->y, rc->width, rc->height, pld.xlat);
		} else {
			pj_blitrect(rc, 0, 0, screen, rc->x, rc->y, rc->width, rc->height);
		}
	}
done:
...
static Errcode write_celframes(Flicel* src, SHORT start, SHORT num_frames, bool keep_cel,
							   bool ringit, Celcfit* cfit)
 ...
 * and writing frames of source from start to end. */
{
	...
	must_compress = (src->flif.hdr.width != vb.pencel->width ||
					 src->flif.hdr.height != vb.pencel->height || cfit != NULL);
	...
	if (must_compress) {
		while (start < max) {
			...
			if (pj_i_is_empty_rec(cbuf)) /* just write an empty frame !! */
			{
				...
				if ((err = write_next_flxframe(NULL, &flix, cbuf, undof, vb.pencel)) < Success) {
					goto error;
				}
			}
			++start;
...
static void acfit_line(Pixel* aline, Pixel* bline, Pixel* dline, Pixel* maxdline, Pixel* acfit)
{
	while (dline < maxdline) {
		if (*dline) {
			*dline++ = *bline++;
...
static void bcfit_line(Pixel* aline, Pixel* bline, Pixel* dline, Pixel* maxdline, Pixel* bcfit)
{
	while (dline < maxdline) {
		if (*dline) {
			*dline++ = bcfit[*bline++];
...
static void abcfit_line(Pixel* aline, Pixel* bline, Pixel* dline, Pixel* maxdline, Pixel* acfit,
						Pixel* bcfit)
{
	while (dline < maxdline) {
		if (*dline) {
...
static Errcode put_celsline(void* pframedat, Pixel* bline, Coor x, Coor y, Ucoor width)
...
/* ccb.cel a goes to mask 0 ccb.celb goes to mask non 0 */
{
	...
	if (ccb.fcelb->flags & FCEL_XFORMED) {
		err = raster_transform(ccb.fcelb->rc, pfd->dest, &ccb.fcelb->xf, put_celsline, pfd, 0);
		goto OUT;
	}
...
static Errcode make_trans_colors(int time_scale, Cmap* cmapa, Cmap* cmapb, Cmap* dcmap)
{
...
static Errcode write_transition_frames(Initpframe init_pframe, Getmask get_mask_frame,
 ...
 * of mask generator 0 being all a and count being all b */
{
	...
	if (fcel_needs_seekbuf(ccb.fcelb) && (size = fcel_cbuf_size(ccb.fcelb)) > cbufsize) {
		cbufsize = size;
	}
	if ((vs.co_type == COMP_MASK) && fcel_needs_seekbuf(ccb.mask_cel) &&
	...
	for (pfd.frame = 0; pfd.frame < pfd.frame_count; ++pfd.frame) {
		...
		if ((err = gb_seek_fcel_frame(ccb.start_cel, frame_a, pfd.cbuf, false)) < Success) {
			goto error;
		}
		if ((err = gb_seek_fcel_frame(ccb.end_cel, frame_b, pfd.cbuf, false)) < Success) {
			goto error;
		}

		if (pfd.frame_count > 1) {
			mask_scale0 = rscale_by(SCALE_ONE, pfd.frame, pfd.frame_count - 1);
			mask_scale1 = cmap_scale = rscale_by(SCALE_ONE, pfd.frame + 1, pfd.frame_count + 1);
		...
		pfd.docfit = 0;
		if (make_simple_cfit(ccb.fcela->rc->cmap, dcmap, ccb.fcela->cfit, ccb.clearc)) {
			pfd.docfit = 1;
		}
		if (make_simple_cfit(ccb.fcelb->rc->cmap, dcmap, ccb.fcelb->cfit, ccb.clearc)) {
			pfd.docfit |= 2;
		}
		...
		if (!ccb.preview_mode) {
			if (flix.hdr.frame_count == 0) {
				...
				err = write_next_flxframe(NULL, &flix, pfd.cbuf, pfd.history, pfd.dest);
			}
			if (err < Success) {
				goto error;
			}
...
static Errcode do_transition(Initpframe init_pframe, Getmask get_mask_frame,
							 Closepframe close_pframe)
{
	...
	switch (ccb.cfit_type) {
		case FIT_TOB:
			...
			if (!make_simple_cfit(ccb.start_cel->rc->cmap, vb.pencel->cmap, cfit, ccb.clearc)) {
				cfit = NULL;
			}
			see_cmap();
			break;
	...
	if (!ccb.preview_mode) {
		if ((err = write_celframes(ccb.start_cel, ccb.cela_start, ccb.cela_frames, true, false,
								   cfit)) < Success) {
			goto error;
		}
	}
	...
	if ((err = write_transition_frames(init_pframe, get_mask_frame, close_pframe)) < Success) {
		goto error;
	}
	...
	switch (ccb.cfit_type) {
		case FIT_TOA:
			...
			if (!make_simple_cfit(ccb.end_cel->rc->cmap, vb.pencel->cmap, cfit, ccb.clearc)) {
				cfit = NULL;
			}
			see_cmap();
			break;
	...
	if (!ccb.preview_mode) {
		...
		err = write_celframes(ccb.end_cel, ccb.celb_tailstart, ccb.celb_frames, false, true, cfit);
	} else {
...
static Errcode make_box_mask(int tscale0, int tscale1, Pframedat* pfd)
{
	...
	pj_set_rast(dest, pfd->invert);
	pj_set_rect(dest, !pfd->invert, cenx - width, ceny - height, width << 1, height << 1);
	return (Success);
}
...
static Errcode make_blind_mask(int tscale, Pframedat* pfd, bool vertical, Pixel offcol, Pixel oncol,
							   USHORT slatspace)
{
	Rcel* dest = pfd->dest;
	SHORT totsize;
...
{
	(void)tscale0;
	return (make_blind_mask(tscale1, pfd, 0, pfd->invert, !pfd->invert, vs.co_venetian_height));
}

static Errcode make_louver_mask(int tscale0, int tscale1, Pframedat* pfd)
{
	(void)tscale0;
	return (make_blind_mask(tscale1, pfd, 1, pfd->invert, !pfd->invert, vs.co_louver_width));
}
...
static Errcode init_boxil_mask(Pframedat* pfd)
{
	...
	spec.width = (vb.pencel->width + vs.co_boxil_width - 1) / vs.co_boxil_width;
	spec.height = (vb.pencel->height + vs.co_boxil_height - 1) / vs.co_boxil_height;
	spec.pdepth = 1;
...
static Errcode get_boxil_mask(int tscale0, int tscale1, Pframedat* pfd)
{
	...
	boxils_needed = (ULONG)(((FLOAT)(width * height)) * ((FLOAT)tscale1) / ((FLOAT)SCALE_ONE));
	...
	zoom_boxil_mask((Raster*)(pfd->boxil_mask), (Raster*)(pfd->dest), vs.co_boxil_width,
					vs.co_boxil_height);
...
/* no cfitting, no transition frames. */
{
	...
	if ((err = write_celframes(ccb.start_cel, ccb.cela_start, ccb.cela_frames, false, false,
							   NULL)) < Success) {
		goto error;
	}
...
static bool comp_abort_verify(void* data)
{
	...
	if (ccb.preview_mode) {
		...
		return (soft_yes_no_box("!%d%d", "join_pabort", ccb.preview_frame, ccb.total_frames));
	}
	return (soft_yes_no_box("!%d%d", "join_abort", flix.hdr.frame_count + 1, ccb.total_frames));
}
 ...
 ****************************************************************************/
{
	...
	ccb.total_frames = ccb.cela_frames + ccb.celb_frames + ccb.transition_frames;
...
error:
	...
	if ((err >= Success) || (make_tempflx(ram_tflx_name, 0) < Success)) {
