 ...
 ****************************************************************************/
{
	...
	if ((name = pj_get_filename(header, suff, button, cs.out.path, cs.out.path, TRUE,
								&cs.out.scroller_top, NULL)) != NULL) {
 ...
 ****************************************************************************/
{
	...
	if ((err = go_resize_screen(init_after_screen, NULL)) < Success && err != Err_abort) {
		return (err); /* screen init failed, fatal */
	}
	if (err >= Success) {
		if ((err = conv_set_pencel(vb.screen->wndo.width, vb.screen->wndo.height)) < Success) {
			return (err);
		}
...
void main(int argc, char** argv)
{
	...
	if ((err = init_pj_startup(NULL, NULL, argc, argv, "conv_help", "aa.mu")) < Success) {
		goto error;
	}
	oldconfig = err;
