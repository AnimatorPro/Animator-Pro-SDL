...
static Errcode get_new_flic(Pdr* pdr, char* pdr_name, Pdr** pnew_pdr, char* flic_name)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	if (name == NULL) {
		...
		name = pj_get_filename(stack_string("conv_loadf", titbuf), suff, load_str, cs.in.path,
							   cs.in.path, FALSE, &cs.in.scroller_top, cs.in.wildcard);
...
ERROR:
	show_mouse();
	softerr(err, "!%s%d", "conv_view", cs.in_name, cs.ifi.frame_ix + 1);
}
 ...
 ****************************************************************************/
{
	...
	pdr_get_suffi(pdr, sufbuf);
	if ((pt = conv_save_name(stack_string("conv_svpic", sbuf), sufbuf, save_str)) == NULL) {
		goto OUT;
	}
	ai = cs.ifi.ai;
	...
	if ((err = pdr_save_frames(ifile, vb.pencel, 1, return_success, NULL, NULL)) < Success) {
...
Errcode save_a_flic(char* pdr_name, char* name, int frames, Errcode (*iseek)(int ix, void* data))
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	if (!pdr_best_fit(pdr, &ai)) /* and that it can handle this resolution */
	{
		if (ai.num_frames != frames) {
			err = softerr(Err_unimpl, "!%s%d", "conv_num_frames", pdr_name, ai.num_frames);
		} else {
			err = softerr(Err_wrong_res, "!%d%d", "conv_wrong_res", ai.width, ai.height);
		}
		goto OUT;
	}
	pdr_get_suffi(pdr, sufbuf);
	if (name == NULL) {
		if ((name = conv_save_name(stack_string("conv_svflic", sbuf), sufbuf, save_str)) == NULL) {
			err = Err_abort;
			goto OUT;
		}
	}
	if ((err = valloc_anycel(&work_cel, ai.width, ai.height)) < Success) {
		goto OUT;
	}
	...
	if ((err = pdr_save_frames(ifile, vb.pencel, frames, iseek, NULL, work_cel)) < Success) {
		pdr_close_ifile(&ifile);
