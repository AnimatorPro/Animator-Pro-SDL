 ...
 ****************************************************************************/
{
	...
	if ((rgb_in = pj_malloc(w)) == NULL || (rgb_out = pj_malloc(new_w)) == NULL ||
...
static Errcode scaley(UBYTE* inbytes,  /* unscaled source 2-d array */
 ...
 ****************************************************************************/
{
	...
	if ((inbytes = pj_malloc(ibsize)) == NULL || (inline = pj_malloc(oh)) == NULL ||
	...
	if ((err = scaley(inbytes, outbytes, inline, outline, w, oh, nh, name)) < Success) {
		goto OUT;
	}
...
Errcode rgb_files_to_cel(Rcel* cel, int comp_count, Boolean new_cmap, Boolean flip)
/*****************************************************************************
...
static Errcode pcel_from_rgb_files(Rcel** pcel, int w, int h, Cmap* cmap, int comp_count)
/*****************************************************************************
...
static Errcode cel_to_scaled_rgb(Rcel* cel, int neww, int newh, int* pcomp_count)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	for (i = 0; i < comp_count; ++i) {
		...
		if ((err = yscale_file(rgb_names[i], neww, cel->height, newh)) < Success) {
			goto OUT;
		}
	}
OUT:
 ...
 ****************************************************************************/
{
	...
	if ((err = cel_to_scaled_rgb(source, dest->width, dest->height, &comp_count)) < Success) {
		goto OUT;
	}
...
Errcode scale_flic(char* sname, int nw, int nh, char* pdr_name)
{
	...
	if ((err = pdr_save_frames(ifile, cur_frame, ai.num_frames, scale_seek, &sd, prev_frame)) <
		Success) {
...
static Errcode render_scale()
{
	...
	if (cs.ifi.ai.num_frames == 1) /* single frame scale */
	{
		...
		if ((name = conv_save_name(stack_string("conv_sclname", titbuf), suffi,
								   stack_string("conv_sclword", nbuf))) != NULL) {
			if ((err = scale_flic(name, cs.scalew, cs.scaleh, pdr_name)) >= Success) {
...
void qscale_menu()
{
	...
	for (;;) {
		choice = soft_qchoice(NULL, "!%d%d%d%d%d%d%d%d", "conv_scale", cs.ifi.cel->width,
							  cs.ifi.cel->height, cs.scalew, cs.scaleh, sw, sh, cs.ifi.ai.width,
							  cs.ifi.ai.height);
		switch (choice) {
			case 0: /* Set width */
				clip_soft_qreq_number(&cs.scalew, 4, 10000, NULL, NULL, "conv_scale_width");
				break;
			case 1: /* Set height */
				clip_soft_qreq_number(&cs.scaleh, 4, 10000, NULL, NULL, "conv_scale_height");
				break;
