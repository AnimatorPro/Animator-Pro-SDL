 ...
 ****************************************************************************/
{
	...
	if (cs.colors_256) {
		...
		while (--count >= 0) {
			cbit = ((long)(*r++ & 0xfc) << 10) + ((*g++ & 0xfc) << 4) + ((*b++ & 0xfc) >> 2);
			cbyte = (cbit >> 3);
			h[cbyte] |= bitmasks[cbit & 7];
		}
	}
 ...
 ****************************************************************************/
{
	...
	soft_status_line("!%6ld%4d%4d", "ctop_cpack", lscount - usedc, freec, threshold);
...
static Errcode fpack_ctable(Rgb3* source, /* Source - array scount long of rgb triples */
...
FINDHI:
	...
	*curthresh = 0;
	...
	hiwater = threshold;
	while (threshold >= 3) {
		if ((newused = find_newc(usedc, dcount - usedc, dest, threshold, source, scount,
								 curthresh)) < Success) {
			if (newused != Err_overflow) {
				goto OUT;
			} else {
				...
				if (!have_reported) {
					have_reported = TRUE;
					continu_box(
						"Original version would have stopped here.\n"
						"Colors used now = %d\n",
						usedc);
				}
#endif
 ...
 ****************************************************************************/
{
	...
	copy_mem(vconfg.mc_ideals, &cmap->ctab[FIRST_MUCOLOR], NUM_MUCOLORS * sizeof(Rgb3));

	err = fpack_ctable((Rgb3*)big_ctab, ccount, cmap->ctab, cmap->num_colors - NUM_MUCOLORS);
