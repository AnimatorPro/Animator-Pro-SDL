 ...
 ****************************************************************************/
{
	if (ainfo->depth == 8 && ainfo->num_frames <= MAXFRAMES) {
		return (TRUE);
	}
 ...
 ****************************************************************************/
{
	...
	if ((err = pj_fli_add_frame1(name, &gf->flif, cbuf, screen)) < Success) {
		goto error;
	}
	...
	for (i = 1; i < num_frames; ++i) {
		...
		if ((err = pj_fli_add_next(name, &gf->flif, cbuf, work_screen, screen)) < Success) {
			goto error;
		}
		pj_freez(&cbuf);
		pj_rcel_copy(screen, work_screen);
	...
	if ((err = pj_fli_add_ring(name, &gf->flif, cbuf, work_screen, screen)) < Success) {
		goto error;
	}
error:
	pj_freez(&cbuf);
