...
pj_copy_bytes(s, d, count) register char* s, *d;
int count;
{
	while (--count >= 0) {
		*d++ = *s++;
...
print_by_cw(w) int w;
{
	fprintf(OUT, "%d+%d*CH_WIDTH, ", w % CH_WIDTH, w / CH_WIDTH);
}

print_by_ch(h) int h;
{
	fprintf(OUT, "%d+%d*CH_HEIGHT, ", h % CH_HEIGHT, h / CH_HEIGHT);
}

write_m(p, ident) Pull* p;
int ident;
{
	print_idents(ident);
