...
void vstretch_cel(int toolmode)
{
	...
	for (;;) {
		...
		x0 = rotm.x = icb.mx;
		y0 = rotm.y = icb.my;
		frotate_points2d(itheta_tofloat(-thecel->cd.rotang.z, FCEL_TWOPI), &thecel->cd.cent, &rotm,
						 &rotm, 1);
		...
		for (;;) {
			...
			top_textf("%5d%% x %5d%% y", sscale_by(100, swidth, rc->width),
			...
			rotm.x = icb.mx;
			rotm.y = icb.my;
			frotate_points2d(itheta_tofloat(-thecel->cd.rotang.z, FCEL_TWOPI), &thecel->cd.cent,
							 &rotm, &rotm, 1);
		}
	}
...
static void fcel_toptext_info(Flicel* fc, Short_xy* ocent)
{
	...
	soft_top_textf("!%4d%4d%4d%4d%4d%4d", "top_celpos", fc->cd.cent.x, fc->cd.cent.y,
				   fc->cd.cent.x - ocent->x, fc->cd.cent.y - ocent->y,
...
static Errcode paste1(void* paste1dat, int ix, int intween, int scale, Autoarg* aa)
/* auto vec for moving a cel in a straight line.  Ie paste multi */
{
	...
	set_fcel_center(thecel, pdat->ocent.x + itmult(dx, scale), pdat->ocent.y + itmult(dy, scale));
...
do_paste:
	...
	if (paste) {
		...
		if (vs.multi) {
			if ((err = go_autodraw(paste1, &pdat, (AUTO_USESCEL | AUTO_UNZOOM | AUTO_PUSHINKS))) <
				0) {
				goto error;
			}
		} else {
