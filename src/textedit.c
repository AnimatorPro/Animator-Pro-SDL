...
static void wwrefresh(Text_file* gf, char* s, Linedata* ldat, SHORT x, SHORT y, SHORT w,
 ...
 * if == 1 will zoom too */
{
	...
	while (--lines >= 0) {
		...
		if (--skiplines < 0) {
			...
			gf->undraw_rect(gf->raster, gf->undraw_data, textstart - font->left_overlap, y,
			...
			if (s != NULL) {
				textwid = justify_line(gf->raster, gf->font, buf, x, y, w, color, TM_MASK1, sblack,
									   gf->justify_mode, &textstart, 0);
...
static void draw_twintext(Text_file* gf, int forceall)
{
	wwrefresh(gf, gf->wstart, gf->ldat, gf->twin.x, gf->twin.y, gf->twin.width,
			  gf->text_lines_visible, gf->ccolor, 0, forceall);
}
...
{
	cline_frame(gf->twin.x - 1, gf->twin.y - 1, gf->twin.x + gf->twin.width,
				gf->twin.y + gf->twin.height, pdot, dotdat);
}
   ...
   line position. */
{
	...
	gf->pixelx =
		just_charstart(gf->font, gf->twin.x, gf->twin.width, lbuf, gf->twxpos, gf->justify_mode);
...
static void hide_text_cursor(Text_file* gf)
{
	...
	if (gf->overwrite) {
		...
		pj_xor_rect(gf->raster, gf->text_cursor_color, gf->pixelx, gf->pixely, width, hgt);
	}

	pj_xor_rect(gf->raster, gf->text_cursor_color, gf->pixelx, top, width, hgt);
...
static void xor_text_cursor(Text_file* gf)
{
	...
	if (gf->overwrite) {
		...
		if (gf->pixelw > 2) {
			...
			pj_xor_rect(gf->raster, gf->text_cursor_color, x, owr.y, owr.width, 2);
		...
		pj_xor_rect(gf->raster, gf->text_cursor_color, x, top2, width, hgt);
	}
do_vertical:

	pj_xor_rect(gf->raster, gf->text_cursor_color, x, top, width, hgt);
...
{
	if (nline != NULL && gf->twypos >= gf->text_lines_visible - 1 &&
...
static void update_mark(Text_file* gf, long alterp, long count, long* pmark)
{
	long mark = *pmark;
	(void)gf;
...
static void update_win(Text_file* gf, long extracs) /* # of characters added or subtracted
													 * at cursor position */
   ...
   that have to be redrawn. */
{
	...
	} else /* more than one line, figure out which ones to update */
	{
		...
		if (gf->llstart != NULL && new_lstart != gf->lstart) {
			...
			{
				wwrefresh(gf, gf->wstart, gf->ldat, gf->twin.x, gf->twin.y, gf->twin.width, 1,
						  gf->ccolor, gf->twypos - 1, 0);
			}
		}
...
static void insert_at_cp(Text_file* gf, char* buf, long count)
{
	if (gf->text_size < ((gf->text_alloc - count) - 1)) {
		if (!gf->read_only) {
			...
			back_copy_mem(gf->text_buf + gf->tcursor_p, gf->text_buf + gf->tcursor_p + count,
...
static void overwrite_char(Text_file* gf, char c)
{
	...
	if ((gf->text_size == gf->tcursor_p) || *cc == '\n' || *cc == 0) {
		insert_char(gf, c);
		return;
	}
...
static void delete_at_cp(Text_file* gf, long dcount)
{
	...
	if (count > 0) {
		gf->is_changed = true;
		gf->nlstart = nextline(gf, gf->lstart);
		pj_copy_bytes(gf->text_buf + gf->tcursor_p + dcount, gf->text_buf + gf->tcursor_p, count);
		gf->text_size -= dcount;
	}
}
	...
	switch (askey) {
		case 0: {
			switch (inkey) {
				...
				case PAGEDN: {
					if (seek_line(gf, gf->wstart, gf->text_lines_visible) != NULL) {
						if (gf->text_lines_visible == 1) {
							gf->text_yoff += 1;
						} else {
							gf->text_yoff += gf->text_lines_visible - 1;
...
static long pixel_to_cursor(Text_file* gf, int x, int y)
{
	...
	while (glb + 1 < lub) {
		if (just_charstart(gf->font, gf->twin.x, gf->twin.width, line, cbound, gf->justify_mode) <
			x) {
			glb = cbound;
...
long ted_copy_block(Text_file* gf, char* clip_name)
{
	...
	if ((count = (gf->block_end - gf->block_start)) > 0) {
		if ((err = write_gulp(clip_name, gf->text_buf + gf->block_start, count)) < Success) {
			return (err);
		}
	}
	return (count);
...
void ted_load_block(Text_file* gf, char* dir, char* clip_name)
{
	...
	if (get_aafilename(stack_string("load_block", titbuf), ".TXT", load_str, path, path, 0) !=
		NULL) {
...
void ted_save_block(Text_file* gf, char* dir, char* clip_name)
{
	...
	make_file_path(dir, "", path);
	if (get_aafilename(stack_string("save_block", titbuf), ".TXT", save_str, path, path, 0) !=
		NULL) {
...
/* returns 1 if there was text in the window 0 if not */
{
	...
	if (!ld) {
		gf->undraw_rect(
			gf->raster, gf->undraw_data, gf->twin.x - font->left_overlap - 1, gf->twin.y - 1,
			gf->twin.width + font->left_overlap + font->right_overlap + 3, gf->twin.height + 2);
	...
	while (linesleft > 0 && ld->cstart != NULL) {
		if (ld->width) {
			istext = 1;
			gf->undraw_rect(
				gf->raster, gf->undraw_data, gf->twin.x + ld->xstart - font->left_overlap, y,
				ld->width + font->left_overlap + font->right_overlap + 1, gf->line_height);
		}
		y += dy;
		ld->width = 0; /* erased */
		++ld;
...
static void define_twin(Text_file* gf)
{
	gf->undraw_rect(gf->raster, gf->undraw_data, 0, 0, gf->raster->width, gf->raster->height);
...
static void move_twin(Text_file* gf)
{
	...
	for (;;) {
		...
		do_leftbehind(gf->twin.x, gf->twin.y, lx, ly, gf->twin.width, gf->twin.height,
