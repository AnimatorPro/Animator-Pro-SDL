...
#ifdef DEBUG
static SHORT* fat_ss2_line(SHORT* s1, SHORT* s2, SHORT* cbuf, int count, int skipct)
...
static SHORT ss2_line(SHORT* old,          /* buffer for previous frame */
	   ...
	   * a skip */
{
	if (++new >= end) {
		while ((init_skips = skip_count) > MAX_SKIPS) {
			comp = save_copy(comp, MAX_SKIPS, new - skip_count + MAX_SKIPS - 1, 1);
...
{
	if (++new >= end) {
		comp = save_copy(comp, init_skips, new - copy_count, copy_count - skip_count);
		goto FINISH;
	}
	lastn = n;
	++copy_count;
	if (*(++old) == (n = *new)) {
		if (++skip_count >= COPYBREAK) /* look for embedded skips */
		{
			comp = save_copy(comp, init_skips, new + 1 - copy_count, copy_count - skip_count);
			/* Write out accumulated copy minus current skips */
			goto SKIPPING;
		}
	} else {
	...
	if (n == lastn) {
		if (++run_count >= RUN_IN_COPY_BREAK) {
			comp = save_copy(comp, init_skips, new + 1 - copy_count, copy_count - run_count);
			init_skips = 0;
			goto RUNNING;
		}
	} else {
				   ...
				   USHORT width, USHORT height)
{
	...
	while (row < height) {
		...
		while (row < height) {
			...
			{
				bskip = dpr << 2;
				if ((bskip = pj_bcompare(line1 + bskip, line2 + bskip, bafter_doubles)) !=
					bafter_doubles) {
					wskip <<= 1;
					wskip += bskip >> 1;
					gotline = 1;
		...
		if (gotline) {
			...
			switch (width) {
				...
				{
					if (ss2_line((SHORT*)line1, (SHORT*)line2, &cpt, wpr, wskip)) {
						break;
					}
