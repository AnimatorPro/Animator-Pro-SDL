...
LONG pj_fli_comp_rect(void* comp_buf, Rcel* last_screen, Rcel* this_screen, Rectangle* rect,
 ...
 * must be within both screens provided */
{
	...
	if (do_colors) {
		switch (frame_type) {
			case COMP_FIRST_FRAME: {
				stamp_size = (*(comp_lib->make_pstamp))(this_screen, chunk, rect->x, rect->y,
			...
			case COMP_BLACK_FIRST: {
				vp = (*(comp_lib->first_colors))(this_screen->cmap->ctab, chunk + 1, COLORS);
			...
			default: {
				...
				vp = (*(comp_lib->delta_colors))(last_screen->cmap->ctab, this_screen->cmap->ctab,
												 chunk + 1, COLORS);
	...
	switch (frame_type) {
		case COMP_DELTA_FRAME:
			vp = (*(comp_lib->delta_comp))((Raster*)last_screen, chunk + 1, rect->x, rect->y,
										   (Raster*)this_screen, rect->x, rect->y, rect->width,
										   rect->height);
			chunk->type = comp_lib->delta_type;
			break;
		case COMP_FIRST_FRAME:
			vp = (*(comp_lib->first_comp))((Raster*)this_screen, chunk + 1, rect->x, rect->y,
										   rect->width, rect->height);
	...
	if (vp == NULL) {
		vp = chunk + 1;
		pj_get_rectpix(this_screen, vp, rect->x, rect->y, rect->width, rect->height);
...
LONG pj_fli_comp_cel(void* comp_buf, Rcel* last_screen, Rcel* this_screen, SHORT frame_type,
					 Flicomp comp_type)
 ...
 * rectangle to compress. assumes both screens the same size */
{
	...
	return (
		pj_fli_comp_rect(comp_buf, last_screen, this_screen, &rect, true, frame_type, comp_type));
}
