 ...
 */
void pj_fcuncomp(const UBYTE* src, Rgb3* dst)
{
 ...
 *  Uncompress colour palette onto a buffer.
 */
void pj_fcuncomp64(const UBYTE* src, Rgb3* dst)
{
	unsigned int count = ((const uint16_t*)src)[0];
	src += 2;
...
void pj_fli_uncomp_rect(Rcel* f, struct fli_frame* frame, Rectangle* rect,
	...
	for (j = 0; j < frame->chunks; j++) {
		switch (CHUNK->type) {
			...
			case FLI_COLOR256:
				...
				break;
			case FLI_LC:
				pj_unlccomp_rect(f, CHUNK + 1, 1, rect->x, rect->y, rect->width, rect->height);
				break;
			case FLI_SS2:
				pj_unss2_rect(f, CHUNK + 1, 1, rect->x, rect->y, rect->width, rect->height);
				break;
			case FLI_COLOR_0:
				pj__set_rect(f, 0, rect->x, rect->y, rect->width, rect->height);
				break;
			case FLI_BRUN:
				pj_unbrun_rect(f, CHUNK + 1, 1, rect->x, rect->y, rect->width, rect->height);
				break;
			case FLI_COPY:
				_pj_put_rectpix(f, (Pixel*)(CHUNK + 1), rect->x, rect->y, rect->width,
								rect->height);
				break;
		}
...
void pj_fli_uncomp_frame(Rcel* screen, Fli_frame* frame, int colors) /* update color registers? */
