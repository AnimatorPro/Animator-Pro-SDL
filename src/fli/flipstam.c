...
void pj_make_pstamp_xlat(Rgb3* ctab, UBYTE* xlat, int count)
{
	...
	while (count-- > 0) {
		*xlat++ = 6 * ctab->r / RGB_MAX * 36 + 6 * ctab->g / RGB_MAX * 6 + 6 * ctab->b / RGB_MAX +
				  START_SIXCUBE;
		++ctab;
	}
}
...
static void comp_pstamp_line(int sy, int dy, void* dat)
{
	...
	if (brd->dst) {
		if (brd->last_sy != sy) {
			pj_get_hseg(brd->screen, (spix = brd->srcline), brd->sx, (brd->last_sy = sy), brd->sw);
		...
		if ((brd->dst = (BYTE*)(*(brd->comp_line))((BYTE*)brd->dline, brd->dst, brd->dw)) >
			brd->dstmax) {
			brd->dst = NULL; /* will fail test above */
		}
	}
}
void pj_get_stampsize(SHORT maxw, SHORT maxh, SHORT sw, SHORT sh, SHORT* pw, SHORT* ph)
...
LONG pj_build_rect_pstamp(Rcel* screen, void* cbuf, SHORT x, SHORT y, USHORT width, USHORT height)
{
	Brundat brd;
	Pstamp_chunk* chunk = (Pstamp_chunk*)cbuf;
	LONG square;

	pj_get_stampsize(PSTAMP_W, PSTAMP_H, width, height, &chunk->width, &chunk->height);
