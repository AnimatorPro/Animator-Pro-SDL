...
static void scale_ubrun_line(int sy, int dy, void* dat)
{
	...
	if (brd->last_sy != sy) {
		...
		brd->src = pj_unbrun_scale_line(brd->src, brd->sw, (BYTE*)(brd->dline), brd->xtable);
	}
...
void pj_unbrun_scale_rect(Raster* dst, void* ucbuf, USHORT sw, USHORT sh, SHORT dx, SHORT dy,
						  USHORT dw, USHORT dh)
