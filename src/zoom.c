...
static Button zoom_source_sel = MB_INIT1(NONEXT,           /* next */
										 NOCHILD,          /* children */
...
static Button zoom_resize_sel = MB_INIT1(&zoom_source_sel, /* next */
										 NOCHILD,          /* children */
...
static Button zoom_fullsize_sel = MB_INIT1(&zoom_resize_sel,            /* next */
										   NOCHILD,                     /* children */
										   54, 11, 5, 12,               /* width height x y */
										   NODATA,                      /* "Fullsize",*/
										   see_fullsize, zwin_fullsize, /* feelme */
										   NOOPT,                       /* optme */
										   NOGROUP,                     /* group */
										   0,                           /* id */
										   NOKEY,                       /* key equivalent */
...
static Button zoom_scale_slider = MB_INIT1(&zoom_fullsize_sel, /* next */
										   NOCHILD,            /* children */
...
static Button zoom_scaletext = MB_INIT1(&zoom_scale_slider, /* next */
										NOCHILD,            /* children */
...
static Button zoom_tit_sel = MB_INIT1(&zoom_scaletext,   /* next */
									  NOCHILD,           /* children */
...
static Button zoom_dragbar = MB_INIT1(NONEXT,                    /* next */
									  NOCHILD,                   /* children */
									  320, 8, 0, 0,              /* w,h,x,y */
									  NOTEXT, see_zoom_title,    /* seeme */
									  feel_titlebar,             /* feelme */
...
static Button zoom_box = MB_INIT1(&zoom_dragbar, /* next */
								  NOCHILD,       /* children */
								  0, 0, 0, 0,    /* w,h,x,y */
								  NULL,          /* datme */
								  see_zoombox,   /* seeme */
...
{
	if ((!JSTHIT(MBPEN | MBRIGHT)) || !vs.zoom_open || icb.iowndo != &(zwinmenu.mw->w) ||
...
static void see_zoombox(Button* b)
{
	...
	draw_quad((Raster*)b->root, vb.screen->SWHITE, b->x - 1, b->y - 1, b->width + 2, b->height + 2);
	draw_quad((Raster*)b->root, vb.screen->SGREY, b->x - 2, b->y - 2, b->width + 4, b->height + 4);
}
...
static void feel_zoombox(Button* b)
{
	...
	if ((PENWNDO->doit) && ISINPUT(PENWNDO->ioflags)) {
...
{
	vs.zcentx = scale_vscoor(vl.zrect.x + vl.zrect.width / 2, vb.pencel->width);
	vs.zcenty = scale_vscoor(vl.zrect.y + vl.zrect.height / 2, vb.pencel->height);
}
...
{
	return (ptin_rect((Rectangle*)&(zoom_box.RECTSTART), icb.sx - vl.zoomwndo->behind.x,
...
void get_zoomcurs_flixy(Short_xy* xy)
{
	...
	xy->x = (icb.cx - (vl.zoomwndo->behind.x + zoom_box.x)) / vs.zoomscale + vl.zrect.x;
	xy->y = (icb.cy - (vl.zoomwndo->behind.y + zoom_box.y)) / vs.zoomscale + vl.zrect.y;
}
 ...
 * and put cursor at fat pixel screen position */
{
	...
	if (curs_in_zoombox()) {
		...
		icb.cx = ((icb.mx - vl.zrect.x) * vs.zoomscale) + zoom_box.x + icb.mset.oset.x;
		icb.cy = ((icb.my - vl.zrect.y) * vs.zoomscale) + zoom_box.y + icb.mset.oset.y;
 ...
 * zoom box is smaller than screen */
{
	...
	if (zwid >= vb.screen->wndo.width && zht >= vb.screen->wndo.height &&
		wrect->width >= vb.screen->wndo.width && wrect->height >= vb.screen->wndo.height) {
...
/* calculates zrect to and clips zoomwindow size for current zoomscale */
{
	...
	zrect->x = uscale_vscoor(vs.zcentx, vb.pencel->width) - zrect->width / 2;
	zrect->y = uscale_vscoor(vs.zcenty, vb.pencel->height) - zrect->height / 2;
...
static Errcode open_zwinmenu(int squawk)
{
	...
	err = smu_load_name_text(&smu_sm, "zoom_panel", "Tzwin_title", (char**)&zoom_dragbar.datme);
	if (err < Success) {
		goto error;
	}
...
/* have user rubberband rectangle around area he/she wants zoomed */
{
	...
	zwinmenu.height = rect.height * vs.zoomscale + ZBOX_BORSIZE + zoom_dragbar.height;
...
static void reposit_zoomwndo(void)
{
	...
	if ((err = screen_cut_rect(vb.screen, (Rectangle*)&(zwinmenu.RECTSTART), &maxrect)) < 0) {
		goto error;
	}
...
static bool is_fullsize(void)
{
	return (vl.zoomwndo != NULL &&
			(zoom_box.width == vb.screen->wndo.width || vl.zrect.width == vb.pencel->width) &&
			(zoom_box.height == vb.screen->wndo.height || vl.zrect.height == vb.pencel->height));
}
...
static void deltascale_zwndo(void* data, Button* b)
{
	...
	draw_buttontop(&zoom_fullsize_sel);
	if (!(zbox.x == zoom_box.x && zbox.y == zoom_box.y && zbox.width == zoom_box.width &&
		  zbox.height == zoom_box.height)) {
		pj_set_rect(&zwinmenu.mw->w, 0, zoom_box.x, zoom_box.y, zoom_box.width, zoom_box.height);
	}
	pj_zoomblit(vb.pencel, vl.zrect.x, vl.zrect.y, vl.zoomwndo, zbox.x, zbox.y, zbox.width,
				zbox.height, vs.zoomscale, vs.zoomscale);
};
...
/* puts an hseg into zoom window in pencel coords */
{
	...
	if (((y -= vl.zrect.y) < 0) || (y >= vl.zrect.height)) {
		return;
	}
...
void zoom_txlatblit(Raster* src, Coor sx, Coor sy, Ucoor width, Ucoor height, Coor dx, Coor dy,
					Tcolxldat* tcxl)
...
/* puts vseg into zoom window */
{
	...
	if (((x -= vl.zrect.x) < 0) || (x >= vl.zrect.width)) {
		return;
	}
...
void zoom_blitrect(Raster* src, Coor sx, Coor sy, Coor x, Coor y, Coor width, Coor height)
 ...
 * rects */
{
	...
	pj_zoomblit(src, sx, sy, vl.zoomwndo, zx + zoom_box.x, zy + zoom_box.y, width, height,
...
static Pentool hand_ptool = PTOOLINIT1(NONEXT, "", PTOOL_OPT, 0, NULL, NO_SUBOPTS, NULL,
									   hand_ptfunc, &hand_cursor.hdr, NULL, /* on install */
									   NULL                                 /* on remove */
);

static Smu_button_list zoom_smblist[] = {
	{"title", {&zoom_tit_sel}},       {"src", {&zoom_source_sel}},  {"dest", {&zoom_resize_sel}},
	{"fullsz", {&zoom_fullsize_sel}}, {"scale", {&zoom_scaletext}},
};
...
void go_zoom_settings(void)
{
	...
	hide_mp();
	fliborder_on();
	if (soft_buttons("zoom_panel", zoom_smblist, Array_els(zoom_smblist), &ss) < Success) {
		goto error;
	}
	optool = vl.ptool;
...
{
	if (vs.zoom_open && vl.zoomwndo) {
		pj_zoomblit(vb.pencel, vl.zrect.x, vl.zrect.y, vl.zoomwndo, zoom_box.x, zoom_box.y,
					zoom_box.width, zoom_box.height, vs.zoomscale, vs.zoomscale);
	}
}
...
static void set_zoom(void)
{
	...
	get_zwinrect();
	calc_zoombox();
	if ((rect_in_place(&vl.zrect)) >= 0 && (clip_move_rect(&vl.zrect)) >= 0) {
		save_zrectpos(); /* save new position setting */
	}
	open_zwinmenu(1);
	show_mp();
