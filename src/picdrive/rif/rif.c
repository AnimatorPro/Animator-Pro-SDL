 ...
 ****************************************************************************/
{
	...
	err = conv_bitmaps(gf->planes, gf->rif.depth, gf->bpr, gf->rif.width, gf->rif.height, screen);
 ...
 ****************************************************************************/
{
	...
	if ((err = xfseek(gf->file, gf->rif.frame_count * sizeof(long) + sizeof(Riff_head),
					  XSEEK_SET)) < Success) {
		return (err);
	}
...
Local_pdr rif_local_pdr = {NULL, rif_pdr_name, &rif_pdr_header};
