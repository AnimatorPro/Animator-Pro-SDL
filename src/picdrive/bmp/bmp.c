...
static void bits_to_bytes(char* in, char* out, int in_size, char zero_color, char one_color)
...
static Errcode read_head(XFILE* f, BITMAPFILEHEADER* head, BITMAPINFOHEADER* info)
 ...
 * generally make sure the values look reasonable. */
{
	...
	if (head->bfSize < sizeof(*head) + sizeof(*info) ||
		head->bfOffBits < sizeof(*head) + sizeof(*info) || head->bfSize < head->bfOffBits) {
		err = Err_format;
		goto ERROR;
	}
		...
		} else /* It's an escape or a bunch of literal data. */
		{
			switch (pair.second) {
				...
				default: /* Copy some bytes (and skip word pad). */
					...
					{
						if ((err = read_buf(f, data_buf, NEXT_QUAD(pair.second) >> 1)) < Success) {
							goto ERROR;
						}
						nibbles_to_bytes(data_buf, pixel_buf, NEXT_EVEN(pair.second) >> 1);
						pj_put_hseg(screen, pixel_buf, curx, cury, pair.second);
					} else /* Byte a pixel literal run. */
					{
						if ((err = read_buf(f, data_buf, NEXT_EVEN(pair.second))) < Success) {
							goto ERROR;
						}
...
static Errcode read_after_header(XFILE* f, BITMAPFILEHEADER* head, BITMAPINFOHEADER* info,
								 enum Info_type itype, Rcel* screen)
 ...
 * compressed and calls appropriate pixel reader. */
{
	...
	{
		/* Seek to color start. */
		if ((err = xfseek(f, sizeof(*head) + info->biSize, XSEEK_SET)) < Success) {
			return err;
		}
		if (itype == INFO_TYPE_NEW) {
			if ((err = read_new_colors(f, screen->cmap, (1 << info->biBitCount))) < Success) {
				return err;
			}
		} else {
			if ((err = read_old_colors(f, screen->cmap, (1 << info->biBitCount))) < Success) {
				return err;
			}
		}
	}
...
/*-------------------------BMP WRITE SECTION--------------------------------*/
static void init_header(int32_t width, int32_t height, BITMAPFILEHEADER* head,
...
static Errcode write_after_header(XFILE* f, BITMAPFILEHEADER* head, BITMAPINFOHEADER* info,
								  Rcel* screen)
 ...
 */
static Errcode open_helper(Bmp_image_file** pf, char* path, enum XReadWriteMode mode)
{
	Errcode err = Success;
	Bmp_image_file* f;
	...
	*pf = NULL;
	...
	if (!suffix_in(path, ".BMP") && !suffix_in(path, ".RLE") && !suffix_in(path, ".DIB")) {
		return (Err_suffix);
	}
...
Local_pdr bmp_local_pdr = {NULL, bmp_pdr_name, &bmp_pdr_header};
