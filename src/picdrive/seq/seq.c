...
void blit_box(int w, int h, int sx, int sy, unsigned char* s, int sbpr, int dx, int dy,
			  unsigned char* d, int dbpr);
void xor_blit_box(int w, int h, int sx, int sy, unsigned char* s, int sbpr, int dx, int dy,
				  unsigned char* d, int dbpr);
 ...
 ****************************************************************************/
{
	...
	nofit = (ainfo->depth == 8 && ainfo->width == STW && ainfo->height == STH);
	ainfo->depth = 8;
	ainfo->width = STW;
	ainfo->height = STH;
 ...
 ****************************************************************************/
{
	...
	if (neo.op == NEO_XOR) {
		for (i = 0; i < 4; ++i) {
			xor_blit_box(neo.width, neo.height, 0, 0, planes, sbpr, neo.xoff, neo.yoff,
						 stscreen + 8000 * i, 40);
			planes += plane_size;
		}
	} else {
		clear_mem(stscreen, 32000);
		for (i = 0; i < 4; ++i) {
			blit_box(neo.width, neo.height, 0, 0, planes, sbpr, neo.xoff, neo.yoff,
					 stscreen + 8000 * i, 40);
			planes += plane_size;
		}
	}
 ...
 ****************************************************************************/
{
	...
	if ((err = fseek(gf->file, gf->seq.cel_count * sizeof(long) + sizeof(Seq_header), SEEK_SET)) <
		Success) {
		return (err);
	}
