...
Errcode load_pdr(char* path, Pdr** ppdr)
{
	...
	if ((lpd = (Local_pdr*)(name_in_list(name, (Names*)local_pdrs))) != NULL) {
		*ppdr = lpd->header;
		return (Success);
	}
...
{
	return sprintf(buf, "%.*s", (int)sizeof(pd->default_suffi) - 1, pd->default_suffi);
}
 ...
 *
 ****************************************************************************/
Errcode pdr_create_ifile(struct pdr* pd, char* path, Image_file** pifile, Anim_info* spec)
{
	Errcode err;
 ...
 *
 ****************************************************************************/
Errcode pdr_open_ifile(struct pdr* pd, char* path, Image_file** pifile, Anim_info* ainfo)
{
	Errcode err;
						...
						Rcel* work_screen)
{
	...
	return (*(pd->save_frames))(ifile, screen, num_frames, seek_frame, seek_data, work_screen);
}
