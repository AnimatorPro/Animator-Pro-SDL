...
static Errcode open_pict_file(Pdr* pd, char* path, Image_file** pif, Anim_info* ainfo)
/* Opens an existing pict file and querys for header info. */
{
	...
	printf("top %d left %d right %d bot %d\n", ph.frame.top, ph.frame.left, ph.frame.right,
		   ph.frame.bot);
#endif
 ...
 * screen. */
{
	...
	stuff_bytes(0, screen->cmap->ctab, screen->cmap->num_colors * sizeof(Rgb3));
...
static Errcode create_pict_file(Pdr* pd, char* path, Image_file** pif, Anim_info* ainfo)
