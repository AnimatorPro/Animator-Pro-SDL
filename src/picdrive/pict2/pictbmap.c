...
{
	static USHORT pmswaps[] = {SET_OFFSET(pixMap, rowBytes),
							   SWAP_WORDS(7),
							   SWAP_LONGS(1),
...
{
	static USHORT ctswaps[] = {SWAP_LONGS(1), SWAP_WORDS(2), END_SWAPTAB};
 ...
 * them to the screen via the transfer methods selected. */
{
	...
	while (height-- > 0) {
		...
		if ((ret = pf->unpack_line(pf->readbuf, pf->bitbuf, pf->pm.rowBytes)) < Success) {
			goto reterr;
		}
 ...
 * on the pf->mode. */
{
	...
	printf(
		"srect t %d l %d b %d r %d\n"
		"drect t %d l %d b %d r %d\n"
		"tmode %d\n",
		pf->srcRect.top, pf->srcRect.left, pf->srcRect.bot, pf->srcRect.right,

		pf->dstRect.top, pf->dstRect.left, pf->dstRect.bot, pf->dstRect.right,
...
/* Read next available RGB line in the file. */
{
	...
	if (pf->dstRect.left > 0 || pf->dstRect.right > pf->ainfo.width) {
		...
		pf->make_pixels(pf->bitbuf, (UBYTE*)linebuf, OPTR(linebuf, pf->pixwidth), pf->pm.rowBytes);
	}
