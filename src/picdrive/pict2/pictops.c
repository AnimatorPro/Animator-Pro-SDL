...
/* for now pen size 1 only */
{
	...
	pj_set_hline(pf->screen, 40, pf->lastRect.left, pf->lastRect.top, pf->lastRWidth);
	pj_set_hline(pf->screen, 40, pf->lastRect.left + 1, pf->lastRect.bot, pf->lastRWidth);
	pj_set_vline(pf->screen, 40, pf->lastRect.left, pf->lastRect.top + 1, pf->lastRHeight);
	pj_set_vline(pf->screen, 40, pf->lastRect.right, pf->lastRect.top, pf->lastRHeight);
	return (Success);
}
...
static Errcode do_fillSameRect(Pfile* pf)
{
	...
	pj_set_rect(pf->screen, 20, pf->lastRect.left, pf->lastRect.top, pf->lastRWidth,
				pf->lastRHeight);
	return (Success);
}
#endif /* DO_DRAWOPS */
