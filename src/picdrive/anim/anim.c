...
Errcode conv_bitmaps(UBYTE* planes[], int pcount, int bpr, int width, int height, Rcel* screen);
...
#ifdef DEBUG
	printf("bmhd: w %d h %d x %d y %d nPlanes %d compression %d masking %#x", bm->w, bm->h, bm->x,
		   bm->y, bm->nPlanes, bm->compression, bm->masking);
#endif /* DEBUG */
	return (Success);
}

static Errcode find_bmhd(FILE* f, struct BitMapHeader* bm, long size, long file_pos)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	while (file_pos < size) {
		...
		if (is_ilbm(&chunk)) {
			if (!gf->got_bufs) {
				if ((err = find_bmhd(f, &gf->bh, chunk.fc_length - 4, file_pos + sizeof(chunk))) <
					Success) {
					goto ERROR;
				}
...
static Errcode read_frame(Ifile* gf, long size, Planes* p, Boolean is_dlta)
/*****************************************************************************
...
static Errcode read_ilbm(Ifile* gf, Rcel* screen, Planes* p, long size, long pos)
/*****************************************************************************
 ...
 ****************************************************************************/
{
	...
	while (pos < end) {
		...
		if (memcmp(chunk.iff_type.b4_name, "CMAP", 4) == 0) {
			...
			if ((err = read_frame(gf, chunk.iff_length, p, FALSE)) < Success) {
				goto ERROR;
			}
			...
			if ((err = read_frame(gf, chunk.iff_length, p, TRUE)) < Success) {
				goto ERROR;
			}
			...
			printf(
				...
				gf->ah.operation, gf->ah.mask, gf->ah.w, gf->ah.h, gf->ah.x, gf->ah.y,
				gf->ah.interleave, gf->ah.bits);
#endif /* DEBUG */
 ...
 ****************************************************************************/
{
	...
	if ((err = read_ilbm(gf, screen, p, chunk.fc_length - 4, ftell(f))) < Success) {
		goto ERROR;
	}
	++gf->frame_ix;

CONV_SCREEN:
	err = conv_bitmaps(p->p, gf->bh.nPlanes, gf->bpr, gf->bh.w, gf->bh.h, screen);
