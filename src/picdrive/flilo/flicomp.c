...
static LONG flow_comp_rect(void* comp_buf, Rcel* last_screen, Rcel* this_screen, Rectangle* rect,
 ...
 * must be within both screens provided */
{
	...
	if (do_colors) {
		...
		if (type == FLI_BRUN || type == FLI_COLOR_0) {
			...
			vp = comp_cmap(last_screen->cmap->ctab, this_screen->cmap->ctab, chunk + 1);
		}
	...
	switch (type) {
		case FLI_LC:
			vp = pj_lccomp_rects((Raster*)last_screen, chunk + 1, rect->x, rect->y,
								 (Raster*)this_screen, rect->x, rect->y, rect->width, rect->height);
			break;
		case FLI_BRUN:
			vp = flow_brun_rect((Raster*)this_screen, chunk + 1, rect->x, rect->y, rect->width,
								rect->height);
			break;
	}
...
LONG flow_comp_cel(void* comp_buf, Rcel* last_screen, Rcel* this_screen, SHORT type)
