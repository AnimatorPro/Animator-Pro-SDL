...
static Errcode flow_add_next(Flifile* flif, void* cbuf, Rcel* last_screen, Rcel* this_screen)
...
static Errcode flow_add_ring(Flifile* flif, void* cbuf, Rcel* last_screen, Rcel* first_screen)
...
{
	if (ainfo->depth == 8 && ainfo->num_frames <= MAXFRAMES && ainfo->width == 320 &&
		ainfo->height == 200) {
		return (true);
	}
...
static Errcode create_flow_file(Pdr* pd, char* path, Image_file** pif, Anim_info* ainfo)
{
	Errcode err;
	Flifile* flif;
	(void)pd;
...
static Errcode flow_save_frames(Image_file* ifile, Rcel* screen, ULONG num_frames,
...
static Pdr flilo_pdr_header = {
	...
	".FLI",     /* default_suffi */
	MAXFRAMES,
	0,                  /* max_write_frames, max_read_frames */
