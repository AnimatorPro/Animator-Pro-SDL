 ...
 ****************************************************************************/
{
	...
	if (lastoffset < tf->off_ifd_start) { /* strips before ifd in file... */
		dmyoffset = tf->off_ifd_start;
	} else { /* strips after ifd in file...	*/
 ...
 ****************************************************************************/
{
	...
	while (entry_count--) {
		...
		switch (entry.tag) {
			...
			case TAG_BITS_PER_SAMPLE:
				...
				switch (entry.count) {
					...
					case 3:
						...
						if (tf->bits_per_sample[0] != 8 || tf->bits_per_sample[1] != 8 ||
			...
			case TAG_SHORT_STRIP_OFFSETS: /* Aldus private tag 32768 */
				...
				if (Success !=
					(err = read_strip_offsets(tf, entry.value.offset, entry.type, entry.count))) {
					return err;
				}
				break;
			...
			case TAG_STRIP_BYTE_COUNTS:
				...
				if (Success != (err = read_strip_bytecounts(tf, entry.value.offset, entry.type,
															entry.count))) {
					return err;
				}
				break;
	...
	tf->pixel_depth = tf->bits_per_sample[0] + tf->bits_per_sample[1] + tf->bits_per_sample[2];
	...
	if (tf->width == 0 || tf->height == 0 || tf->strips_per_image == 0 || tf->longest_strip == 0) {
		return Err_format;
	}
