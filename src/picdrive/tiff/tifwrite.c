...
static Errcode write_stripdata_array(Tiff_file* tf, char* wrkbuf, long* offsets, long* counts)
/*****************************************************************************
...
static Errcode write_next_dirent(Tiff_file* tf, short tag, short type, long count, long offset)
/*****************************************************************************
...
#define W(a, b, c, d) \
	if (Success != (err = write_next_dirent(tf, a, b, c, d))) return err;
	...
	switch (tf->photometric) {
		case PHMET_RGB:
			...
			if (1 != fwrite(tf->bits_per_sample, sizeof(tf->bits_per_sample), 1, tf->file)) {
				return Err_truncated;
			}
			break;
...
Errcode write_strip(Tiff_file* tf, char* stripbuf, long striplen, int stripnum)
/*****************************************************************************
