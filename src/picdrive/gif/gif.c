...
static Errcode read_gif_start(Gif_file* gf, struct gif_header* ghdr, struct gif_image* gimg,
...
/* attempt non destructive verify and reading of info from a gif file */
{
	...
	if ((0 != memcmp(ghdr->giftype, gifsig, 3)) ||
		(!(isdigit(ghdr->giftype[3]) && isdigit(ghdr->giftype[4]) && islower(ghdr->giftype[5])))) {
		err = Err_bad_magic;
		goto error;
	}
...
static Errcode gif_open_ifsub(Gif_file** gifile, char* path, enum XReadWriteMode mode)
{
	Errcode err = Success;
	Gif_file* gf;
...
static Errcode open_gif_file(Pdr* pd, char* path, Image_file** pif, Anim_info* ainfo)
{
	Errcode err;
	Gif_file** pgif;
	...
	if ((err = read_gif_start(*pgif, &ghdr, &gimg, NULL, &((*pgif)->ainfo))) < Success) {
		goto error;
	}
...
static Errcode create_gif_file(Pdr* pd, char* path, Image_file** pif, Anim_info* ainfo)
{
	Errcode err;
	Gif_file** pgif;
	(void)pd;
