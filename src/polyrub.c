...
void rub_poly_points(Poly* p, bool closed, Pixel dit_color, Pixel dot_color, Pixel pt_color,
					 Pixel pt1_color)
{
	Marqihdr mh;
...
void move_poly_points(Poly* poly, bool closed)
{
	...
	save_undo();
	for (;;) {
		rub_poly_points(poly, closed, vs.ccolor, vs.ccolor, vb.screen->SBRIGHT, vb.screen->SBRIGHT);
		wait_wndo_input(ANY_CLICK);
		if (JSTHIT(MBRIGHT | KEYHIT) || (lp = closest_point(poly, icb.mx, icb.my, &dist)) == NULL) {
		...
		for (;;) {
			...
			lp->x = icb.mx;
			lp->y = icb.my;
			rub_poly_points(poly, closed, vs.ccolor, vs.ccolor, vb.screen->SBRIGHT,
							vb.screen->SBRIGHT);
		}
	}
}
...
static Errcode rub_sz_polys(Mpl_data* mpl, Poly** origs, int pcount, int* pp, int* pq)
  ...
  scale. */
{
	...
	doval(cent.x, cent.y, q, vb.pencel->aspect_dx, vb.pencel->aspect_dy, cd.mh.pdot, &cd.mh, NULL,
		  NULL, false);
	save_undo();
	dests = mpl->polys;
	for (;;) {
...
Errcode rub_keep_star_type(Poly* p, int startype, int color, int* ptheta, int* prad, int* prad2)
   ...
   must be initialized) */
{
	...
	if (startype == WP_ELLIPSE) {
		...
		err = polystar_loop(p, startype, color, color, icb.mx, icb.my, ptheta, prad);
	}
...
#ifdef SLUFFED
Errcode rub_star_type(int startype, int color, int* ptheta, int* prad, int* prad2)
...
static Errcode shape_loop(Poly* poly, LLpoint* this, Pixel oncolor, Pixel offcolor)
{
	Marqihdr mh;
	LLpoint* last;
	...
	for (;;) {
		...
		if (ISDOWN(MBPEN)) {
			...
			pj_cline(last->x, last->y, this->x, this->y, mh.pdot, &mh);
		} else {
			break;
		}
	}
