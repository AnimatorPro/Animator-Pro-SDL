...
static Errcode new_smu_symbol(Smu_symbol** psym, Smu_class* class, char* name)
{
	Smu_symbol* newsym;
...
static Errcode skip_to_match(Swork* swork, int uptype, int downtype, int ipos)
{
	SHORT ts;
...
static Errcode smu_parse_one(Swork* swork, Softmenu* s)
{
	...
	if (swork_top(swork)->ttype == TOK_UNDEF && swork_tok(swork, 1)->ttype == TOK_UNDEF &&
		swork_tok(swork, 2)->ttype == TOK_LBRACE) {
		if ((err = classix = class_array_ix(s->classes, s->class_count, swork_top(swork)->d.buf)) <
			Success) {
			goto ERR;
		}
		if ((err = new_smu_symbol(&sym, &s->classes[classix], swork_tok(swork, 1)->d.buf)) <
			Success) {
			goto ERR;
		}
...
char* smu_class_names[] = {
	"Strings", "Qchoice", "NumString", "Pull", "Text", "NameString",
};

Errcode smu_init(struct softmenu* smu, /* constructor */
				 char* resource_file)  /* resource file name */
{
	return (smu_init_classes(smu, smu_class_names, Array_els(smu_class_names), resource_file));
}
				   ...
				   char* name)               /* symbol name */
{
	...
	if ((*psym = (Smu_symbol*)name_in_list(name, (Names*)(sm->classes[class].symbols))) == NULL) {
		return (Err_not_found);
	}
	return (Success);
}
