...
static Errcode smu_do_name_things(struct softmenu* sm, char* symname,
								  Errcode (*dothing)(Swork* swork, void* data), void* data)
/*************************************************************************
...
static Errcode nametext_err(Errcode err, char* symname, char* strname)
{
	...
	return (
		errline(err, "?? %s %s %s ??", smu_class_names[SMU_NAMESTRING_CLASS], symname, strname));
}
						   ...
						   char** ptext)        /* place to put text */
{
	...
	*ptext = NULL; /* in case we don't find anything... */
	...
	if ((err = smu_do_name_things(sm, symname, found_string_to_dstring, &sdsl)) < Success) {
		goto error;
	}
...
static struct sctlist* sct_in_list(struct sctlist* lst, char* name, struct sctlist** pprev)
{
	struct sctlist* prev;
 ...
 * If an error is detected loaded string pointers will be NULLED out */
{
	...
	*allocd = NULL;
	...
	if ((err = smu_do_name_things(sm, symname, load_name_scatter, &sl)) < Success) {
...
error: /* note success gets you here too */
	...
	if (err < Success) {
		...
		err = softerr(err, "!%s%s%s", "no_smu_class_item", smu_class_names[SMU_NAMESTRING_CLASS],
					  symname, sl.item ? sl.item->sct->name : "");
	} else {
