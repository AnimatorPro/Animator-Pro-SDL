...
static int shrink_uncompfli(Rcel* f, Chunk_id* chunk, int count, int sw, int sh, int dw, int dh,
							bool do_colors)
...
static Errcode get_fli_pstamp(Rcel* pscel, Flifile* flif, SHORT stampw, SHORT stamph)
 ...
 * fli into for making a translation table */
{
	...
	if (not_a_pstamp || frec.ps.data.type == FPS_XLAT256) {
		ctype = shrink_uncompfli(pscel, (Chunk_id*)&(psframe->ps), frec.ff.chunks, flif->hdr.width,
								 flif->hdr.height, stampw, stamph, not_a_pstamp);

		if (not_a_pstamp) /* if we don't have a translation record, make one */
		{
			/* only one color needed for a one color frame */

			pj_make_pstamp_xlat(pscel->cmap->ctab, tctab, ctype == FLI_COLOR_0 ? 1 : COLORS);
	...
	{
		if ((err = shrink_uncompfli(pscel, (Chunk_id*)&(psframe->ps.data), 1, frec.ps.width,
									frec.ps.height, stampw, stamph, false)) < Success) {
			goto error;
		}
	}
 ...
 */
static Errcode get_pic_pstamp(Rcel* smallcel, XFILE* xf, Pic_header* pic, SHORT stampw,
							  SHORT stamph)
{
	Errcode err;
	Rcel* pcel;
	...
	pj_scale_blit(pcel, 0, 0, pic->width, pic->height, smallcel, 0, 0, stampw, stamph, NULL);
...
Errcode postage_stamp(Raster* r, char* name, SHORT x, SHORT y, USHORT width, USHORT height,
					  Rectangle* actual)
{
	Errcode err;
	XFILE* xf;
