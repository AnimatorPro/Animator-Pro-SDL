...
{
	pj_get_hseg((Raster*)cd->mh.w, cd->hline, 0, cd->lasty, cd->mh.port.width);
	pj_get_vseg((Raster*)cd->mh.w, cd->vline, cd->lastx, 0, cd->mh.port.height);
	cd->doneonce = 1;
}
...
static void rest_cut(Cutdata* cd, bool zoomed)
{
	...
	(*(cd->puthseg))((Raster*)cd->mh.w, cd->hline, 0, cd->lasty, cd->mh.port.width);
	(*(cd->putvseg))((Raster*)cd->mh.w, cd->vline, cd->lastx, 0, cd->mh.port.height);
}
...
{
	return (get_cut(vb.screen->SBLACK, vb.screen->SWHITE, vb.screen->SGREY));
}
