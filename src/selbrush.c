...
static Button gel_brush_sel = MB_INIT1(NONEXT, NOCHILD, 136, 49, 1, 4, NOTEXT, see_gel_brush,
...
static Button gel_brush_sl_sel =
	MB_INIT1(&gel_brush_sel, NOCHILD, 126, 11, 6, 64, &gel_brush_sl, see_abovetext_qslider,
			 feel_qslider, NOOPT, NODATA, 0, NOKEY, 0);
Button gel_brush_group = MB_INIT1(NONEXT, &gel_brush_sl_sel, 138, 82, 0, 0, NODATA, NOSEE, NOFEEL,
								  NOOPT, NOGROUP, 0, NOKEY, 0);

/* ------------- block for brush selector options ------------------ */

static Button bmu_circle_sel = MB_INIT1(NONEXT, NOCHILD, 23, 20, 115, 0, NOTEXT, see_circle_button,
										mb_set_brush_type, mb_set_brush_type, &vs.pen_brush_type,
										CIRCLE_BRUSH, NOKEY, MB_B_GHILITE | MB_SCALE_ABS);
static Button bmu_square_sel = MB_INIT1(
	&bmu_circle_sel, NOCHILD, 23, 20, 115, 19, NOTEXT, see_square_button, mb_set_brush_type,
	mb_set_brush_type, &vs.pen_brush_type, SQUARE_BRUSH, NOKEY, MB_B_GHILITE | MB_SCALE_ABS);
static Button bmu_line_sel =
	MB_INIT1(&bmu_square_sel, NOCHILD, 23, 20, 115, 38, NOTEXT, see_line_button, mb_set_brush_type,
			 rclick_line_sel, &vs.pen_brush_type, LINE_BRUSH, NOKEY, MB_B_GHILITE | MB_SCALE_ABS);

static Button bmu_langlable_sel =
	MB_INIT1(NONEXT, NOCHILD, 37, 11, 1, 44, NODATA, /* "Angle", */
			 black_label, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABS);

static SHORT lang;
static Qslider lang_sl = QSL_INIT1(-90, 90, &lang, 0, update_brush_sel_with_data, leftright_arrs);

static Button bmu_langsl_sel =
	MB_INIT1(&bmu_langlable_sel, NOCHILD, 78, 11, 38, 44, &lang_sl, see_lang_slider,
			 feel_lang_slider, zero_lang_slider, NOGROUP, 0, NOKEY, MB_SCALE_ABS);

static Button bmu_langhang_sel = MB_INIT1(&bmu_langsl_sel, NOCHILD, 110, 52, 3, 3, &lang_sl, NOSEE,
										  NOFEEL, NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABSW);

static Button bmu_brush_sel =
	MB_INIT1(&bmu_line_sel, NOCHILD, /* may have langhang attached */
			 110, 52, 3, 3, NOTEXT, see_brush_sel, NOFEEL, NOOPT, NULL, 0, NOKEY, MB_SCALE_ABS);

static SHORT pwidth;
static Qslider bsize_sl = QSL_INIT1(0, 31, &pwidth, 1, update_brush_sel_with_data, leftright_arrs);

static Button bmu_sizesl_sel =
	MB_INIT1(&bmu_brush_sel, NOCHILD, 138, 11, 0, 57, &bsize_sl, see_bsize_slider,
			 feel_bsize_slider, NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABSW);
static Button bmu_sizelab_sel =
	MB_INIT1(&bmu_sizesl_sel, NOCHILD, 81, 9, 1, 70, NODATA, /* "Brush Size", */
			 black_label, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABS);

#ifdef NOT_USED /* at present we can't load and save brushes */
static Button bmu_load_sel = MB_INIT1(&bmu_sizelab_sel, NOCHILD, 53, 9, 82, 70, "Load",
									  ccorner_text, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, 0);
#endif /* NOT_USED */

Button pen_brush_group = MB_INIT1(&bmu_sizelab_sel, NOCHILD, 138, 82, 0, 0, NOTEXT, NOSEE, NOFEEL,
								  NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABS);

static Button bmu_hanger_sel =
	MB_INIT1(NONEXT, NOCHILD, 138, 82, 0, 8, NODATA,                       /* datme */
			 hang_children, NOFEEL, NOOPT, NOGROUP, 0, NOKEY, MB_SCALE_ABS /* flags */
	);

static Button bmu_title_sel =
	MB_INIT1(&bmu_hanger_sel, NOCHILD, 138, 9, 0, 0, /* w,h,x,y */
			 NODATA,                                 /* "Brush menu", */
			 see_titlebar, feel_titlebar, NOOPT, &tbg_moveclose, 0, NOKEY, MB_SCALE_ABS /* flags */
	);
...
Errcode nest_alloc_brush_texts(void)
{
	...
	return (soft_buttons("brush_panels", sel_smblist, Array_els(sel_smblist), &smbs));
}
...
{
	wbg_ncorner_back(b);
	circle(b->root, wbg_textcolor(b), b->x + (b->width >> 1), b->y + (b->height >> 1),
...
static void see_square_button(Button* b)
{
	...
	pj_set_rect(b->root, wbg_textcolor(b), b->x + (b->width >> 1) - rad,
				b->y + (b->height >> 1) - rad, rad * 2, rad * 2);
}
...
static void see_line_button(Button* b)
{
	...
	line(b->root, wbg_textcolor(b), cent.x + rad, cent.y - rad, cent.x - rad, cent.y + rad);
}
...
static void see_brush_sel(Button* b)
{
	...
	switch (vs.pen_brush_type) {
		...
		case SQUARE_BRUSH:
			++size;
			pj_set_rect(&cb, mc_grey(b), cent.x - (size >> 1), cent.y - (size >> 1), size, size);
			break;
		case LINE_BRUSH:
