...
static Errcode init_mag_table(short** ptable, Poly* source, int ix, int iy, long pthresh,
							  int mag_mode)
...
static void mag_transform(short* mag_table, Poly* source, Poly* dest, int dx, int dy)
...
Errcode mag_loop(Poly* poly, Pixel color, long min_dist, int mag_mode)
{
	...
	if ((err = init_mag_table(&mag_table, &opoly, ix, iy, min_dist, mag_mode)) < Success) {
