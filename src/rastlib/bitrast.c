...
static void _bim_get_hseg(Raster* bitmap, Pixel* pixbuf, Coor x, Coor y, Ucoor width)
...
static Errcode _bim_blitrect(Raster* source_bitmap,  /* source raster */
							 ...
							 Coor dest_x, Coor dest_y, Ucoor width, Ucoor height) /* blit size */
{
	Bitmap* src = (Bitmap*)source_bitmap;

	pj_mask2blit(src->bm.bp[0], src->bm.bpr, src_x, src_y, dest, dest_x, dest_y, width, height, 1,
				 0);
...
/* sets entire raster to a color fast only for one plane now */
{
	...
	if (color & 0x01) {
		pj_stuff_words((USHORT)(~0), r->bm.bp[0], (USHORT)(r->bm.psize >> 1));
		if (r->bm.psize & 0x01) {
...
static void _bim_xor_rect(Raster* bitmap, Pixel color, Coor x, Coor y, Ucoor width, Ucoor height)
{
	Bitmap* bm = (Bitmap*)bitmap;
