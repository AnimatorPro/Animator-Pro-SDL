 ...
 * and vertically by zoom_y */
{
	if (source->type == dest->type) {
		return (*((source)->lib->zoomblit[RL_TO_SAME]))(source, src_x, src_y, dest, dest_x, dest_y,
														width, height, zoom_x, zoom_y);
	}
	if (dest->type == RT_BYTEMAP) {
		return (*((source)->lib->zoomblit[RL_TO_BYTEMAP]))(source, src_x, src_y, dest, dest_x,
														   dest_y, width, height, zoom_x, zoom_y);
	}
	if (source->type == RT_BYTEMAP) {
		return (*((dest)->lib->zoomblit[RL_FROM_BYTEMAP]))(source, src_x, src_y, dest, dest_x,
														   dest_y, width, height, zoom_x, zoom_y);
	}
	return (*((source)->lib->zoomblit[RL_TO_OTHER]))(source, src_x, src_y, dest, dest_x, dest_y,
													 width, height, zoom_x, zoom_y);
}
