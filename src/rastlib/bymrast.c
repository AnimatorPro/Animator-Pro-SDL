...
extern void pj_bym_dto_ptable(Bytemap* r, Pixel* pt, Ucoor psz, Coor x0, Coor y0, Coor x1, Coor y1);
...
void pj_bym_set_rect(Bytemap* bm, Pixel color, Coor x, Coor y, Ucoor width, Ucoor height);
void pj_bym_xor_rect(Bytemap* bm, Pixel color, Coor x, Coor y, Ucoor width, Ucoor height);
void pj_bym_mask1blit(UBYTE* mbytes, Coor mbpr, Coor mx, Coor my, Bytemap* r, Coor rx, Coor ry,
					  Ucoor width, Ucoor height, Pixel oncolor);
void bym_mask2blit(UBYTE* mbytes, Coor mbpr, Coor mx, Coor my, Bytemap* r, Coor rx, Coor ry,
				   Ucoor width, Ucoor height, Pixel oncolor, Pixel offcolor);

void pj_bli8(Bytemap* dv, int dx, int dy, int w, int h, int sx, int sy, Bytemap* sv);

void pj_tbli8(Bytemap* dv, int dx, int dy, int w, int h, int sx, int sy, Bytemap* sv, int tcolor);
void zoom2blit(int swidth, int dheight, int sx, int sy, Pixel* sp, int spseg, int sbpr, int dx,
			   int dy, Pixel* dp, int dpseg, int dbpr);
void zoom4blit(int swidth, int dheight, int sx, int sy, Pixel* sp, int spseg, int sbpr, int dx,
			   int dy, Pixel* dp, int dpseg, int dbpr);
void* pj_unlccomp(void* csource, Pixel* byteplane, int plane_bpr, USHORT plane_segment);

/**** written in "c" here *************/

static void bym_get_rectpix(Bytemap* r, void* pixbuf, Coor x, Coor y, Ucoor width, Ucoor height)
...
static void bym_put_rectpix(Bytemap* r, void* pixbuf, Coor x, Coor y, Ucoor width, Ucoor height)
...
static void bym_mask2blit(UBYTE* mbytes, Coor mbpr, Coor mx, Coor my, Bytemap* r, Coor rx, Coor ry,
						  Ucoor width, Ucoor height, Pixel oncolor, Pixel offcolor)
...
static void bym_unlccomp_rect(Bytemap* r, void* ucbuf, LONG pixsize, LONG x, LONG y, Ucoor width,
							  Ucoor height)
{
	pj_unlccomp(ucbuf, ((Bytemap*)r)->bm.bp[0] + y * r->bm.bpr + x, r->bm.bpr, r->bm.segment);
}

static void bym_unss2_rect(Bytemap* r, void* ucbuf, LONG pixsize, LONG x, LONG y, Ucoor width,
						   Ucoor height)
{
	pj_unss2(ucbuf, ((Bytemap*)r)->bm.bp[0] + y * r->bm.bpr + x, r->bm.bpr, r->bm.segment, width);
}
...
{
	pj_tbli8(dest, dest_x, dest_y, width, height, src_x, src_y, source, tcolor);
}
...
static void pj_bym_put_dot(Raster* bytemap, Pixel col, Coor x, Coor y)
{
	Bytemap* r = (Bytemap*)bytemap;
	(r->bm.bp[0])[r->bm.bpr * y + x] = col;
}

static Pixel pj_bym_get_dot(Raster* bytemap, Coor x, Coor y)
{
	Bytemap* r = (Bytemap*)bytemap;
...
Rastlib* pj_get_bytemap_lib(void)
{
	...
	if (!loaded) {
		...
		bytemap_lib.blitrect[RL_TO_SAME] = (rl_type_blitrect)bym_blitrect;
		bytemap_lib.blitrect[RL_TO_BYTEMAP] = (rl_type_blitrect)bym_t_blitrect;
		bytemap_lib.blitrect[RL_FROM_BYTEMAP] = (rl_type_blitrect)bym_f_blitrect;

		bytemap_lib.tblitrect[RL_TO_SAME] = (rl_type_tblitrect)bym_tblitrect;
		bytemap_lib.tblitrect[RL_TO_BYTEMAP] = (rl_type_tblitrect)bym_t_tblitrect;
		bytemap_lib.tblitrect[RL_FROM_BYTEMAP] = (rl_type_tblitrect)bym_f_tblitrect;

		bytemap_lib.xor_rast[RL_TO_SAME] = (rl_type_xor_rast)bym_xor_rast;
		bytemap_lib.xor_rast[RL_TO_BYTEMAP] = (rl_type_xor_rast)bym_t_xor_rast;
		bytemap_lib.xor_rast[RL_FROM_BYTEMAP] = (rl_type_xor_rast)bym_f_xor_rast;

#ifdef NOT_DONE
		bytemap_lib.zoomblit[RL_TO_SAME] = (rl_type_zoomblit)bym_zoomblit;
		bytemap_lib.zoomblit[RL_TO_BYTEMAP] = (rl_type_zoomblit)bym_t_zoomblit;
		bytemap_lib.zoomblit[RL_FROM_BYTEMAP] = (rl_type_zoomblit)bym_f_zoomblit;
#endif /* NOT_DONE */

#ifndef FLILIB_CODE
		bytemap_lib.diag_to_ptable = (rl_type_diag_to_ptable)pj_bym_dto_ptable;
#endif /* FLILIB_CODE */
