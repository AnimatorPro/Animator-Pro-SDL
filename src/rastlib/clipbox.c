...
static void pj__cbox_put_hseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor width)
{
	Clipbox* cb = (Clipbox*)clipbox;
	PUT_HSEG(cb->root, pbuf, x + cb->x, y + cb->y, width);
}

static void pj__cbox_get_hseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor width)
{
	Clipbox* cb = (Clipbox*)clipbox;
	GET_HSEG(cb->root, pbuf, x + cb->x, y + cb->y, width);
}

static void pj__cbox_put_vseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
	PUT_VSEG(cb->root, pbuf, x + cb->x, y + cb->y, height);
}

static void pj__cbox_get_vseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
	GET_VSEG(cb->root, pbuf, x + cb->x, y + cb->y, height);
}

static void pj__cbox_set_hline(Raster* clipbox, Pixel color, Coor x, Coor y, Ucoor width)
{
	Clipbox* cb = (Clipbox*)clipbox;
	SET_HLINE(cb->root, color, x + cb->x, y + cb->y, width);
}

static void pj__cbox_set_vline(Raster* clipbox, Pixel color, Coor x, Coor y, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
	SET_VLINE(cb->root, color, x + cb->x, y + cb->y, height);
}

static void pj__cbox_put_rectpix(Raster* clipbox, Pixel* pixbuf, Coor x, Coor y, Ucoor width,
								 Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	(cb->root->lib->put_rectpix)(cb->root, pixbuf, x + cb->x, y + cb->y, width, height);
}

static void pj__cbox_get_rectpix(Raster* clipbox, Pixel* pixbuf, Coor x, Coor y, Ucoor width,
								 Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	(cb->root->lib->get_rectpix)(cb->root, pixbuf, x + cb->x, y + cb->y, width, height);
}

static void pj__cbox_set_rect(Raster* clipbox, Pixel color, Coor x, Coor y, Ucoor width,
							  Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
...
static void pj__cbox_xor_rect(Raster* clipbox, Pixel color, Coor x, Coor y, Ucoor width,
							  Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
...
static void pj__cbox_mask1blit(UBYTE* mbytes, unsigned int mbpr, Coor mx, Coor my, Raster* clipbox,
							   Coor x, Coor y, Ucoor width, Ucoor height, Pixel oncolor)
{
	Clipbox* cb = (Clipbox*)clipbox;

	MASK1BLIT(mbytes, mbpr, mx, my, cb->root, x + cb->x, y + cb->y, width, height, oncolor);
}

static void pj__cbox_mask2blit(UBYTE* mbytes, unsigned int mbpr, Coor mx, Coor my, Raster* clipbox,
							   Coor x, Coor y, Ucoor width, Ucoor height, Pixel oncolor,
							   Pixel offcolor)
{
	Clipbox* cb = (Clipbox*)clipbox;

	MASK2BLIT(mbytes, mbpr, mx, my, cb->root, x + cb->x, y + cb->y, width, height, oncolor,
			  offcolor);
}

/* decompressors */

static void cbox_unbrun_rect(Raster* clipbox, void* ucbuf, LONG pixsize, Coor x, Coor y,
							 Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	(cb->root->lib->unbrun_rect)(cb->root, ucbuf, pixsize, x + cb->x, y + cb->y, width, height);
}

static void cbox_unlccomp_rect(Raster* clipbox, void* ucbuf, LONG pixsize, Coor x, Coor y,
							   Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	(cb->root->lib->unlccomp_rect)(cb->root, ucbuf, pixsize, x + cb->x, y + cb->y, width, height);
}

static Errcode cbox_unss2_rect(Raster* clipbox, void* ucbuf, LONG pixsize, Coor x, Coor y,
							   Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return (cb->root->lib->unss2_rect)(cb->root, ucbuf, pixsize, x + cb->x, y + cb->y, width,
									   height);
}

/***** binary calls *******/

static Errcode cbox_blitrect(Raster* clipbox, Coor x, Coor y, Raster* dest, Coor dest_x,
							 Coor dest_y, Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return pj__blitrect(cb->root, x + cb->x, y + cb->y, dest, dest_x, dest_y, width, height);
}

static Errcode cbox_f_blitrect(Raster* source, Coor src_x, Coor src_y, Raster* clipbox, Coor x,
							   Coor y, Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return (cb->root->lib->blitrect[RL_FROM_BYTEMAP])(source, src_x, src_y, cb->root, x + cb->x,
													  y + cb->y, width, height);
}

static void cbox_swaprect(Raster* clipbox, Coor x, Coor y, Raster* rastb, Coor rastb_x,
						  Coor rastb_y, Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	pj__swaprect(cb->root, x + cb->x, y + cb->y, rastb, rastb_x, rastb_y, width, height);
}

static void cbox_f_swaprect(Raster* rasta, Coor rasta_x, Coor rasta_y, Raster* clipbox, Coor x,
							Coor y, Ucoor width, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;

	(cb->root->lib->swaprect[RL_FROM_BYTEMAP])(rasta, rasta_x, rasta_y, cb->root, x + cb->x,
											   y + cb->y, width, height);
}

static Errcode cbox_tblitrect(Raster* clipbox, Coor x, Coor y, Raster* dest, Coor dest_x,
							  Coor dest_y, Ucoor width, Ucoor height, Pixel tcolor)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return pj__tblitrect(cb->root, x + cb->x, y + cb->y, dest, dest_x, dest_y, width, height,
						 tcolor);
}

static Errcode cbox_f_tblitrect(Raster* source, Coor src_x, Coor src_y, Raster* clipbox, Coor x,
								Coor y, Ucoor width, Ucoor height, Pixel tcolor)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return (cb->root->lib->tblitrect[RL_FROM_BYTEMAP])(source, src_x, src_y, cb->root, x + cb->x,
													   y + cb->y, width, height, tcolor);
}

static Errcode cbox_zoomblit(Raster* clipbox, Coor x, Coor y, Raster* dst, Coor dx, Coor dy,
							 Ucoor dw, Ucoor dh, LONG zxs, LONG zys)
{
	Clipbox* cb = (Clipbox*)clipbox;

	return pj_zoomblit(cb->root, x + cb->x, y + cb->y, dst, dx, dy, dw, dh, zxs, zys);
}
static Errcode cbox_f_zoomblit(Raster* src, Coor sx, Coor sy, Raster* clipbox, Coor x, Coor y,
							   Ucoor dw, Ucoor dh, LONG zxs, LONG zys)
 ...
 * called from the destination raster *****/
{
	...
	return (cb->root->lib->zoomblit[RL_FROM_BYTEMAP])(src, sx, sy, cb->root, x + cb->x, y + cb->y,
													  dw, dh, zxs, zys);
}
...
static void scbox_get_hseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor width)
{
	Clipbox* cb = (Clipbox*)clipbox;
	Ucoor rwid;
	Raster* root;
...
static void scbox_get_vseg(Raster* clipbox, Pixel* pbuf, Coor x, Coor y, Ucoor height)
{
	Clipbox* cb = (Clipbox*)clipbox;
	Ucoor rht;
	Raster* root;
 ...
 * and will read zeros with get_dot() and get_hseg() outside root rast */
{
	...
	if (!loaded) {
		...
		safe_cbox_lib.blitrect[RL_FROM_BYTEMAP] = (rl_type_blitrect)pj_sclip_blitrect;
		safe_cbox_lib.blitrect[RL_TO_SAME] = (rl_type_blitrect)pj_sclip_blitrect;
		safe_cbox_lib.blitrect[RL_TO_BYTEMAP] = (rl_type_blitrect)pj_sclip_blitrect;
		safe_cbox_lib.blitrect[RL_TO_OTHER] = (rl_type_blitrect)pj_sclip_blitrect;
		...
		safe_cbox_lib.swaprect[RL_TO_SAME] = (rl_type_swaprect)sclip_swaprect;
		safe_cbox_lib.swaprect[RL_TO_BYTEMAP] = (rl_type_swaprect)sclip_swaprect;
		safe_cbox_lib.swaprect[RL_FROM_BYTEMAP] = (rl_type_swaprect)sclip_swaprect;
		safe_cbox_lib.swaprect[RL_TO_OTHER] = (rl_type_swaprect)sclip_swaprect;

		safe_cbox_lib.tblitrect[RL_TO_SAME] = (rl_type_tblitrect)sclip_tblitrect;
		safe_cbox_lib.tblitrect[RL_TO_BYTEMAP] = (rl_type_tblitrect)sclip_tblitrect;
		safe_cbox_lib.tblitrect[RL_FROM_BYTEMAP] = (rl_type_tblitrect)sclip_tblitrect;
		safe_cbox_lib.tblitrect[RL_TO_OTHER] = (rl_type_tblitrect)sclip_tblitrect;
		...
		safe_cbox_lib.zoomblit[RL_TO_SAME] = (rl_type_zoomblit)sclip_zoomblit;
		safe_cbox_lib.zoomblit[RL_TO_BYTEMAP] = (rl_type_zoomblit)sclip_zoomblit;
		safe_cbox_lib.zoomblit[RL_FROM_BYTEMAP] = (rl_type_zoomblit)sclip_zoomblit;
		safe_cbox_lib.zoomblit[RL_TO_OTHER] = (rl_type_zoomblit)sclip_zoomblit;
...
bool pj_clipbox_make(Clipbox* cb, Raster* r, Coor x, Coor y, Coor width, Coor height)
 ...
 * repeatedly for moving the box */
{
	...
	*((Rasthdr*)cb) = *((Rasthdr*)r);
	...
	if (outside || r->width < (x + width) || r->height < (y + height)) {
