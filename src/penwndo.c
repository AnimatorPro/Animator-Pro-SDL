 ...
 * undof (the backup screen.) */
{
	...
	if (undof && (undof->width != width || undof->height != height)) {
		free_undof(); /* free old wrong size undo screen if any */
	}

	/* save original size in case have to back out */
	if (vb.pencel != NULL) { /* This bracket necessary due to wierdness in copy_rectfields
							  * macro */
	...
	if ((err = set_pencel_size(width, height, vl.flicent.x, vl.flicent.y)) < Success) {
		goto error;
	}
